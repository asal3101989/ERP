/**
 * migrate-dqs-sqlite.js
 * Migrates data from the old DQS ERP SQLite database (dqs_erp.db) into
 * the PostgreSQL dqs_bills, dqs_bill_updates, dqs_bill_history, and
 * dqs_material_tracker tables.
 *
 * Run from the backend directory:
 *   node src/scripts/migrate-dqs-sqlite.js
 */
...
const SQLITE_PATH = path.resolve(
  __dirname,
  '../../../DQS TRACKER/final01042026/dqs_erp.db'
);

async function loadSQLite() {
  const initSqlJs = require(path.resolve(
    __dirname,
    '../../../DQS TRACKER/final01042026/node_modules/sql.js'
  ));
...
async function run() {
  ...
  const existing = await query(`SELECT sl_number FROM dqs_bills`);
  ...
  const slToId = {};
  const idRows = await query(`SELECT id, sl_number FROM dqs_bills`);
  ...
  for (const h of history) {
    ...
    try {
      await query(`
        INSERT INTO dqs_bill_history (bill_id, dept, action, ts)
        VALUES ($1, $2, $3, $4)
  ...
  const existingMT = await query(`SELECT tracker_no FROM dqs_material_tracker`);
  ...
  for (const mt of mtItems) {
    ...
    try {
      await query(`
        INSERT INTO dqs_material_tracker (
  ...
  const totals = await query(`
    SELECT
      (SELECT COUNT(*) FROM dqs_bills) AS bills,
      (SELECT COUNT(*) FROM dqs_bill_updates) AS updates,
      (SELECT COUNT(*) FROM dqs_bill_history) AS history,
      (SELECT COUNT(*) FROM dqs_material_tracker) AS mt
  `);
  const t = totals.rows[0];
  console.log(`\n✅ PostgreSQL now has:`);
  console.log(`   dqs_bills:            ${t.bills}`);
  console.log(`   dqs_bill_updates:     ${t.updates}`);
  console.log(`   dqs_bill_history:     ${t.history}`);
  console.log(`   dqs_material_tracker: ${t.mt}`);
