// src/routes/dqs-transmittal.routes.js
...
async function ensureTables() {
  await query(`
    CREATE TABLE IF NOT EXISTS dqs_transmittals (
    ...
    CREATE TABLE IF NOT EXISTS dqs_transmittal_items (
      id              UUID PRIMARY KEY DEFAULT gen_random_uuid(),
      transmittal_id  UUID REFERENCES dqs_transmittals(id) ON DELETE CASCADE,
      sl_no           INT,
      dqs_bill_id     UUID,
      invoice_no      TEXT,
      invoice_date    DATE,
...
async function nextTransmittalNumber(companyId) {
  const res = await query(
    `SELECT COUNT(*) AS cnt FROM dqs_transmittals WHERE company_id = $1`,
    [companyId]
  );
  ...
  try {
    ...
    const rows = await query(`
      SELECT t.*,
             ...
             (SELECT COUNT(*) FROM dqs_transmittal_items ti WHERE ti.transmittal_id = t.id) AS bill_count,
             (SELECT COALESCE(SUM(ti.amount),0) FROM dqs_transmittal_items ti WHERE ti.transmittal_id = t.id) AS total_amount
      FROM dqs_transmittals t
...
router.get('/:id', async (req, res) => {
  try {
    ...
    const items = await query(
      `SELECT * FROM dqs_transmittal_items WHERE transmittal_id = $1 ORDER BY sl_no`,
      [req.params.id]
    );
    ...
    const result = await withTransaction(async (client) => {
      ...
      if (bill_ids.length) {
        const bills = await client.query(
          `SELECT id, inv_number, inv_date, po_number, po_date, vendor_name,
                  COALESCE(qs_total, total_amount, 0) AS amount
           FROM dqs_bills
        ...
        for (const bid of bill_ids) {
          ...
          if (!b) continue;
          await client.query(`
            INSERT INTO dqs_transmittal_items
              (transmittal_id, sl_no, dqs_bill_id, invoice_no, invoice_date,
...
// ── PATCH /dqs/transmittals/:id/submit ────────────────────────────────────
router.patch('/:id/submit', async (req, res) => {
  try {
    const r = await query(
      `UPDATE dqs_transmittals
...
// ── PATCH /dqs/transmittals/:id/receive ───────────────────────────────────
...
// ── GET /dqs/transmittals/lookup/bills ────────────────────────────────────
  ...
  try {
    ...
    const rows = await query(`
      SELECT b.id, b.sl_number, b.inv_number, b.inv_date, b.po_number, b.po_date,
             ...
             COALESCE(u.qs_total, b.total_amount, 0) AS amount,
             p.name AS project_name
      FROM dqs_bills b
