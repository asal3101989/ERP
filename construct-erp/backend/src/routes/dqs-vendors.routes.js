// src/routes/dqs-vendors.routes.js
// Unified vendor management — reads/writes the main `vendors` table.
// dqs_vendors legacy table is no longer written to; this file is kept as
...
async function ensureVendorColumns() {
  ...
  for (const sql of extras) {
    await query(sql).catch(() => {}); // ignore if column already exists
  }
}
ensureVendorColumns();

// ── GET /dqs/vendors — list from unified vendors table ───────────────────────
router.get('/', async (req, res) => {
  try {
    ...
    console.error('dqs-vendors GET error:', err);
    res.status(500).json({ error: 'Failed to fetch vendors' });
  }
});

// ── POST /dqs/vendors — create vendor in main vendors table ──────────────────
router.post('/', async (req, res) => {
  try {
    ...
    console.error('dqs-vendors POST error:', err);
    res.status(500).json({ error: 'Failed to create vendor' });
  }
});

// ── PUT /dqs/vendors/:id — update vendor in main vendors table ───────────────
router.put('/:id', async (req, res) => {
  try {
    ...
    console.error('dqs-vendors PUT error:', err);
    res.status(500).json({ error: 'Failed to update vendor' });
  }
});

// ── DELETE /dqs/vendors/:id — soft delete from main vendors table ────────────
router.delete('/:id', async (req, res) => {
  try {
    ...
    console.error('dqs-vendors DELETE error:', err);
