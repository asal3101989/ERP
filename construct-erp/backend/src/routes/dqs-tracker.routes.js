...
async function ensureTable() {
  await query(`
    CREATE TABLE IF NOT EXISTS dqs_material_tracker (
...
(async () => {
  const alts = [
    `ALTER TABLE dqs_material_tracker ADD COLUMN IF NOT EXISTS company_id UUID`,
    `ALTER TABLE dqs_material_tracker ADD COLUMN IF NOT EXISTS mr_date DATE`,
    `ALTER TABLE dqs_material_tracker ADD COLUMN IF NOT EXISTS material_head TEXT`,
    `ALTER TABLE dqs_material_tracker ADD COLUMN IF NOT EXISTS required_qty NUMERIC(14,3)`,
    `ALTER TABLE dqs_material_tracker ADD COLUMN IF NOT EXISTS pm_certification_date DATE`,
    `ALTER TABLE dqs_material_tracker ADD COLUMN IF NOT EXISTS qs_certification_date DATE`,
    `ALTER TABLE dqs_material_tracker ADD COLUMN IF NOT EXISTS po_date DATE`,
    `ALTER TABLE dqs_material_tracker ADD COLUMN IF NOT EXISTS ordered_qty NUMERIC(14,3)`,
    `ALTER TABLE dqs_material_tracker ADD COLUMN IF NOT EXISTS unit_price NUMERIC(14,2)`,
    `ALTER TABLE dqs_material_tracker ADD COLUMN IF NOT EXISTS basic_value NUMERIC(14,2)`,
    `ALTER TABLE dqs_material_tracker ADD COLUMN IF NOT EXISTS gst_amount NUMERIC(14,2) DEFAULT 0`,
    `ALTER TABLE dqs_material_tracker ADD COLUMN IF NOT EXISTS total_po_value NUMERIC(14,2)`,
    `ALTER TABLE dqs_material_tracker ADD COLUMN IF NOT EXISTS supplier_invoice_no TEXT`,
    `ALTER TABLE dqs_material_tracker ADD COLUMN IF NOT EXISTS supplier_invoice_date DATE`,
    `ALTER TABLE dqs_material_tracker ADD COLUMN IF NOT EXISTS supplier_invoice_qty NUMERIC(14,3)`,
    `ALTER TABLE dqs_material_tracker ADD COLUMN IF NOT EXISTS material_received_qty NUMERIC(14,3)`,
    `ALTER TABLE dqs_material_tracker ADD COLUMN IF NOT EXISTS invoice_forwarded_ho_date DATE`,
    `ALTER TABLE dqs_material_tracker ADD COLUMN IF NOT EXISTS invoice_received_qs_date DATE`,
    `ALTER TABLE dqs_material_tracker ADD COLUMN IF NOT EXISTS qty_certified_qs NUMERIC(14,3)`,
    `ALTER TABLE dqs_material_tracker ADD COLUMN IF NOT EXISTS basic_certified_amount NUMERIC(14,2)`,
    `ALTER TABLE dqs_material_tracker ADD COLUMN IF NOT EXISTS mob_advance NUMERIC(14,2) DEFAULT 0`,
    `ALTER TABLE dqs_material_tracker ADD COLUMN IF NOT EXISTS total_certified_amount NUMERIC(14,2)`,
    `ALTER TABLE dqs_material_tracker ADD COLUMN IF NOT EXISTS certified_to_accounts_date DATE`,
    `ALTER TABLE dqs_material_tracker ADD COLUMN IF NOT EXISTS current_stage TEXT DEFAULT 'I'`,
  ];
  for (const s of alts) { try { await query(s); } catch(e) { /* column may already exist */ } }
  // Backfill company_id from linked project
  try {
    await query(`
      UPDATE dqs_material_tracker t SET company_id = p.company_id
...
async function nextTrackerNo() {
  ...
  const { rows } = await query(
    `SELECT COUNT(*) AS cnt FROM dqs_material_tracker WHERE tracker_no LIKE $1`,
    [`MT-${yr}-%`]
  );
...
// ── GET /dqs/material-tracker/lifecycle (live cross-module view) ──────────────
router.get('/lifecycle', async (req, res) => {
  try {
    ...
    const { rows } = await query(`
      ...
      LEFT JOIN dqs_bills b ON b.po_number = po.po_number
...
router.post('/', async (req, res) => {
  try {
    ...
    const { rows } = await query(
      `INSERT INTO dqs_material_tracker (${cols.join(',')}) VALUES (${placeholders.join(',')}) RETURNING *`,
      vals
    );
...
// ── PUT /dqs/material-tracker/:id ────────────────────────────────────────────
router.put('/:id', async (req, res) => {
  try {
    const check = await query(
      `SELECT id FROM dqs_material_tracker WHERE id = $1 AND company_id = $2`,
    ...
    const { rows } = await query(
      `UPDATE dqs_material_tracker SET ${sets.join(', ')} WHERE id = $${i} RETURNING *`,
      vals
    );
...
// ── DELETE /dqs/material-tracker/:id ─────────────────────────────────────────
router.delete('/:id', async (req, res) => {
  try {
    const result = await query(
      `DELETE FROM dqs_material_tracker WHERE id = $1 AND company_id = $2 RETURNING id`,
      [req.params.id, req.user.company_id]
    );
