// src/routes/dqs-bills.routes.js
...
const storage = multer.diskStorage({
  destination: (req, file, cb) => {
    const dir = path.join(__dirname, '../../uploads/dqs-bills', req.params.id || 'tmp');
...
async function getBillProjectName(billId) {
  ...
  const pr = await query(`
    ...
    JOIN dqs_bills b ON b.project_id = p.id
    WHERE b.id = $1
  `, [billId]);
...
async function ensureTables() {
  await query(`
    CREATE TABLE IF NOT EXISTS dqs_bills (
    ...
    CREATE TABLE IF NOT EXISTS dqs_bill_updates (
      bill_id           UUID PRIMARY KEY REFERENCES dqs_bills(id) ON DELETE CASCADE,
    ...
    CREATE TABLE IF NOT EXISTS dqs_bill_line_items (
      id           UUID PRIMARY KEY DEFAULT gen_random_uuid(),
      bill_id      UUID REFERENCES dqs_bills(id) ON DELETE CASCADE,
      item_name    TEXT,
      unit         TEXT,
    ...
    CREATE TABLE IF NOT EXISTS dqs_bill_files (
      id          UUID PRIMARY KEY DEFAULT gen_random_uuid(),
      bill_id     UUID REFERENCES dqs_bills(id) ON DELETE CASCADE,
      file_name   TEXT,
      file_size   INT,
    ...
    CREATE TABLE IF NOT EXISTS dqs_bill_history (
      id          UUID PRIMARY KEY DEFAULT gen_random_uuid(),
      bill_id     UUID REFERENCES dqs_bills(id) ON DELETE CASCADE,
      dept        TEXT,
      action      TEXT,
  ...
  const alterBills = [
    `ALTER TABLE dqs_bills ADD COLUMN IF NOT EXISTS work_desc TEXT`,
    `ALTER TABLE dqs_bills ADD COLUMN IF NOT EXISTS tax_mode TEXT DEFAULT 'intrastate'`,
    `ALTER TABLE dqs_bills ADD COLUMN IF NOT EXISTS credit_note_num TEXT`,
    `ALTER TABLE dqs_bills ADD COLUMN IF NOT EXISTS credit_note_val NUMERIC(14,2) DEFAULT 0`,
    `ALTER TABLE dqs_bills ADD COLUMN IF NOT EXISTS transport_gst_pct NUMERIC(5,2) DEFAULT 0`,
    `ALTER TABLE dqs_bills ADD COLUMN IF NOT EXISTS transport_gst_amt NUMERIC(14,2) DEFAULT 0`,
    `ALTER TABLE dqs_bills ADD COLUMN IF NOT EXISTS transport_desc TEXT`,
    `ALTER TABLE dqs_bills ADD COLUMN IF NOT EXISTS other_charges_desc TEXT`,
    // ── Cross-module linkage (procurement) ─────────────────────────
    `ALTER TABLE dqs_bills ADD COLUMN IF NOT EXISTS po_id UUID`,
    `ALTER TABLE dqs_bills ADD COLUMN IF NOT EXISTS grn_id UUID`,
  ];
  const alterItems = [
    `ALTER TABLE dqs_bill_line_items ADD COLUMN IF NOT EXISTS category TEXT`,
    `ALTER TABLE dqs_bill_line_items ADD COLUMN IF NOT EXISTS item_code TEXT`,
    `ALTER TABLE dqs_bill_line_items ADD COLUMN IF NOT EXISTS gst_mode TEXT DEFAULT 'intrastate'`,
    `ALTER TABLE dqs_bill_line_items ADD COLUMN IF NOT EXISTS cgst_pct NUMERIC(5,2) DEFAULT 0`,
    `ALTER TABLE dqs_bill_line_items ADD COLUMN IF NOT EXISTS cgst_amt NUMERIC(14,2) DEFAULT 0`,
    `ALTER TABLE dqs_bill_line_items ADD COLUMN IF NOT EXISTS sgst_pct NUMERIC(5,2) DEFAULT 0`,
    `ALTER TABLE dqs_bill_line_items ADD COLUMN IF NOT EXISTS sgst_amt NUMERIC(14,2) DEFAULT 0`,
    `ALTER TABLE dqs_bill_line_items ADD COLUMN IF NOT EXISTS igst_pct NUMERIC(5,2) DEFAULT 0`,
    `ALTER TABLE dqs_bill_line_items ADD COLUMN IF NOT EXISTS igst_amt NUMERIC(14,2) DEFAULT 0`,
    // PO linkage — tracks which PO line item this bill line is drawn against
    `ALTER TABLE dqs_bill_line_items ADD COLUMN IF NOT EXISTS po_item_id UUID`,
  ];
  const alterUpdates = [
    `ALTER TABLE dqs_bill_updates ADD COLUMN IF NOT EXISTS ra_sequence INT DEFAULT 1`,
    `ALTER TABLE dqs_bill_updates ADD COLUMN IF NOT EXISTS ra_bill_number TEXT`,
    `ALTER TABLE dqs_bill_updates ADD COLUMN IF NOT EXISTS previous_certified_amount NUMERIC(14,2) DEFAULT 0`,
    `ALTER TABLE dqs_bill_updates ADD COLUMN IF NOT EXISTS cumulative_certified_amount NUMERIC(14,2) DEFAULT 0`,
    `ALTER TABLE dqs_bill_updates ADD COLUMN IF NOT EXISTS is_final_bill BOOLEAN DEFAULT FALSE`,
    `ALTER TABLE dqs_bill_updates ADD COLUMN IF NOT EXISTS pc_number TEXT`,
    `ALTER TABLE dqs_bill_updates ADD COLUMN IF NOT EXISTS pc_generated_at TIMESTAMPTZ`,
    `ALTER TABLE dqs_bill_updates ADD COLUMN IF NOT EXISTS pc_qs_sig_img TEXT`,
    `ALTER TABLE dqs_bill_updates ADD COLUMN IF NOT EXISTS pc_qs_signed_by TEXT`,
    `ALTER TABLE dqs_bill_updates ADD COLUMN IF NOT EXISTS pc_qs_signed_at TIMESTAMPTZ`,
    `ALTER TABLE dqs_bill_updates ADD COLUMN IF NOT EXISTS pc_pm_sig_img TEXT`,
    `ALTER TABLE dqs_bill_updates ADD COLUMN IF NOT EXISTS pc_pm_signed_by TEXT`,
    `ALTER TABLE dqs_bill_updates ADD COLUMN IF NOT EXISTS pc_pm_signed_at TIMESTAMPTZ`,
    `ALTER TABLE dqs_bill_updates ADD COLUMN IF NOT EXISTS pc_accts_sig_img TEXT`,
    `ALTER TABLE dqs_bill_updates ADD COLUMN IF NOT EXISTS pc_accts_signed_by TEXT`,
    `ALTER TABLE dqs_bill_updates ADD COLUMN IF NOT EXISTS pc_accts_signed_at TIMESTAMPTZ`,
    `ALTER TABLE dqs_bill_updates ADD COLUMN IF NOT EXISTS sent_to_ho_date DATE`,
    `ALTER TABLE dqs_bill_updates ADD COLUMN IF NOT EXISTS ho_received_date DATE`,
    `ALTER TABLE dqs_bill_updates ADD COLUMN IF NOT EXISTS handed_over_qs_date DATE`,
    `ALTER TABLE dqs_bill_updates ADD COLUMN IF NOT EXISTS document_controller_remarks TEXT`,
    `ALTER TABLE dqs_bill_updates ADD COLUMN IF NOT EXISTS handed_over_accounts_date DATE`,
    // ── Finance linkage ──
    `ALTER TABLE dqs_bill_updates ADD COLUMN IF NOT EXISTS payment_mode TEXT`,
    `ALTER TABLE dqs_bill_updates ADD COLUMN IF NOT EXISTS reference_number TEXT`,
    `ALTER TABLE dqs_bill_updates ADD COLUMN IF NOT EXISTS bank_name TEXT`,
    `ALTER TABLE dqs_bill_updates ADD COLUMN IF NOT EXISTS finance_payment_id UUID`,
    `ALTER TABLE dqs_bill_updates ADD COLUMN IF NOT EXISTS qs_summary_template JSONB DEFAULT '[]'::jsonb`,
    `ALTER TABLE dqs_bill_updates ADD COLUMN IF NOT EXISTS qs_ra_items JSONB DEFAULT '[]'::jsonb`,
    `ALTER TABLE dqs_bill_updates ADD COLUMN IF NOT EXISTS ra_cgst_pct NUMERIC(5,2) DEFAULT 9`,
    `ALTER TABLE dqs_bill_updates ADD COLUMN IF NOT EXISTS ra_sgst_pct NUMERIC(5,2) DEFAULT 9`,
    `ALTER TABLE dqs_bill_updates ADD COLUMN IF NOT EXISTS ra_igst_pct NUMERIC(5,2) DEFAULT 0`,
    `ALTER TABLE dqs_bill_updates ADD COLUMN IF NOT EXISTS accts_received_from_qs_date DATE`,
    `ALTER TABLE dqs_bill_updates ADD COLUMN IF NOT EXISTS proc_received_from_accounts_date DATE`,
    `ALTER TABLE dqs_bill_updates ADD COLUMN IF NOT EXISTS proc_handed_over_to_accounts_date DATE`,
    `ALTER TABLE dqs_bill_updates ADD COLUMN IF NOT EXISTS procurement_remarks TEXT`,
  ];
  // ── Extend payments table to accept DQS-originated records ──────────────
  const alterPayments = [
    `ALTER TABLE payments ADD COLUMN IF NOT EXISTS dqs_bill_id UUID`,
  ...
  const alterFiles = [
    `ALTER TABLE dqs_bill_files ADD COLUMN IF NOT EXISTS onedrive_id TEXT`,
    `ALTER TABLE dqs_bill_files ADD COLUMN IF NOT EXISTS onedrive_url TEXT`,
    `ALTER TABLE dqs_bill_files ADD COLUMN IF NOT EXISTS onedrive_web_url TEXT`,
  ];
...
async function nextSlNumber() {
  ...
  const res = await query(
    `SELECT sl_number FROM dqs_bills WHERE sl_number IS NOT NULL ORDER BY created_at DESC LIMIT 50`
  );
...
async function logHistory(billId, dept, action, userId) {
  await query(
    `INSERT INTO dqs_bill_history (bill_id, dept, action, changed_by) VALUES ($1,$2,$3,$4)`,
...
async function syncImportedFinancePayment(client, { billId, companyId, userId, bill, updates }) {
  ...
  const remarks = `DQS Bill ${bill.sl_number} - Inv: ${bill.inv_number || '-'}`;

  const existing = await client.query(
    `SELECT id FROM payments WHERE dqs_bill_id=$1 AND source='dqs' ORDER BY created_at DESC LIMIT 1`,
    [billId]
  );
    ...
    const inserted = await client.query(`
      INSERT INTO payments
        (project_id, payment_type, entity_name,
         ...
         remarks, created_by, dqs_bill_id, source)
  ...
  await client.query(
    `UPDATE dqs_bill_updates SET finance_payment_id=$1 WHERE bill_id=$2`,
    [paymentId, billId]
  );
...
async function nextPCNumber() {
  ...
  const { rows } = await query(
    `SELECT COUNT(*) AS cnt FROM dqs_bill_updates WHERE pc_number LIKE $1`,
    [`PC-${yr}-%`]
  );
...
// ── POST /dqs/bills ────────────────────────────────────────────────────────
// Import the legacy "DQS PO Bill Tracker.xlsx" workbook.
router.post('/import-excel', importUpload.single('file'), async (req, res) => {
  ...
  try {
    ...
    const summary = await withTransaction(async (client) => {
      ...
      for (const row of rows) {
        ...
        const billRes = await client.query(
          `SELECT id, project_id, bill_type, vendor_name, sl_number, inv_number FROM dqs_bills WHERE id=$1`,
          [billId]
        );
    ...
    logger.error('DQS tracker import failed', err);
    res.status(500).json({ error: err.message });
  }
});
...
router.post('/', async (req, res) => {
  try {
    ...
    const result = await withTransaction(async (client) => {
      ...
      await client.query(
        `INSERT INTO dqs_bill_updates (bill_id, balance_to_pay) VALUES ($1, $2)`,
        [billId, total_amount]
      );
      ...
      for (let idx = 0; idx < items.length; idx++) {
        ...
        await client.query(`
          INSERT INTO dqs_bill_line_items
...
// ── PATCH /dqs/bills/:id/meta — update project & package description ───────
router.patch('/:id/meta', async (req, res) => {
  try {
    const { project_id, work_desc } = req.body;
    await query(`UPDATE dqs_bills SET project_id=$1, work_desc=$2, updated_at=NOW() WHERE id=$3`,
...
router.get('/:id', async (req, res) => {
  try {
    const [bill, updates, items, files, history] = await Promise.all([
      ...
      `, [req.params.id]),
      query(`SELECT * FROM dqs_bill_updates WHERE bill_id = $1`, [req.params.id]),
      query(`SELECT * FROM dqs_bill_line_items WHERE bill_id = $1 ORDER BY sort_order`, [req.params.id]),
      query(`SELECT * FROM dqs_bill_files WHERE bill_id = $1 ORDER BY uploaded_at DESC`, [req.params.id]),
      query(`SELECT h.*, u.name AS changed_by_name FROM dqs_bill_history h LEFT JOIN users u ON u.id = h.changed_by WHERE h.bill_id = $1 ORDER BY h.ts DESC`, [req.params.id]),
    ]);
...
router.put('/:id', async (req, res) => {
  try {
    ...
    const r = await query(`UPDATE dqs_bills SET ${sets.join(', ')} WHERE id = $1 AND company_id = $2 RETURNING *`, params);
...
router.patch('/:id/stores', requireTqsStageAccess('stores'), async (req, res) => {
  try {
    ...
    await query(`UPDATE dqs_bills SET workflow_status='document_controller', updated_at=NOW() WHERE id=$1`, [req.params.id]);
    await logHistory(
      req.params.id,
      'stores',
...
// ── PATCH /dqs/bills/:id/document-control
router.patch('/:id/document-control', requireTqsStageAccess('document_control'), async (req, res) => {
  try {
    ...
    await query(`UPDATE dqs_bills SET workflow_status='qs', updated_at=NOW() WHERE id=$1`, [req.params.id]);
    await logHistory(
      req.params.id,
      'document_controller',
...
router.patch('/:id/qs', requireTqsStageAccess('qs'), async (req, res) => {
  try {
    ...
    const cur = await query(
      `SELECT vendor_name, po_number, company_id FROM dqs_bills WHERE id=$1`,
      [req.params.id]
    );
    ...
    await query(`UPDATE dqs_bills SET workflow_status='accounts', updated_at=NOW() WHERE id=$1`, [req.params.id]);
...
// ── POST /dqs/bills/:id/payment-certificate ────────────────────────────────
router.post('/:id/payment-certificate', async (req, res) => {
  try {
    // Check if a PC already exists — regenerate only if requested
    const existing = await query(`SELECT pc_number FROM dqs_bill_updates WHERE bill_id=$1`, [req.params.id]);
    let pc_number = existing.rows[0]?.pc_number;
    if (!pc_number) {
      pc_number = await nextPCNumber();
      await query(
        `UPDATE dqs_bill_updates SET pc_number=$1, pc_generated_at=NOW(), updated_at=NOW() WHERE bill_id=$2`,
...
// ── PATCH /dqs/bills/:id/payment-certificate/sign ─────────────────────────
...
router.patch('/:id/accounts', requireTqsStageAccess('accounts'), async (req, res) => {
  try {
    ...
    await query(`UPDATE dqs_bills SET workflow_status='procurement', updated_at=NOW() WHERE id=$1`, [req.params.id]);
...
router.patch('/:id/payment', requireTqsStageAccess('payment'), async (req, res) => {
  try {
    ...
    const updRow = await query(
      `SELECT certified_net, paid_amount, tds_deduction FROM dqs_bill_updates WHERE bill_id=$1`,
      [req.params.id]
    );
    ...
    const billRow = await query(`SELECT * FROM dqs_bills WHERE id=$1`, [req.params.id]);
    if (!billRow.rows.length) return res.status(404).json({ error: 'Bill not found' });
    const bill = billRow.rows[0];

    const result = await withTransaction(async (client) => {
      // 1. Update dqs_bill_updates
      await client.query(`
        UPDATE dqs_bill_updates SET
      ...
      await client.query(`UPDATE dqs_bills SET workflow_status=$1, updated_at=NOW() WHERE id=$2`,
      ...
      if (new_paid > 0 && payment_date && bill.project_id) {
        ...
        `, [
          ...
          req.user.id, req.params.id, 'dqs',
        ]);
        finance_payment_id = fp.rows[0].id;
        // Back-link on dqs_bill_updates
        await client.query(
          `UPDATE dqs_bill_updates SET finance_payment_id=$1 WHERE bill_id=$2`,
...
// ── POST /dqs/bills/:id/files ──────────────────────────────────────────────
router.post('/:id/files', upload.single('file'), async (req, res) => {
  if (!req.file) return res.status(400).json({ error: 'No file uploaded' });
  try {
    const local_url = `/uploads/dqs-bills/${req.params.id}/${req.file.filename}`;
...
// ── DELETE /dqs/bills/:id/files/:fid ──────────────────────────────────────
router.post('/:id/files/:fid/sync-onedrive', async (req, res) => {
  try {
    const fileResult = await query(`
      SELECT f.*, b.company_id
      FROM dqs_bill_files f
      JOIN dqs_bills b ON b.id = f.bill_id
...
router.delete('/:id/files/:fid', async (req, res) => {
  try {
    const f = await query(`SELECT * FROM dqs_bill_files WHERE id=$1 AND bill_id=$2`, [req.params.fid, req.params.id]);
    if (f.rows.length && f.rows[0].local_url) {
      const fullPath = path.join(__dirname, '../../', f.rows[0].local_url);
      if (fs.existsSync(fullPath)) fs.unlinkSync(fullPath);
    }
    await query(`DELETE FROM dqs_bill_files WHERE id=$1`, [req.params.fid]);
...
router.delete('/:id', async (req, res) => {
  try {
    await query(`UPDATE dqs_bills SET is_deleted=TRUE, updated_at=NOW() WHERE id=$1 AND company_id=$2`, [req.params.id, req.user.company_id]);
