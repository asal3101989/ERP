...
function buildProjectScope(user, filters = {}) {
  ...
  try {
    const revenue = await query(
      `SELECT
       ...
       FROM ra_bills
       WHERE status IN ('certified', 'authorized', 'verified', 'paid')`
    );

    const materialCost = await query(
      `SELECT COALESCE(SUM(net_amount), 0) as total FROM invoices WHERE status IN ('authorized', 'verified', 'paid')`
    );
    ...
    const assetCost = await query(
      `SELECT
         (SELECT COALESCE(SUM(total_cost), 0) FROM asset_fuel_logs) +
         (SELECT COALESCE(SUM(ul.units_worked * a.hourly_rate), 0) FROM asset_usage_logs ul JOIN assets a ON ul.asset_id = a.id) as total`
    );
    ...
    const safety = await query(
      `SELECT
       ...
       FROM incidents WHERE incident_date > NOW() - INTERVAL '30 days'`
    );
    ...
    const portfolio = await query("SELECT COUNT(*) as project_count FROM projects WHERE status = 'active'");
...
router.get('/executive', async (req, res) => {
  try {
    ...
    const scope = buildProjectScope(req.user, filters);
    ...
    const totalCollections = payments.reduce((sum, payment) => sum + toNumber(payment.net_amount || payment.amount), 0);
...
router.get('/project-360/:project_id', async (req, res) => {
  try {
    ...
    const projectInfo = await query("SELECT * FROM projects WHERE id = $1", [projectId]);
