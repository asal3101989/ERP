...
export default function DocumentsPage() {
  ...
  const [uploadForm, setUploadForm] = useState({ module: 'general', project_id: '', tags: '' });
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
        <div>
          <div className="flex items-center gap-2 text-xs text-slate-500 mb-1">
            <FolderOpen className="w-3.5 h-3.5" /> Document Repository
          </div>
          <h1 className="text-2xl font-bold text-slate-900">Documents</h1>
          <p className="text-sm text-slate-400 mt-0.5">All module documents — stored locally & synced to OneDrive</p>
        </div>
        <button
        ...
        <div className="flex items-center gap-1.5 flex-wrap">
          <button
            ...
            className={clsx('px-3 py-1.5 rounded-lg text-xs font-medium border transition-all',
              moduleFilter === 'all' ? 'bg-indigo-600 text-white border-indigo-600' : 'bg-white text-slate-500 border-slate-200 hover:border-indigo-300')}
          >All</button>
          {moduleCounts.map(m => (
            <button
              key={m.module}
              ...
              className={clsx('px-3 py-1.5 rounded-lg text-xs font-medium border transition-all',
                moduleFilter === m.module ? 'bg-indigo-600 text-white border-indigo-600' : 'bg-white text-slate-500 border-slate-200 hover:border-indigo-300')}
            >
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-slate-100 bg-slate-50">
                <th className="text-left px-4 py-3 text-xs font-semibold text-slate-500">File</th>
                <th className="text-left px-4 py-3 text-xs font-semibold text-slate-500">Module</th>
                <th className="text-left px-4 py-3 text-xs font-semibold text-slate-500">Project</th>
                ...
                <th className="text-left px-4 py-3 text-xs font-semibold text-slate-500">OneDrive</th>
                ...
                <tr key={doc.id} className="hover:bg-slate-50 transition-colors group">
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2.5">
                      ...
                      <span className="text-sm font-medium text-slate-800 truncate max-w-[200px]" title={doc.file_name}>
                        {doc.file_name}
                      </span>
                    </div>
                  </td>
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                      ...
                      <button
                        ...
                        className="w-7 h-7 flex items-center justify-center rounded-lg border border-slate-200 text-slate-400 hover:text-red-500 hover:border-red-200 transition-all"
                      >
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg">
            <div className="flex items-center justify-between p-5 border-b border-slate-100">
              <div>
                ...
                <p className="text-xs text-slate-400 mt-0.5">File will be stored locally and synced to OneDrive if configured</p>
              </div>
              <button onClick={() => setShowUpload(false)} className="w-8 h-8 flex items-center justify-center rounded-lg border border-slate-200 text-slate-400 hover:text-slate-600 transition-all">
                <X className="w-4 h-4" />
              </button>
            </div>
            <div className="p-5 space-y-4">
              ...
              >
                ...
                <p className="text-xs text-slate-400 mt-1">PDF, DOCX, XLSX, PNG, JPG, DWG — up to 25 MB</p>
                <input
                  ref={fileRef}
                  type="file"
                  multiple
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-xs font-medium text-slate-500 mb-1 block">Module *</label>
                  <select
                    className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm text-slate-800 outline-none focus:border-indigo-400"
