      ...
      <div className="border-b border-gray-200 bg-white px-6 py-4">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <button onClick={() => refetch()} className="flex items-center gap-1.5 rounded border border-gray-200 bg-white px-3 py-1.5 text-xs text-gray-600 hover:bg-gray-50">
              <RefreshCw className="h-3.5 w-3.5" /> Refresh
            </button>
            <button onClick={() => navigate('/projects/new')} className="flex items-center gap-1.5 rounded bg-blue-600 px-4 py-1.5 text-xs font-semibold text-white hover:bg-blue-700">
      ...
      <div className="p-6 space-y-5">
        ...
        <div className="flex flex-wrap items-center gap-3 rounded-lg border border-gray-200 bg-white px-4 py-3 shadow-sm">
          <div className="relative flex-1 min-w-[200px] max-w-xs">
            ...
            <input
              className="w-full rounded border border-gray-200 bg-gray-50 py-1.5 pl-9 pr-3 text-sm text-gray-800 outline-none focus:border-blue-400 placeholder:text-gray-400"
            ...
            {STATUS_TABS.map(s => (
              <button
                ...
                className={clsx(
                  ...
                  statusFilter === s ? 'border-blue-600 bg-blue-600 text-white' : 'border-gray-200 bg-white text-gray-500 hover:border-blue-300 hover:text-blue-600'
                )}
              >
          ...
          <div className="ml-auto flex items-center rounded border border-gray-200 bg-gray-50">
            <button onClick={() => setView('grid')} className={clsx('rounded-l p-1.5 transition', view === 'grid' ? 'bg-blue-600 text-white' : 'text-gray-400 hover:text-gray-700')}>
              <LayoutGrid className="h-4 w-4" />
            </button>
            <button onClick={() => setView('list')} className={clsx('rounded-r p-1.5 transition', view === 'list' ? 'bg-blue-600 text-white' : 'text-gray-400 hover:text-gray-700')}>
        ...
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center rounded-lg border-2 border-dashed border-gray-200 bg-white py-20 text-center">
            ...
            <h3 className="text-sm font-semibold text-gray-500">No Projects Found</h3>
            <p className="mt-1 text-xs text-gray-400">Try adjusting your search or filters</p>
            <button onClick={() => navigate('/projects/new')} className="mt-4 flex items-center gap-1.5 rounded bg-blue-600 px-4 py-2 text-xs font-semibold text-white hover:bg-blue-700">
                    ...
                    <div className="p-4">
                      ...
                      <div className="grid grid-cols-2 gap-2 mb-3">
                        <div className="rounded border border-gray-100 bg-gray-50 p-2.5">
                          <div className="text-[10px] text-gray-400 uppercase tracking-wide">Contract Value</div>
                          <div className="mt-0.5 text-sm font-bold text-gray-900">{inr(proj.contract_value)}</div>
                        </div>
                        <div className={clsx('rounded border p-2.5', overrun ? 'border-red-100 bg-red-50' : 'border-gray-100 bg-gray-50')}>
                          <div className="flex items-center justify-between">
                            <div className="text-[10px] text-gray-400 uppercase tracking-wide">Spent</div>
                ...
                {projects.map(proj => {
                  ...
                  return (
                    <tr key={proj.id} onClick={() => navigate(`/projects/${proj.id}`)} className="cursor-pointer hover:bg-blue-50/30 transition-colors">
                      ...
                      <td className="px-4 py-3 text-sm font-semibold text-gray-800">{inr(proj.contract_value)}</td>
                      <td className="px-4 py-3 w-32">
                        <div className="flex items-center gap-2">
                          <div className="flex-1 h-1.5 rounded-full bg-gray-100 overflow-hidden">
                            <div className={clsx('h-full rounded-full', sc.bar)} style={{ width: `${Math.min(progress, 100)}%` }} />
                          </div>
                          <span className="text-xs text-gray-500 w-8 text-right">{progress.toFixed(0)}%</span>
                        </div>
                      </td>
