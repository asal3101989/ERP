  ...
  return (
    <div className="p-6 md:p-8 max-w-7xl mx-auto min-h-screen bg-[#f4f6f9] space-y-6">
      ...
      <div className="flex items-start gap-4">
        <Link
          to="/projects"
          className="mt-1 w-9 h-9 flex items-center justify-center bg-white border border-slate-200 rounded-lg text-slate-500 hover:text-indigo-600 hover:border-indigo-300 shadow-sm transition-all shrink-0"
        >
          <ArrowLeft className="w-4 h-4" />
        </Link>
        <div className="flex-1 min-w-0">
          <h1 className="text-xl font-bold text-slate-900 leading-tight truncate">{project.name}</h1>
        ...
        <div className="flex gap-2 shrink-0 flex-wrap justify-end">
          <Link to={`/qs/boq?project_id=${id}`} className="px-4 py-2 bg-white border border-slate-200 text-slate-600 text-xs font-medium rounded-lg hover:border-indigo-300 hover:text-indigo-600 shadow-sm transition-all">
            Master BOQ
          </Link>
          <Link to={`/qs/ra-bills?project_id=${id}`} className="px-4 py-2 bg-white border border-slate-200 text-slate-600 text-xs font-medium rounded-lg hover:border-indigo-300 hover:text-indigo-600 shadow-sm transition-all">
            RA Bills
          </Link>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          ...
          <div key={m.label} className={clsx('bg-white border rounded-xl p-5 shadow-sm', m.border)}>
            <div className={clsx('text-2xl font-bold tracking-tight font-mono mb-1', m.color)}>{m.value}</div>
        ...
        <div className="lg:col-span-4 space-y-5">
          ...
          <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm">
            <div className="flex items-center justify-between mb-4">
              <span className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Construction Progress</span>
            ...
            <div className="grid grid-cols-2 gap-3 pt-3 border-t border-slate-100">
              <div className="text-center">
                <div className="text-[10px] text-slate-400 uppercase tracking-wider mb-1">Budgeted</div>
                <div className="text-sm font-bold text-slate-900 font-mono">{inr(project.contract_value)}</div>
              </div>
              <div className="text-center">
                <div className="text-[10px] text-slate-400 uppercase tracking-wider mb-1">Certified</div>
                <div className="text-sm font-bold text-emerald-600 font-mono">{inr(project.total_certified)}</div>
              </div>
            </div>
          </div>
              ...
              <div className="space-y-3">
                {project.start_date && (
                  <div className="flex justify-between items-center">
                    ...
                    <span className="text-xs font-semibold text-slate-800">{dayjs(project.start_date).format('DD MMM YYYY')}</span>
                  </div>
                )}
                {project.end_date && (
                  <div className="flex justify-between items-center">
                    <span className="text-xs text-slate-400">End Date</span>
                    <span className="text-xs font-semibold text-slate-800">{dayjs(project.end_date).format('DD MMM YYYY')}</span>
                  </div>
                )}
                {project.worker_count > 0 && (
                  <div className="flex justify-between items-center pt-2 border-t border-slate-100">
                    <span className="text-xs text-slate-400 flex items-center gap-1.5"><Users className="w-3 h-3" /> Workers on site</span>
                    <span className="text-xs font-semibold text-slate-800">{project.worker_count}</span>
                  </div>
                )}
              </div>
            </div>
        ...
        <div className="lg:col-span-8 flex flex-col gap-5">
          ...
          <div className="bg-white border border-slate-200 rounded-xl p-6 shadow-sm">
            <div className="flex items-center gap-3 mb-5 pb-4 border-b border-slate-100">
              ...
              <h3 className="text-sm font-semibold text-slate-800">Contract Financial Ledger</h3>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="space-y-3 bg-slate-50 p-4 rounded-lg border border-slate-100">
                ...
                <div className="pt-3 border-t border-slate-200">
                  <div className="flex justify-between items-center">
                    <span className="text-xs font-semibold text-slate-500">Balance to Execute</span>
                    <span className={clsx('text-base font-bold font-mono', balanceToExecute < 0 ? 'text-red-600' : 'text-emerald-600')}>
            ...
            <div className="bg-white border border-slate-200 rounded-xl p-6 shadow-sm">
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-3">
                  ...
                  <h3 className="text-sm font-semibold text-slate-800">Recent RA Bills</h3>
                </div>
              ...
              <div className="space-y-2">
                {bills.slice(0, 5).map(b => (
                  <div key={b.id} className="flex items-center justify-between py-2 px-3 rounded-lg hover:bg-slate-50 transition-colors">
                    <div>
                      <div className="text-xs font-semibold text-slate-800">{b.bill_number}</div>
                      <div className="text-[11px] text-slate-400">{b.bill_date ? dayjs(b.bill_date).format('DD MMM YYYY') : '—'}</div>
                    </div>
                    <div className="flex items-center gap-3">
                      <span className="text-xs font-semibold text-slate-700 font-mono">{inr(b.net_payable)}</span>
                      <span className={clsx(
          ...
          <div className="bg-white border border-slate-200 rounded-xl p-6 shadow-sm">
            <div className="flex items-center justify-between mb-4 pb-4 border-b border-slate-100">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-sm font-semibold text-slate-800">Project Repository</h3>
