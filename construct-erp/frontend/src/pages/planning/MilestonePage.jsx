...
export default function MilestonePage() {
  ...
  const { data: milestones = [] } = useQuery({
    ...
    queryFn: () => planningAPI.listMilestones({ project_id: projectId }).then(r => r.data.data),
    enabled: !!projectId,
  });
  ...
  return (
    <div className="p-6 md:p-8 max-w-5xl mx-auto min-h-screen bg-[#f4f6f9]">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
        <div>
          <div className="flex items-center gap-2 text-xs text-slate-500 mb-1"><Flag className="w-3.5 h-3.5" /> Planning</div>
          <h1 className="text-2xl font-bold text-slate-900">Milestones</h1>
          <p className="text-sm text-slate-400 mt-0.5">Contractual & operational milestone gates</p>
        </div>
        ...
        <>
          ...
          <div className="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden">
            <div className="px-5 py-3.5 border-b border-slate-100 flex items-center gap-2">
              ...
              <h2 className="text-sm font-bold text-slate-800">Milestone Timeline</h2>
            </div>
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        <span className="text-sm font-semibold text-slate-800">{m.milestone_name}</span>
                        <span className="text-xs text-slate-400 font-mono">{m.milestone_code}</span>
    ...
    <div className="fixed inset-0 z-[60] bg-black/40 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="bg-white w-full max-w-md rounded-xl shadow-2xl border border-slate-200">
        <div className="px-5 py-4 border-b border-slate-100 flex items-center justify-between bg-slate-50">
          <h2 className="text-base font-bold text-slate-900">{m.milestone_name}</h2>
        ...
        <div className="p-5 space-y-4">
          <div className="grid grid-cols-2 gap-3 text-xs">
            <div className="bg-slate-50 rounded-lg p-3">
              <div className="text-slate-400 mb-1">Target Date</div>
              <div className="font-semibold text-slate-800">{dayjs(m.target_date).format('DD MMM YYYY')}</div>
            </div>
            <div className="bg-slate-50 rounded-lg p-3">
              <div className="text-slate-400 mb-1">Type</div>
              <div className="font-semibold text-slate-800 capitalize">{m.milestone_type || '—'}</div>
            </div>
          </div>
          {m.is_achieved ? (
            <div className="bg-emerald-50 border border-emerald-200 rounded-lg p-4 text-center">
              <CheckCircle2 className="w-6 h-6 text-emerald-500 mx-auto mb-2" />
              <p className="text-sm font-semibold text-emerald-700">Achieved on {dayjs(m.actual_date).format('DD MMM YYYY')}</p>
              {m.deviation_days <= 0
            ...
            <div className="space-y-2">
              <label className="text-xs font-semibold text-slate-600">Actual Achievement Date</label>
            ...
            <p className="text-xs text-slate-400 text-center py-2">Only Project Managers can mark milestones as achieved.</p>
          )}
        </div>
      ...
      <div className="bg-white w-full max-w-md rounded-xl shadow-2xl border border-slate-200">
        ...
        <div className="p-5 space-y-3">
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1"><label className="text-xs font-semibold text-slate-600">Code *</label><input className="inp" placeholder="M1" value={form.milestone_code} onChange={e => F('milestone_code', e.target.value)} /></div>
            <div className="space-y-1"><label className="text-xs font-semibold text-slate-600">Type</label>
              <select className="inp" value={form.milestone_type} onChange={e => F('milestone_type', e.target.value)}>
                {Object.entries(TYPE_LABELS).map(([k, v]) => <option key={k} value={k}>{v}</option>)}
              </select>
            </div>
            <div className="space-y-1 col-span-2"><label className="text-xs font-semibold text-slate-600">Milestone Name *</label><input className="inp" placeholder="Foundation Completion" value={form.milestone_name} onChange={e => F('milestone_name', e.target.value)} /></div>
            <div className="space-y-1 col-span-2"><label className="text-xs font-semibold text-slate-600">Target Date *</label><input type="date" className="inp" value={form.target_date} onChange={e => F('target_date', e.target.value)} /></div>
            <div className="space-y-1 col-span-2">
              <label className="text-xs font-semibold text-slate-600">Linked to Payment Release</label>
            ...
            <div className="space-y-1 col-span-2"><label className="text-xs font-semibold text-slate-600">Remarks</label><textarea className="inp" rows={2} value={form.remarks} onChange={e => F('remarks', e.target.value)} /></div>
          </div>
        </div>
        <div className="px-5 py-3 border-t border-slate-100 bg-slate-50 flex justify-end gap-2">
