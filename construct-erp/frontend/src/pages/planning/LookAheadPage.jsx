...
export default function LookAheadPage() {
  ...
  const { data: plans = [] } = useQuery({
    ...
    queryFn: () => planningAPI.getLookAhead({ project_id: projectId, week_start: weekStart }).then(r => r.data.data),
    enabled: !!projectId,
  });
  ...
  return (
    <div className="p-6 md:p-8 max-w-5xl mx-auto min-h-screen bg-[#f4f6f9]">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
        <div>
          <div className="flex items-center gap-2 text-xs text-slate-500 mb-1"><CalendarDays className="w-3.5 h-3.5" /> Planning</div>
          <h1 className="text-2xl font-bold text-slate-900">2-Week Look-Ahead Plan</h1>
          <p className="text-sm text-slate-400 mt-0.5">Rolling forward planning window for activities, resources and risk</p>
        </div>
      ...
      {!projectId ? (
        <div className="bg-white border border-slate-200 rounded-xl p-16 text-center shadow-sm">
          ...
          <p className="text-slate-500 font-medium">Select a project to manage its look-ahead plan</p>
        </div>
      ) : (
        <>
          {/* Week Navigator */}
          <div className="bg-white border border-slate-200 rounded-xl shadow-sm p-4 mb-5 flex items-center justify-between">
            ...
            <div className="text-center">
              ...
              <div className="text-xs text-slate-400 mt-0.5">2-Week Planning Window</div>
            </div>
              ...
              <div className="flex items-center gap-2">
                ...
                {!isEditing ? (
                  ...
                  <div className="flex gap-2">
                    <button onClick={cancelEdit} className="px-3 py-1.5 border border-slate-200 text-slate-600 text-xs font-medium rounded-lg hover:bg-slate-50">Cancel</button>
                    ...
                    {plan.planned_activities.map((a, i) => (
                      <div key={i} className="flex items-start gap-3 bg-slate-50 rounded-lg p-3">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="text-xs font-semibold text-slate-800">{a.description}</div>
                          {a.planned_qty && <div className="text-xs text-slate-400 mt-0.5">Qty: {a.planned_qty} | Resource: {a.resource || '—'}</div>}
                        </div>
                      </div>
                    ))}
                  </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <Section title="Manpower Required" icon={Users}>
                  {isEditing ? (
                    ...
                    <div className="text-2xl font-bold text-slate-900">{plan.planned_manpower} <span className="text-sm font-normal text-slate-400">workers</span></div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <Section title="Potential Risks" icon={AlertTriangle}>
                  {isEditing ? (
                    ...
                    <p className="text-xs text-slate-700 leading-relaxed">{plan.potential_risks}</p>
                ...
                <Section title="Mitigation Measures">
                  {isEditing ? (
                    ...
                    <p className="text-xs text-slate-700 leading-relaxed">{plan.mitigation_measures}</p>
...
function Empty({ text }) {
  return <p className="text-xs text-slate-400 italic py-1">{text}</p>;
}
    ...
    <div className="space-y-2">
      {items.map((a, i) => (
        <div key={i} className="flex items-start gap-2">
          ...
          <button onClick={() => remove(i)} className="p-2 text-slate-400 hover:text-red-500 mt-0.5"><X className="w-3.5 h-3.5" /></button>
        </div>
      ))}
    ...
    <div className="space-y-1.5">
      {items.map((item, i) => (
        <div key={i} className="flex items-center gap-1.5">
          ...
          <button onClick={() => remove(i)} className="p-1.5 text-slate-400 hover:text-red-500"><X className="w-3 h-3" /></button>
        </div>
      ))}
