...
const STATUS_CFG = {
  planned:     { label: 'Planned',     color: 'bg-slate-50 text-slate-600 border-slate-200',   dot: 'bg-slate-400',   icon: Clock },
  ...
  cancelled:   { label: 'Cancelled',   color: 'bg-slate-100 text-slate-400 border-slate-200',  dot: 'bg-slate-300',   icon: X },
};
    ...
    <div className="p-6 md:p-8 max-w-7xl mx-auto min-h-screen bg-[#f4f6f9]">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
        <div>
          ...
          <p className="text-sm text-slate-400 mt-0.5">Activity-based project schedule management</p>
        </div>
        <div className="flex items-center gap-2">
          <select
            value={projectId}
            onChange={e => setProjectId(e.target.value)}
            className="bg-white border border-slate-200 rounded-lg px-3 py-2 text-sm text-slate-700 outline-none focus:border-indigo-400 shadow-sm"
          >
      ...
      {!projectId ? (
        <div className="bg-white border border-slate-200 rounded-xl shadow-sm p-16 text-center">
          ...
          <p className="text-slate-500 font-medium">Select a project to view its activities</p>
        </div>
      ) : (
        <>
          {/* Toolbar */}
          <div className="bg-white border border-slate-200 rounded-xl p-3 mb-5 flex flex-wrap items-center gap-3 shadow-sm">
            ...
            <span className="text-xs text-slate-400 ml-auto">{filtered.length} of {activities.length}</span>
          </div>
                    ...
                    return (
                      <tr key={a.id} onClick={() => setSelected(a)} className="cursor-pointer hover:bg-slate-50 transition-colors group">
                        <td className="px-4 py-3 whitespace-nowrap">
                          <span className="text-xs font-bold font-mono text-indigo-600 group-hover:underline">{a.activity_code}</span>
                          {a.is_critical_path && <span className="ml-1.5 text-[9px] bg-red-100 text-red-600 px-1.5 py-0.5 rounded font-bold">CP</span>}
                        </td>
                        <td className="px-4 py-3">
                          <div className="text-xs font-semibold text-slate-800 max-w-52 truncate">{a.activity_name}</div>
                        ...
                        <td className="px-4 py-3 whitespace-nowrap">
                          <div className="text-xs text-slate-700 font-medium">{dayjs(a.baseline_start_date).format('DD MMM')}</div>
                        ...
                        <td className="px-4 py-3 whitespace-nowrap w-32">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-xs text-slate-500 w-8 text-right">{a.progress_pct || 0}%</span>
                          </div>
                        </td>
