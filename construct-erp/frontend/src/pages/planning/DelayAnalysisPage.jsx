...
export default function DelayAnalysisPage() {
  ...
  const { data: delays = [], isLoading } = useQuery({
    ...
    queryFn: () => planningAPI.listDelays({ project_id: projectId }).then(r => r.data.data),
    enabled: !!projectId,
  });
    ...
    <div className="p-6 md:p-8 max-w-6xl mx-auto min-h-screen bg-[#f4f6f9]">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
        <div>
          ...
          <p className="text-sm text-slate-400 mt-0.5">Root-cause tracking and mitigation for schedule delays</p>
        </div>
      ...
      {!projectId ? (
        <div className="bg-white border border-slate-200 rounded-xl p-16 text-center shadow-sm">
          ...
          <p className="text-slate-500 font-medium">Select a project to view delay analysis</p>
        </div>
      ) : (
        <>
          {/* KPI Cards */}
                    ...
                    <div key={d.id}>
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="text-sm font-semibold text-slate-800 truncate">{actName}</div>
                        ...
                        <div className="text-right flex-shrink-0">
                          <div className="text-lg font-bold text-red-600">{d.delay_days}d</div>
                          <div className="text-[10px] text-slate-400">delay</div>
                        </div>

                        {/* Category */}
                        <span className="hidden md:block text-xs bg-slate-100 text-slate-600 px-2 py-1 rounded-md whitespace-nowrap">
...
function DetailBlock({ label, value }) {
  return (
    <div>
      <div className="text-[10px] font-bold text-slate-500 uppercase tracking-wide mb-1">{label}</div>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100">
          <h2 className="text-sm font-bold text-slate-800">{isEdit ? 'Edit Delay Record' : 'Log Delay'}</h2>
