...
function SCurveTooltip({ active, payload, label }) {
  ...
  return (
    <div className="bg-white border border-slate-200 rounded-lg p-3 shadow-lg text-xs">
      <div className="font-semibold text-slate-700 mb-1.5">{dayjs(label).format('DD MMM YYYY')}</div>
      {payload.map(p => (
...
function VarianceChip({ planned, actual }) {
  ...
  if (diff > 0)  return <span className="flex items-center gap-0.5 text-xs font-semibold text-emerald-600"><ArrowUp className="w-3 h-3" />+{diff}%</span>;
  if (diff < 0)  return <span className="flex items-center gap-0.5 text-xs font-semibold text-red-500"><ArrowDown className="w-3 h-3" />{diff}%</span>;
...
export default function ProgressDashboard() {
  ...
  const { data: scurve = [] } = useQuery({
    ...
    queryFn: () => planningAPI.getScurve({ project_id: projectId }).then(r => r.data.data),
    enabled: !!projectId,
  });

  const { data: activities = [] } = useQuery({
    queryKey: ['planning-activities', projectId],
    queryFn: () => planningAPI.listActivities({ project_id: projectId }).then(r => r.data.data),
    enabled: !!projectId,
  });

  const { data: progress = [] } = useQuery({
    queryKey: ['planning-progress', projectId],
    queryFn: () => planningAPI.listProgress({ project_id: projectId }).then(r => r.data.data),
    enabled: !!projectId,
  });
    ...
    <div className="p-6 md:p-8 max-w-6xl mx-auto min-h-screen bg-[#f4f6f9]">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
        <div>
          ...
          <h1 className="text-2xl font-bold text-slate-900">Progress & S-Curve</h1>
          <p className="text-sm text-slate-400 mt-0.5">Baseline vs actual vs forecast cumulative progress</p>
        </div>
      ...
      {!projectId ? (
        <div className="bg-white border border-slate-200 rounded-xl p-16 text-center shadow-sm">
          ...
          <p className="text-slate-500 font-medium">Select a project to view progress & S-curve</p>
        </div>
      ) : (
        <>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-5">
              {[
                ...
                <div key={label} className="bg-white border border-slate-200 rounded-xl p-4 shadow-sm">
                  ...
                  <div className="text-xs text-slate-400 mt-0.5">as of {dayjs(latest.reporting_date).format('DD MMM YYYY')}</div>
                </div>
              ))}
            </div>
          )}
          ...
          <div className="bg-white border border-slate-200 rounded-xl shadow-sm p-5 mb-5">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-sm font-bold text-slate-800">S-Curve Chart</h2>
          ...
          <div className="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden">
            <div className="px-5 py-3.5 border-b border-slate-100 flex items-center justify-between">
              <h2 className="text-sm font-bold text-slate-800">Activity Progress</h2>
                      ...
                      return (
                        <tr key={a.id} className="hover:bg-slate-50 transition-colors">
                          <td className="px-4 py-3">
                            <div className="font-semibold text-slate-800 text-xs">{a.activity_name}</div>
                          ...
                          <td className="px-4 py-3 min-w-[120px]">
                            ...
                            <div className="text-[10px] text-slate-400 mt-0.5">{actual ?? 0}%</div>
                          </td>
...
function StatusBadge({ status }) {
  ...
  return (
    <span className={clsx('text-[10px] px-2 py-0.5 rounded-md font-semibold capitalize', STATUS_CFG[status] || 'bg-slate-100 text-slate-500')}>
      {label}
    </span>
  );
}
