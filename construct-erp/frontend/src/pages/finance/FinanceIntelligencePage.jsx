...
const lakh = v => { const n = Number(v || 0); return n >= 1e7 ? `${(n/1e7).toFixed(2)}Cr` : n >= 1e5 ? `${(n/1e5).toFixed(2)}L` : `₹${inr(n)}`; };
...
function VendorLedgerTab({ projectId }) {
  ...
  if (isLoading) return <div className="py-20 text-center text-slate-400 text-sm animate-pulse">Loading vendor ledger…</div>;
  ...
  return (
    <div className="space-y-5">
      ...
      {rows.length === 0 ? (
        <div className="py-16 text-center text-slate-400 text-sm">No vendor data found.</div>
      ) : (
        <div className="bg-white rounded-xl border border-slate-100 overflow-hidden">
          <table className="w-full text-sm">
                ...
                return (
                  <tr key={i} className="hover:bg-slate-50/50 transition-colors">
                    <td className="px-4 py-3">
                      <p className="font-semibold text-slate-800 text-sm">{r.vendor_name || '—'}</p>
                    </td>
                    <td className="px-4 py-3 text-slate-500 text-xs">{r.bill_count}</td>
                    <td className="px-4 py-3 font-mono text-slate-600 text-xs">₹{inr(r.total_invoiced)}</td>
                    ...
                    <td className="px-4 py-3">
                      <span className={`font-bold text-sm font-mono ${outstanding > 0 ? 'text-red-600' : 'text-emerald-600'}`}>
                        ₹{inr(outstanding)}
                      </span>
                    </td>
                  </tr>
            ...
            <tfoot className="bg-slate-50 border-t-2 border-slate-200">
              <tr>
                <td colSpan={2} className="px-4 py-3 text-xs font-bold text-slate-600 uppercase tracking-wide">TOTAL ({rows.length} vendors)</td>
                <td className="px-4 py-3 font-mono font-bold text-slate-700 text-xs">₹{inr(rows.reduce((s,r)=>s+parseFloat(r.total_invoiced||0),0))}</td>
                <td className="px-4 py-3 font-mono font-bold text-indigo-700 text-xs">₹{inr(totalCertified)}</td>
                <td className="px-4 py-3 font-mono font-bold text-emerald-700 text-xs">₹{inr(rows.reduce((s,r)=>s+parseFloat(r.total_paid||0),0))}</td>
                <td className="px-4 py-3 font-mono font-bold text-amber-700 text-xs">₹{inr(totalTDS)}</td>
                <td className="px-4 py-3 font-mono font-bold text-amber-700 text-xs">₹{inr(totalRetention)}</td>
                <td className="px-4 py-3 font-mono font-black text-red-600 text-sm">₹{inr(totalOutstanding)}</td>
              </tr>
            </tfoot>
          </table>
        </div>
  ...
  return (
    <div className="space-y-5">
      ...
      {!projectId ? null : isLoading ? (
        <div className="py-20 text-center text-slate-400 animate-pulse text-sm">Loading budget data…</div>
      ) : rows.length === 0 ? (
        <div className="py-16 text-center text-slate-400 text-sm">No budget lines for this project.</div>
      ) : (
        <>
            ...
            <table className="w-full text-sm">
                  ...
                  return (
                    <tr key={i} className={`hover:bg-slate-50/50 ${r.unbudgeted ? 'bg-orange-50/30' : ''}`}>
                      <td className="px-4 py-3">
                        ...
                        {r.unbudgeted && <span className="text-[9px] text-orange-500 font-bold uppercase">No Budget Set</span>}
                      </td>
                      <td className="px-4 py-3 font-mono text-slate-600 text-xs">₹{inr(budget)}</td>
                      <td className="px-4 py-3 font-mono text-xs font-semibold">{actual > 0 ? `₹${inr(actual)}` : '—'}</td>
                      <td className={`px-4 py-3 font-mono text-xs font-semibold ${variance >= 0 ? 'text-emerald-600' : 'text-red-500'}`}>
                        {variance >= 0 ? '+' : '−'}₹{inr(Math.abs(variance))}
                      </td>
                      <td className={`px-4 py-3 text-xs font-bold ${over ? 'text-red-600' : 'text-slate-600'}`}>
              ...
              <tfoot className="bg-slate-50 border-t-2 border-slate-200">
                <tr>
                  ...
                  <td className="px-4 py-3 font-mono font-bold text-slate-700 text-xs">₹{inr(totalBudget)}</td>
                  <td className="px-4 py-3 font-mono font-bold text-xs">₹{inr(totalActual)}</td>
                  <td className={`px-4 py-3 font-mono font-bold text-xs ${overrun <= 0 ? 'text-emerald-600' : 'text-red-600'}`}>
                    {overrun <= 0 ? '+' : '−'}₹{inr(Math.abs(overrun))}
                  </td>
                  <td className="px-4 py-3 font-bold text-xs text-slate-600">
...
function ProjectPLTab() {
  ...
  if (isLoading) return <div className="py-20 text-center text-slate-400 animate-pulse text-sm">Loading P&L data…</div>;
  ...
  return (
    <div className="space-y-5">
      ...
      {rows.length === 0 ? (
        <div className="py-16 text-center text-slate-400 text-sm">No project data found.</div>
      ) : (
        <div className="bg-white rounded-xl border border-slate-100 overflow-hidden">
          <table className="w-full text-sm">
            <thead className="bg-slate-50 border-b border-slate-100">
                ...
                return (
                  <tr key={i} className="hover:bg-slate-50/50">
                    <td className="px-3 py-3">
                      <p className="font-semibold text-slate-800 text-sm max-w-[180px] truncate">{r.project_name}</p>
                      <span className={`text-[9px] font-bold px-1.5 py-0.5 rounded-full ${r.status === 'active' ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-500'}`}>{r.status}</span>
                    </td>
                    <td className="px-3 py-3 text-xs text-slate-500 capitalize">{r.project_type}</td>
                    <td className="px-3 py-3 font-mono text-xs text-slate-600">{lakh(r.contract_value)}</td>
                    <td className="px-3 py-3 font-mono text-xs font-semibold text-indigo-700">{lakh(r.net_billed)}</td>
            ...
            <tfoot className="bg-slate-50 border-t-2 border-slate-200 font-bold">
              <tr>
                <td colSpan={2} className="px-3 py-3 text-xs text-slate-600 uppercase tracking-wide">TOTAL ({rows.length} projects)</td>
  ...
  const PCStatus = ({ row }) => {
    ...
    return (
      <div className="flex items-center gap-1">
        <span className={`w-4 h-4 rounded-full text-[8px] flex items-center justify-center font-bold ${qs ? 'bg-emerald-500 text-white' : 'bg-slate-200 text-slate-400'}`}>Q</span>
        <span className={`w-4 h-4 rounded-full text-[8px] flex items-center justify-center font-bold ${pm ? 'bg-emerald-500 text-white' : 'bg-slate-200 text-slate-400'}`}>P</span>
        <span className={`w-4 h-4 rounded-full text-[8px] flex items-center justify-center font-bold ${accts ? 'bg-emerald-500 text-white' : 'bg-slate-200 text-slate-400'}`}>A</span>
      </div>
    );
  };

  if (isLoading) return <div className="py-20 text-center text-slate-400 animate-pulse text-sm">Loading AP aging…</div>;
    ...
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-5 gap-3">
        {buckets.map(b => (
          <div key={b} className={`rounded-xl p-3 border text-center ${
            ...
            <p className="text-lg font-black text-slate-800">{summary[b].count}</p>
            <p className="text-[10px] font-semibold text-slate-600 mt-0.5">{lakh(summary[b].balance)}</p>
          </div>
        ))}
      </div>
      ...
      {rows.length === 0 ? (
        <div className="py-16 text-center">
          ...
          <p className="text-slate-500 text-sm">All certified bills are paid. Nothing pending.</p>
        </div>
      ) : (
        <div className="bg-white rounded-xl border border-slate-100 overflow-hidden">
          <table className="w-full text-sm">
              ...
              {rows.map((r, i) => {
                ...
                return (
                  <tr key={i} className={`hover:bg-slate-50/50 ${bucket === '90+' ? 'bg-red-50/20' : ''}`}>
                    ...
                    <td className="px-3 py-3 text-slate-500 text-xs">{r.inv_number || '—'}</td>
                    <td className="px-3 py-3 text-slate-500 text-xs">{fmt(r.qs_certified_date)}</td>
                    <td className="px-3 py-3">
                      <span className={`text-[9px] font-bold px-2 py-0.5 rounded-full ${BUCKET_COLORS[bucket] || 'bg-slate-100 text-slate-500'}`}>
                        {r.days_outstanding != null ? `${r.days_outstanding}d` : '—'}
                      </span>
                    </td>
                    <td className="px-3 py-3 font-mono text-xs font-semibold text-slate-700">₹{inr(r.certified_net)}</td>
                    <td className="px-3 py-3 font-mono text-xs text-emerald-700">₹{inr(r.paid_amount)}</td>
                    <td className="px-3 py-3 font-mono text-xs font-bold text-red-600">₹{inr(balance)}</td>
            ...
            <tfoot className="bg-slate-50 border-t-2 border-slate-200">
              <tr>
                <td colSpan={6} className="px-3 py-3 text-xs font-bold text-slate-600">TOTAL ({rows.length} bills)</td>
                <td className="px-3 py-3 font-mono font-bold text-xs text-slate-700">₹{inr(rows.reduce((s,r)=>s+parseFloat(r.certified_net||0),0))}</td>
                <td className="px-3 py-3 font-mono font-bold text-xs text-emerald-700">₹{inr(rows.reduce((s,r)=>s+parseFloat(r.paid_amount||0),0))}</td>
                <td className="px-3 py-3 font-mono font-black text-xs text-red-600">₹{inr(totalBalance)}</td>
                <td colSpan={2} />
              </tr>
            </tfoot>
          </table>
    ...
    <div className="p-6 space-y-6 bg-[#f4f6f9] min-h-full">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <h1 className="text-xl font-bold text-slate-800">Finance Intelligence</h1>
          <p className="text-sm text-slate-500 mt-0.5">Vendor ledger · Budget vs actual · Project P&L · AP aging</p>
        </div>
      </div>
        ...
        {TABS.map(tab => {
          ...
          return (
            <button
              ...
              className={`flex items-center gap-2 px-4 py-2.5 rounded-xl text-xs font-semibold transition-all border-2 ${
                isActive ? RING[tab.color] + ' border-2 shadow-sm' : 'border-transparent text-slate-500 bg-white hover:bg-slate-50'
              }`}
            >
