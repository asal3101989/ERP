    ...
    <div className="p-4 md:p-6 space-y-6 max-w-[1500px] mx-auto bg-slate-50 min-h-screen text-[0.94rem]">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-[1.25rem] md:text-[1.5rem] font-black text-slate-900 uppercase tracking-tight italic">Control Dashboard</h1>
            <p className="text-[9px] text-slate-500 font-bold uppercase tracking-widest mt-1">Budget control, approvals and variance monitoring</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
        <section className="bg-white border border-slate-200 rounded-[2rem] overflow-hidden shadow-sm">
          <div className="p-4 border-b border-slate-100 flex items-center justify-between">
            <div>
              <h2 className="font-black text-[0.88rem] text-slate-900 uppercase tracking-tight">Budget vs Actual</h2>
              <p className="text-[11px] text-slate-500 mt-1">Current selected project budget lines</p>
            </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-slate-50 border-b border-slate-100">
                <tr>
                  {['Cost Head', 'Budget', 'Actual', 'Variance'].map(h => <th key={h} className="px-3 py-2.5 text-[9px] font-black text-slate-400 uppercase tracking-widest text-left">{h}</th>)}
                </tr>
              </thead>
                  ...
                  return (
                    <tr key={row.id || row.cost_head} className="hover:bg-slate-50/50">
                      <td className="px-4 py-3">
                        <div className="font-bold text-slate-900">{row.cost_head}</div>
                        <div className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-1">{row.remarks || 'Budget line'}</div>
                      </td>
                      <td className="px-4 py-3 font-mono text-slate-700">{inr(row.budgeted_amount)}</td>
                      <td className="px-4 py-3 font-mono text-slate-900 font-black">{inr(row.actual_amount)}</td>
                      <td className={`px-4 py-3 font-mono font-black ${variance >= 0 ? 'text-emerald-600' : 'text-rose-600'}`}>{variance >= 0 ? '+' : '-'}{inr(Math.abs(variance))}</td>
                    </tr>
                  );
                })}
                {!filteredBudgetRows.length && <tr><td colSpan={4} className="py-12 text-center text-slate-400 text-xs font-bold uppercase tracking-widest">Select a project to view budget control</td></tr>}
              </tbody>
            </table>
          </div>
        </section>
        ...
        <section className="bg-white border border-slate-200 rounded-[2rem] overflow-hidden shadow-sm">
          <div className="p-5 border-b border-slate-100 flex items-center justify-between">
            <div>
              <h2 className="font-black text-slate-900 uppercase tracking-tight">Control Summary</h2>
              <p className="text-xs text-slate-500 mt-1">Open finance items that need attention</p>
            </div>
          ...
          <div className="p-5 grid grid-cols-1 gap-4">
            {projectRows.slice(0, 8).map(p => (
              <div key={p.id} className="rounded-2xl border border-slate-200 p-4">
                <div className="font-black text-slate-900">{p.project_name || p.name}</div>
                <div className="mt-2 grid grid-cols-3 gap-3 text-[11px] font-bold uppercase tracking-widest text-slate-500">
                  <div>Billed<br /><span className="text-slate-900 text-sm font-black normal-case">{inr(p.net_billed)}</span></div>
                  <div>Margin<br /><span className="text-slate-900 text-sm font-black normal-case">{p.margin_pct || 0}%</span></div>
                  <div>Contract<br /><span className="text-slate-900 text-sm font-black normal-case">{inr(p.contract_value)}</span></div>
                </div>
              </div>
            ))}
            {!projectRows.length && <div className="py-10 text-center text-slate-400 text-xs font-bold uppercase tracking-widest">No project summary available</div>}
          </div>
        </section>
      </div>
