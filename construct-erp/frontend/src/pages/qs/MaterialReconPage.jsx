...
export default function MaterialReconPage() {
  ...
  const { data: auditRes, isLoading, refetch, isRefetching } = useQuery({ 
    ...
    queryFn: () => materialReconAPI.audit(activeProject).then(r => r.data.data),
    ...
    <div className="space-y-6 animate-in fade-in duration-500 text-left">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-400 mt-1 uppercase text-[10px] font-black tracking-[0.2em]">Theoretical vs Actual Consumption Intelligence</p>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <button 
            ...
            className="p-2.5 bg-slate-800 border border-slate-700 text-slate-400 hover:text-white rounded-xl transition-all disabled:opacity-50"
          >
      ...
      {!activeProject ? (
        <div className="py-32 bg-slate-900/40 border border-dashed border-slate-800 rounded-[2.5rem] flex flex-col items-center justify-center">
            <PackageSearch className="w-16 h-16 text-slate-700 mb-4" />
            <h3 className="text-xl font-bold text-slate-500 italic">Project Stream Selection Required</h3>
            <p className="text-slate-600 font-medium mt-2">Choose a project above to run the automated engineering audit.</p>
        </div>
      ) : isLoading ? (
        ...
        <>
          ...
          <div className="bg-slate-900/60 border border-slate-800 rounded-[2.5rem] overflow-hidden shadow-2xl">
            <div className="overflow-x-auto">
              <table className="w-full text-left text-sm">
                <thead className="bg-slate-800/40 text-[10px] font-black uppercase tracking-[0.25em] text-slate-500 border-b border-slate-800">
                  <tr>
                  ...
                  {records.map((r, i) => (
                    <tr key={i} className="hover:bg-slate-800/30 transition-colors group">
                      <td className="px-8 py-5">
                        <div className="font-bold text-slate-100">{r.material_name}</div>
                        <div className="text-[10px] text-slate-500 font-black uppercase tracking-widest mt-0.5">{r.unit} · Norm Factor Active</div>
                      </td>
                      ...
                      <td className={clsx("px-6 py-5 text-right font-black font-mono", r.status === 'ok' ? 'text-emerald-500' : 'text-rose-500')}>
                      ...
                      <td className="px-6 py-5 text-right">
                        ...
                        <div className="text-[9px] font-black text-slate-600 uppercase">Above {r.allowed_wastage_pct}% Allowable</div>
                      </td>
                  ...
                  {records.length === 0 && (
                    <tr>
                      <td colSpan="6" className="py-24 text-center">
                        ...
                        <h3 className="text-slate-500 font-bold uppercase tracking-widest text-xs">No issue notes or norms found for this project</h3>
                        <p className="text-slate-600 text-[10px] mt-1 font-medium italic">Please verify if Consumption Norms are mapped in the Library.</p>
                      </td>
                    </tr>
                  )}
                </tbody>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="p-8 bg-slate-900/60 border border-slate-800 rounded-3xl space-y-4">
              <div className="flex items-center gap-3 text-blue-500 uppercase text-[10px] font-black tracking-widest">
              ...
              <button className="flex items-center gap-2 text-blue-400 font-bold text-xs uppercase tracking-widest group">
                Review Recovery Protocol <ArrowRight size={14} className="group-hover:translate-x-1 transition-transform" />
              </button>
            </div>

            <div className="p-8 bg-slate-900/60 border border-slate-800 rounded-3xl space-y-4">
              <div className="flex items-center gap-3 text-amber-500 uppercase text-[10px] font-black tracking-widest">
              ...
              <button className="flex items-center gap-2 text-amber-500 font-bold text-xs uppercase tracking-widest group">
...
function SummaryCard({ label, value, icon, color }) {
  return (
    <div className="bg-slate-900 border border-slate-800 p-6 rounded-3xl hover:border-slate-700 transition-colors group">
      ...
      <div className={clsx("text-4xl font-black tracking-tighter", color)}>{value}</div>
    </div>
  );
}
