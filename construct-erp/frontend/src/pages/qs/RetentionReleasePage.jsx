    ...
    <div className="p-6 md:p-8 max-w-7xl mx-auto min-h-screen bg-[#f4f6f9]">
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold text-slate-900">Retention Release</h1>
      ...
      <div className="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden mb-6">
        <div className="px-5 py-3.5 border-b border-slate-100 bg-slate-50 flex items-center justify-between">
          <h2 className="text-xs font-semibold text-slate-500 uppercase tracking-widest">Retention by Project</h2>
          <span className="text-xs text-slate-400">{summary.length} project{summary.length !== 1 ? 's' : ''}</span>
        </div>
        {sumLoading ? (
          <div className="py-8 text-center text-slate-400 text-sm">Loading…</div>
        ) : summary.length === 0 ? (
          <div className="py-8 text-center text-slate-400 text-sm">No certified RA bills with retention found</div>
        ) : (
            ...
            {summary.map(proj => {
              ...
              return (
                <div key={proj.project_id}>
                  ...
                  >
                    {isOpen
                      ? <ChevronDown className="w-4 h-4 text-slate-400 mr-3 shrink-0" />
                    ...
                    <div className="flex-1 min-w-0 grid grid-cols-2 md:grid-cols-5 gap-2 items-center">
                      <div className="col-span-2 md:col-span-1">
                        <p className="text-sm font-semibold text-slate-800 truncate">{proj.project_name}</p>
                      ...
                      <div className="hidden md:block text-right">
                        ...
                        <p className="text-sm font-bold text-slate-800">{fmtL(proj.total_held)}</p>
                      </div>
                      <div className="hidden md:block text-right">
                        <p className="text-xs text-slate-500">Released</p>
                        <p className="text-sm font-bold text-emerald-600">{fmtL(proj.total_released)}</p>
                      </div>
                      <div className="hidden md:block text-right">
                        <p className="text-xs text-slate-500">Pending</p>
                        <p className="text-sm font-bold text-amber-600">{fmtL(proj.pending_approval)}</p>
                      </div>
                      <div className="text-right">
                        <p className="text-xs text-slate-500">Outstanding</p>
                        <p className="text-sm font-bold text-slate-900">{fmtL(proj.net_outstanding)}</p>
                        <div className="w-full bg-slate-100 rounded-full h-1.5 mt-1">
                          <div className="h-1.5 bg-emerald-500 rounded-full" style={{ width: `${Math.min(parseFloat(relPct), 100)}%` }} />
                        </div>
                        <p className="text-xs text-slate-400 mt-0.5">{relPct}% released</p>
                      </div>
                    </div>
                  </button>
                  {isOpen && (
                    <div className="bg-slate-50 border-t border-slate-100 px-5 py-4">
                      <p className="text-xs font-semibold text-slate-500 uppercase tracking-wide mb-3">Retention breakdown from RA bills</p>
      ...
      <div className="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden">
        <div className="px-5 py-3.5 border-b border-slate-100 bg-slate-50 flex items-center justify-between gap-3 flex-wrap">
          <h2 className="text-xs font-semibold text-slate-500 uppercase tracking-widest">Release Requests</h2>
                ...
                {releases.map(r => (
                  <tr key={r.id} className="border-b border-slate-100 hover:bg-slate-50/60">
                    <td className="px-4 py-3">
                      <span className="font-mono text-xs font-semibold text-indigo-600">{r.release_number}</span>
                    </td>
                    <td className="px-4 py-3">
                      <p className="text-slate-800 font-medium text-xs">{r.project_name}</p>
                      <p className="text-slate-400 text-xs">{r.project_code}</p>
                    </td>
                    <td className="px-4 py-3 text-slate-600 text-xs">{r.contractor_name}</td>
                    <td className="px-4 py-3">
                      <span className="text-xs px-2 py-0.5 bg-slate-100 text-slate-600 rounded-full">
                        {MILESTONES.find(m => m.value === r.milestone)?.label || r.milestone}
                      </span>
                    </td>
                    <td className="px-4 py-3 font-bold text-slate-800">{fmtL(r.release_amount)}</td>
                    <td className="px-4 py-3 text-slate-500 text-xs">{dayjs(r.release_date).format('DD MMM YYYY')}</td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur-sm p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg border border-slate-100">
            <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100">
              <h2 className="text-base font-bold text-slate-800">New Retention Release Request</h2>
              <button onClick={() => setShowNewModal(false)} className="p-1.5 text-slate-400 hover:text-slate-600 hover:bg-slate-50 rounded-lg">
                <X className="w-4 h-4" />
              </button>
            </div>
            ...
            <div className="px-6 py-5 space-y-4">
              ...
              <div>
                <label className="block text-xs font-medium text-slate-600 mb-1.5">Project <span className="text-red-500">*</span></label>
                <select
              ...
              <div>
                <label className="block text-xs font-medium text-slate-600 mb-1.5">Contractor Name <span className="text-red-500">*</span></label>
                <input
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-medium text-slate-600 mb-1.5">Release Date <span className="text-red-500">*</span></label>
                  <input
                    type="date"
                ...
                <div>
                  <label className="block text-xs font-medium text-slate-600 mb-1.5">Release Milestone <span className="text-red-500">*</span></label>
                  <select
              ...
              <div>
                <label className="block text-xs font-medium text-slate-600 mb-1.5">Release Amount (₹) <span className="text-red-500">*</span></label>
                <input
                  type="number"
                  min="0"
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur-sm p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm border border-slate-100 p-6">
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                <h2 className="text-base font-bold text-slate-800">Approve Release</h2>
                <p className="text-xs text-slate-500">This will authorise the payment to be made</p>
              </div>
            </div>
            <div className="flex gap-3 mt-5">
              <button onClick={() => setShowApprove(null)} className="flex-1 py-2.5 border border-slate-200 text-slate-500 rounded-xl text-sm hover:bg-slate-50">Cancel</button>
              <button
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur-sm p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm border border-slate-100 p-6">
            ...
            <div className="flex gap-3 mt-4">
              <button onClick={() => { setShowReject(null); setRejectNote(''); }} className="flex-1 py-2.5 border border-slate-200 text-slate-500 rounded-xl text-sm hover:bg-slate-50">Cancel</button>
              <button
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur-sm p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm border border-slate-100 p-6">
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                <h2 className="text-base font-bold text-slate-800">Mark as Released</h2>
            ...
            <div className="space-y-3 mt-2">
              ...
              <div>
                <label className="block text-xs font-medium text-slate-600 mb-1.5">Payment Ref / UTR</label>
                <input
            ...
            <div className="flex gap-3 mt-5">
              <button onClick={() => setShowPayOut(null)} className="flex-1 py-2.5 border border-slate-200 text-slate-500 rounded-xl text-sm hover:bg-slate-50">Cancel</button>
              <button
      ...
      <tbody className="divide-y divide-slate-100">
        {bills.map(b => (
          <tr key={b.id}>
            ...
            <td className="py-1.5 font-semibold text-slate-800">{fmtL(b.retention_amount)}</td>
            <td className="py-1.5">
        ...
        <tr className="border-t border-slate-200">
          <td colSpan={4} className="py-2 font-semibold text-slate-600">Total Retention Held</td>
