    ...
    <div className="px-7 py-4 flex items-start justify-between gap-4">
      <div className="flex items-start gap-4 min-w-0">
        ...
        <div className="min-w-0">
          <div className="text-[18px] font-bold tracking-tight text-slate-900 leading-none">BCIM Engineering Private Limited</div>
          <div className="mt-1 text-[10px] uppercase tracking-[0.24em] text-slate-500 font-semibold">
      ...
      <div className="text-right flex-shrink-0">
        <div className="inline-flex items-center px-3 py-1 rounded-full border border-slate-300 text-[9px] uppercase tracking-[0.24em] text-slate-500 font-bold bg-slate-50">
          Client Copy
        </div>
        <div className="mt-2 text-[10px] font-semibold text-slate-500 uppercase tracking-[0.2em]">Bill No.</div>
        <div className="text-[18px] font-mono font-bold text-slate-900 leading-none">{billNumber || '---'}</div>
      </div>
    </div>
  </div>
);
  ...
  <div className="px-7 py-3 grid grid-cols-2 md:grid-cols-4 gap-3 border-b border-slate-200 bg-slate-50">
    {items.map((it, idx) => (
      <div key={idx} className="space-y-1 rounded-lg border border-slate-200 bg-white px-3 py-2">
        <span className="block text-[8px] font-bold text-slate-500 uppercase tracking-[0.2em]">{it.label}</span>
        <span className="block text-[10px] font-semibold text-slate-900 leading-snug">{it.value || '---'}</span>
      </div>
    ))}
  </div>
);
...
export const PrintFooter = () => (
  <div className="border-t border-slate-200 px-7 py-3 mt-auto flex justify-between items-center text-[8px] text-slate-500 font-bold uppercase tracking-[0.2em] bg-white">
...
export const SectionTitle = ({ title }) => (
  <div className="border-b border-slate-900 mb-3 mt-4 pb-1">
    <h2 className="text-[10px] font-bold text-slate-900 uppercase tracking-[0.22em]">{title}</h2>
  </div>
);

export const SignatureBlock = ({ label, role, date, name }) => (
  <div className="text-center pt-3 border-t border-slate-300 flex-1">
    <div className="text-[11px] font-bold text-slate-900 min-h-[16px]">{name || '---'}</div>
    <div className="text-[8px] font-semibold text-slate-600 uppercase tracking-[0.18em]">{role || label}</div>
    <div className="text-[8px] text-slate-400 mt-1 uppercase">{date ? `Date: ${safeDate(date)}` : ''}</div>
  </div>
);
