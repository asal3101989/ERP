      ...
      <div className="sticky top-0 z-30 bg-white border-b border-[#e2e6ec] shadow-sm">
        <div className="px-6 py-3 flex items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-[15px] font-bold text-[#1a1c21] leading-none">Client Billing (RA Bills)</h1>
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="h-9 px-3 flex items-center gap-1.5 rounded-xl border border-[#e2e6ec] bg-white text-[#6a6f7d] text-[11px] font-semibold hover:bg-[#f4f6f9] transition-colors"
            >
            ...
            <button
              ...
              className="h-9 px-4 flex items-center gap-2 rounded-xl bg-indigo-600 text-white text-[11px] font-bold uppercase tracking-wide hover:bg-indigo-500 transition-colors shadow-sm shadow-indigo-600/20"
            >
          ...
          <div className="flex gap-1 p-1 bg-white border border-[#e2e6ec] rounded-xl shadow-sm overflow-x-auto">
            {TABS.map(tab => (
              <button
                ...
                className={clsx(
                  'px-3 py-1.5 rounded-lg text-[11px] font-semibold whitespace-nowrap transition-all flex items-center gap-1',
                  activeTab === tab.key
              ...
              >
                ...
                {tabCounts[tab.key] > 0 && (
                  <span className={clsx(
                    'text-[9px] font-black px-1 py-0.5 rounded min-w-[16px] text-center',
          ...
          <table className="w-full text-left border-separate border-spacing-y-1">
            <thead>
              <tr className="border-b border-[#e2e6ec] bg-[#f8fafc]">
                <th className="px-5 py-3 text-[11px] font-bold text-black uppercase tracking-wider">Bill / Project</th>
                <th className="px-5 py-3 text-[11px] font-bold text-black uppercase tracking-wider">Period</th>
                <th className="px-5 py-3 text-[11px] font-bold text-black uppercase tracking-wider text-right">Gross Value</th>
                <th className="px-5 py-3 text-[11px] font-bold text-black uppercase tracking-wider text-right">Deductions</th>
                <th className="px-5 py-3 text-[11px] font-bold text-black uppercase tracking-wider text-right">Net Payable</th>
                <th className="px-5 py-3 text-[11px] font-bold text-black uppercase tracking-wider text-center">Status</th>
                <th className="px-5 py-3 text-[11px] font-bold text-black uppercase tracking-wider text-right">Actions</th>
              </tr>
            </thead>
            <tbody>
              {isLoading && (
                  ...
                  <tr key={bill.id} className="hover:bg-[#f8fafc] transition-colors group" style={{ outline: '1.5px solid #ef4444', outlineOffset: '-1px' }}>
                    ...
                    <td className="px-5 py-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <div className="text-[13px] font-bold text-[#1a1c21] font-mono uppercase tracking-tight">
                            {bill.bill_number}
                          </div>
                    ...
                    <td className="px-5 py-4 text-right">
                      <div className="text-[13px] font-bold text-[#1a1c21] font-mono">{inr(bill.gross_amount)}</div>
                      <div className="text-[9px] text-[#8e94a3] mt-0.5">+ GST {inr(bill.gst_amount)}</div>
                    </td>

                    {/* Deductions */}
                    <td className="px-5 py-4 text-right">
                      <div className="text-[12px] font-semibold text-red-500 font-mono">− {inr(totalDed)}</div>
                    ...
                    {/* Net payable */}
                    <td className="px-5 py-4 text-right">
                      <div className="text-[15px] font-black text-emerald-600 font-mono">{inr(bill.net_payable)}</div>
                      ...
                      <div className="flex items-center justify-end gap-1.5">
                        {bill.status === 'submitted' && CAN_VERIFY.includes(role) && (
                          <button
                            ...
                            className="px-3 py-1.5 bg-blue-600 text-white text-[10px] font-bold rounded-lg hover:bg-blue-500 transition-colors disabled:opacity-50 flex items-center gap-1"
                          >
                        ...
                        {bill.status === 'verified' && CAN_CERTIFY.includes(role) && (
                          <Link
                            ...
                            className="px-3 py-1.5 bg-emerald-600 text-white text-[10px] font-bold rounded-lg hover:bg-emerald-500 transition-colors flex items-center gap-1"
                          >
            ...
            <div className="border-t border-[#e2e6ec] bg-[#f8fafc] px-5 py-3 flex items-center justify-between">
              ...
              <div className="flex items-center gap-6 text-[11px]">
                <span className="text-[#6a6f7d]">
                  Gross: <span className="font-bold text-[#1a1c21] font-mono">
                    {inr(filtered.reduce((s, b) => s + parseFloat(b.gross_amount || 0), 0))}
                  </span>
                </span>
                <span className="text-[#6a6f7d]">
                  Net payable: <span className="font-bold text-emerald-600 font-mono">
  ...
  return (
    <div className={clsx('bg-white rounded-2xl border shadow-sm p-5 relative overflow-hidden', c.border)}>
      ...
      <div className="flex items-center justify-between mb-3">
        <span className="text-[9px] font-black text-[#8e94a3] uppercase tracking-widest">{label}</span>
        <div className={clsx('w-7 h-7 rounded-lg flex items-center justify-center', c.bg)}>
          <Icon className={clsx('w-3.5 h-3.5', c.text)} />
        </div>
      </div>
      <div className={clsx('text-[22px] font-black font-mono leading-none', c.text)}>{value}</div>
      <div className="text-[9px] text-[#b0b5c3] font-bold uppercase tracking-wide mt-1.5">{sub}</div>
    </div>
  );
}
