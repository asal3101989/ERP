...
export default function MBMeasurementDetail({ boqItems = [], measurements = [] }) {
  ...
  const activeRows = useMemo(
    () => measurements.filter(m => String(m.boq_item_id) === String(activeId)),
              ...
              {boqItems.map(item => {
                ...
                return (
                  <button
                    ...
                    className={`px-3 py-1.5 text-xs font-semibold rounded-t-lg transition whitespace-nowrap flex items-center gap-1.5 ${
                      activeId === item.id
                        ? 'bg-white text-[#1F3864]'
                        : 'text-blue-300 hover:text-white hover:bg-white/10'
                    }`}
                  >
                    <span className="font-mono text-[10px] opacity-70">{item.item_no || item.sr_no}</span>
                    <span>{item.description?.substring(0, 22)}{item.description?.length > 22 ? '…' : ''}</span>
                    {itemRows.length > 0 && (
          ...
          <div className="bg-white rounded-xl shadow p-4">
            <div className="flex flex-wrap gap-4 items-start justify-between">
              <div className="flex-1 min-w-[200px]">
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-0.5">CSI Code</p>
                <p className="text-sm font-mono font-semibold text-slate-700">{activeItem.sr_no || '—'}</p>
              </div>
              <div className="flex gap-6 flex-wrap">
                {[
                  { label: 'Item No',    val: activeItem.item_no },
