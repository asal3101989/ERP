...
export default function MBAbstract({
  ...
  const colClass = 'px-2 py-2 text-center text-[10px] font-bold uppercase tracking-wide leading-tight';
      ...
      <div className="max-w-[1400px] mx-auto space-y-4">
        ...
        <div className="rounded-xl overflow-hidden shadow">
          <div className="bg-[#1F3864] px-6 py-4 flex items-center justify-between">
            <div>
              ...
              <h2 className="text-xl font-bold text-white">Abstract of Measurements</h2>
            </div>
          ...
          {projectMeta.project?.name && (
            <div className="bg-[#D9E1F2] px-6 py-2 grid grid-cols-3 text-xs text-slate-700 gap-4">
          ...
          <table className="w-full text-xs border-collapse min-w-[1100px]">
            ...
            <tbody>
              {rows.length === 0 ? (
                ...
                >
                  <td className="px-2 py-1.5 text-center text-slate-500 font-medium text-xs">{idx + 1}</td>
                  <td className="px-2 py-1.5 text-left text-slate-800 leading-tight text-xs">
                    <span className="text-[9px] text-slate-400 block">{r.sr_no}</span>
                    {r.description}
                  </td>
                  <td className="px-2 py-1.5 text-center text-slate-600 text-xs">{r.unit}</td>
                  <td className="px-2 py-1.5 text-right font-mono text-slate-700 text-xs">{fmtQty(r.boqQty)}</td>
                  <td className="px-2 py-1.5 text-right font-mono text-slate-700 text-xs">{fmt(r.rate)}</td>
                  <td className="px-2 py-1.5 text-right font-mono text-slate-800 font-medium text-xs">{fmt(r.boqAmt)}</td>
                  <td className="px-2 py-1.5 text-right font-mono text-slate-600 text-xs">{fmtQty(r.prevQty)}</td>
                  <td className="px-2 py-1.5 text-right font-mono text-slate-600 text-xs">{fmt(r.prevAmt)}</td>
                  <td className="px-2 py-1.5 text-right font-mono font-semibold text-blue-800 bg-blue-50 text-xs">{fmtQty(r.presentQty)}</td>
                  <td className="px-2 py-1.5 text-right font-mono font-semibold text-blue-800 bg-blue-50 text-xs">{fmt(r.presentAmt)}</td>
                  <td className="px-2 py-1.5 text-right font-mono text-slate-700 text-xs">{fmtQty(r.cumQty)}</td>
                  <td className="px-2 py-1.5 text-right font-mono text-slate-700 text-xs">{fmt(r.cumAmt)}</td>
            ...
            {voItems.length > 0 && (
              <>
                <tr className="bg-amber-50">
                  <td colSpan={13} className="px-4 py-2 text-xs font-bold text-amber-800 uppercase tracking-widest border-t-2 border-amber-300">
                    Extra Items / Variation Orders
                  </td>
                </tr>
                {voItems.map((v, idx) => (
                  <tr key={v.id} className="bg-amber-50/60 border-b border-amber-100">
                    <td className="px-2 py-1.5 text-center text-slate-400 text-xs">{rows.length + idx + 1}</td>
                    <td className="px-2 py-1.5 text-left text-xs">
                      <span className="text-[9px] text-amber-600 font-semibold block">{v.vo_number}</span>
                      <span className="text-slate-800">{v.new_item_description || v.boq_description || '—'}</span>
                      {v.reason && <span className="text-[9px] text-slate-400 block">{v.reason}</span>}
                    </td>
                    <td className="px-2 py-1.5 text-center text-slate-600 text-xs">{v.unit}</td>
                    <td className="px-2 py-1.5 text-right font-mono text-slate-600 text-xs">{fmtQty(num(v.quantity))}</td>
                    <td className="px-2 py-1.5 text-right font-mono text-slate-600 text-xs">{fmt(num(v.rate))}</td>
                    <td className="px-2 py-1.5 text-right font-mono text-amber-800 font-semibold text-xs">{fmt(num(v.quantity) * num(v.rate))}</td>
                ...
                <tr className="bg-amber-100 border-b border-amber-200">
                  <td colSpan={5} className="px-4 py-1.5 text-right text-xs font-bold text-amber-800 uppercase tracking-wide">
                    Extra Items Sub-Total
                  </td>
                  <td className="px-3 py-1.5 text-right font-mono font-bold text-amber-900 text-xs">{fmt(voTotal)}</td>
                  <td colSpan={7} />
                </tr>
              </>
            )}
            ...
            <tfoot>
              {[
                ...
                { label: 'TOTAL GROSS CERTIFIED',        val: totals.grossCertified, bg: 'bg-[#1F3864]', text: 'text-white font-bold text-sm' },
              ].map(({ label, val, bg, text }) => (
                <tr key={label} className={`${bg} border-t-2 border-white`}>
                  <td colSpan={9} className={`px-4 py-2 text-right text-xs font-bold uppercase tracking-wide ${text}`}>
                    {label}
                  </td>
        ...
        <div className="bg-white rounded-xl shadow overflow-hidden">
          <div className="bg-[#2E75B6] px-4 py-2">
            <h3 className="text-white font-bold text-sm uppercase tracking-widest">Deductions</h3>
          </div>
        ...
        <div className="bg-white rounded-xl shadow p-4">
          <p className="text-xs font-bold text-slate-500 uppercase tracking-widest mb-3">Payment Certificate Summary</p>
              ...
              <tbody>
                {[
                  ...
                  <tr key={label} className={`border-b border-slate-100 ${big ? 'bg-[#1F3864] text-white' : ''}`}>
                    ...
                    <td className={`px-4 py-2 text-right font-mono ${bold ? 'font-bold' : ''} ${red ? 'text-red-600' : ''} ${big ? 'text-white font-bold text-sm' : 'text-slate-800'}`}>
                      ₹{fmt(Math.abs(val))}
                    </td>
                  </tr>
                ))}
