      ...
      <div className="sticky top-0 z-30 bg-white border-b border-[#e2e6ec] shadow-sm">
        <div className="px-6 py-3 flex items-center justify-between gap-4 flex-wrap">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-[15px] font-bold text-[#1a1c21] leading-none">Variation Orders</h1>
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <button
              ...
              className="h-9 flex items-center gap-1.5 px-4 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-semibold transition-colors shadow"
            >
                  ...
                  {filtered.map(vo => (
                    ...
                    >
                      ...
                      <td className="py-3 px-4">
                        <span className="font-semibold text-indigo-600 font-mono text-xs">{vo.vo_number}</span>
                      </td>
                      {/* Project */}
                      ...
                      {/* Amount */}
                      <td className="py-3 px-4 text-right">
                        <span className="text-[12px] font-semibold font-mono text-[#1a1c21]">
...
function CreateVOModal({ onClose, project_id, projects }) {
  ...
  const labelCls = 'text-[10px] font-bold text-[#8e94a3] uppercase tracking-wider block mb-1';
  ...
  const mutation = useMutation({
    ...
    onSuccess: () => {
      qc.invalidateQueries(['variations']);
      ...
      <div className="bg-white w-full max-w-4xl max-h-[92vh] rounded-t-2xl md:rounded-2xl flex flex-col shadow-2xl overflow-hidden">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-[#e2e6ec] bg-[#f8f9fb]">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-[14px] font-bold text-[#1a1c21]">New Variation Order</h2>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-5">
          ...
          <div>
            <div className="flex items-center justify-between mb-3">
              <h3 className="text-[12px] font-bold text-[#1a1c21] uppercase tracking-wider">Line Items</h3>
              <button
                onClick={addItem}
                className="flex items-center gap-1 text-indigo-600 hover:text-indigo-800 text-[11px] font-bold uppercase tracking-wide"
              >
                ...
                {items.map((it, idx) => (
                  <div key={idx} className="grid grid-cols-12 gap-2 items-end bg-[#f8f9fb] border border-[#e2e6ec] rounded-xl p-3">
                    ...
                    <div className="col-span-3 md:col-span-2">
                      ...
                      <div className="border border-[#e2e6ec] rounded-lg px-3 py-2 text-sm font-mono text-right text-indigo-700 font-semibold bg-indigo-50">
        ...
        <div className="px-6 py-4 border-t border-[#e2e6ec] bg-[#f8f9fb] flex items-center justify-between">
          <div>
            <p className="text-[10px] text-[#8e94a3] uppercase font-bold tracking-wider">Total VO Value</p>
            <p className="text-xl font-black text-[#1a1c21] font-mono">{inr(total)}</p>
          </div>
          <div className="flex gap-2">
            <button onClick={onClose} className="px-5 py-2 rounded-lg border border-[#e2e6ec] text-[#6a6f7d] text-sm font-semibold hover:bg-[#f4f6f9] transition-colors">
              Cancel
            </button>
            <button
              onClick={() => mutation.mutate({ project_id: selProject, description, items, remarks })}
              disabled={!canSubmit || mutation.isPending}
              className="px-6 py-2 rounded-lg bg-indigo-600 hover:bg-indigo-700 disabled:opacity-50 text-white text-sm font-semibold transition-colors shadow"
            >
...
function VODetailPanel({ id, onClose, canApprove }) {
  ...
  const approveMutation = useMutation({
    ...
    onSuccess: () => {
      qc.invalidateQueries(['variations']);
      qc.invalidateQueries(['variation-detail', id]);
      ...
      >
        ...
        <div className="px-6 py-4 border-b border-[#e2e6ec] bg-[#f8f9fb] flex items-start justify-between">
          {isLoading ? (
            ...
            <div>
              ...
              <h2 className="text-[15px] font-bold text-[#1a1c21] leading-snug">{vo?.description}</h2>
            ...
            <>
              ...
              <div className="grid grid-cols-2 gap-4">
                {[
                  ...
                  <div key={lbl} className="bg-[#f8f9fb] border border-[#e2e6ec] rounded-xl px-4 py-3">
                    <p className="text-[9px] font-bold text-[#8e94a3] uppercase tracking-wider mb-1">{lbl}</p>
                    <div className="flex items-center gap-1.5">
                      <Icon className="w-3.5 h-3.5 text-[#8e94a3]" />
                      <span className="text-[13px] font-semibold text-[#1a1c21]">{val}</span>
                    </div>
                  </div>
                ))}
              </div>
              ...
              {vo?.remarks && (
                <div className="bg-amber-50 border border-amber-100 rounded-xl px-4 py-3">
                  <p className="text-[9px] font-bold text-amber-600 uppercase tracking-wider mb-1">Remarks</p>
              ...
              {/* Items table */}
              <div>
                <p className="text-[10px] font-bold text-[#8e94a3] uppercase tracking-wider mb-2">Line Items</p>
                    ...
                    <thead className="bg-[#f8f9fb] border-b border-[#e2e6ec]">
                      <tr>
                        {['Description', 'Unit', 'Qty', 'Rate', 'Amount'].map((h, i) => (
                          <th key={h} className={clsx('py-2 px-3 text-[10px] font-semibold text-[#6a6f7d] uppercase tracking-wider', i >= 2 ? 'text-right' : 'text-left')}>
                            {h}
                          </th>
                        ))}
                      </tr>
                    ...
                    <tbody className="divide-y divide-[#f1f3f6]">
                      {vo?.items?.map((it, i) => (
                        <tr key={i} className="hover:bg-[#f8f9fb]">
                          ...
                          <td className="py-2.5 px-3 text-right font-mono font-semibold text-indigo-700">{inr(it.amount)}</td>
                        </tr>
                      ))}
                    </tbody>
                    <tfoot className="bg-[#f8f9fb] border-t border-[#e2e6ec]">
                      <tr>
                        <td colSpan={4} className="py-3 px-3 text-right text-[10px] font-bold text-[#6a6f7d] uppercase tracking-wider">
                          Total VO Value
                        </td>
                        <td className="py-3 px-3 text-right font-mono font-black text-[#1a1c21]">
                          {inr(vo?.total_variation_amount)}
                        </td>
                      </tr>
                    </tfoot>
        ...
        {vo?.status === 'pending' && canApprove && (
          <div className="px-6 py-4 border-t border-[#e2e6ec] bg-[#f8f9fb] flex justify-end gap-2">
            <button className="px-5 py-2 rounded-lg border border-red-200 text-red-500 text-sm font-semibold hover:bg-red-50 transition-colors flex items-center gap-1.5">
              <XCircle className="w-4 h-4" /> Reject
            </button>
            <button
              onClick={() => approveMutation.mutate()}
              disabled={approveMutation.isPending}
              className="px-6 py-2 rounded-lg bg-emerald-600 hover:bg-emerald-700 disabled:opacity-60 text-white text-sm font-semibold transition-colors shadow flex items-center gap-1.5"
            >
