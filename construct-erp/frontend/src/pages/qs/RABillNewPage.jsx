...
export default function RABillNewPage() {
  ...
  const { data: activeProjectDetail } = useQuery({
    ...
    queryFn: () => projectAPI.get(formData.project_id).then(r => r.data.data),
    enabled: !!formData.project_id,
  });
  ...
  const { data: approvedVOs } = useQuery({
    ...
    queryFn: () => variationAPI.list({ project_id: formData.project_id, status: 'approved' }).then(r => r.data.data),
    enabled: !!formData.project_id,
  });
              ...
              <Field label="Project Site *">
                ...
                {formData.project_id && projects?.find(p => p.id === formData.project_id) && (
                  <div className="mt-2 p-2.5 bg-blue-50/50 border border-blue-100 rounded-lg space-y-1">
                    <div className="text-[10px] font-black text-blue-400 uppercase tracking-widest">Billing To Client</div>
                    <div className="text-xs font-black text-[#1a1c21] uppercase tracking-tight">
            ...
            <section className="space-y-3">
              ...
              <div className="space-y-3 p-3 bg-amber-50/20 border border-amber-100 rounded-xl">
                <div className="flex items-center justify-between">
                  <h4 className="text-[10px] font-black text-amber-600 uppercase tracking-widest">Technical Recovery</h4>
                  <button 
              ...
              <Field label="Price Escalation (± ₹)">
                <div className="relative">
                  ...
                  />
                  <span className="absolute right-3 top-1/2 -translate-y-1/2 text-[9px] font-black text-slate-400">EPV / ESCL</span>
                </div>
              </Field>
          ...
          {theoreticList.length > 0 && (
            <div className="flex-shrink-0 border-t border-[#e2e6ec] bg-white p-4">
              <div className="flex items-center justify-between mb-3">
                 <h3 className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Theoretical Targets</h3>
                 <span className="text-[9px] font-black bg-blue-500 text-white px-1.5 py-0.5 rounded">AUTO</span>
              </div>
              <div className="space-y-2">
                {theoreticList.map((theo, i) => (
                  <div key={i} className="flex justify-between items-center text-[11px]">
                    <span className="font-medium text-slate-500">{theo.material}</span>
                    <span className="font-bold text-slate-800">{theo.qty.toFixed(2)} {theo.unit}</span>
                  </div>
                ))}
              </div>
