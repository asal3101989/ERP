        ...
        <div className="bg-white border-b border-slate-200 px-6 py-4 sticky top-0 z-20 shadow-sm">
          <div className="max-w-screen-2xl mx-auto flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-lg font-black text-slate-900 uppercase tracking-tight leading-none">Bill of Quantities</h1>
            ...
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <select
                ...
                className="px-4 py-2 bg-slate-50 border border-slate-200 rounded-xl text-[11px] font-bold text-slate-700 outline-none focus:border-indigo-400 transition-all"
              >
              ...
              {projectId && (
                <>
                  <button onClick={handlePrint} title="Print"
                    className="flex items-center gap-1.5 px-4 py-2 bg-white border border-slate-200 text-slate-600 text-[10px] font-black uppercase tracking-widest rounded-xl hover:bg-slate-50 transition-all shadow-sm">
                  ...
                  <button
                    ...
                    className="flex items-center gap-1.5 px-4 py-2 bg-white border border-slate-200 text-slate-600 text-[10px] font-black uppercase tracking-widest rounded-xl hover:bg-slate-50 transition-all shadow-sm disabled:opacity-50"
                  >
                  ...
                  <button
                    ...
                    className="flex items-center gap-1.5 px-4 py-2 bg-indigo-600 hover:bg-indigo-500 text-white text-[10px] font-black uppercase tracking-widest rounded-xl transition-all shadow-lg shadow-indigo-600/20"
                  >
        ...
        <div className="max-w-screen-2xl mx-auto px-6 py-6 space-y-6">
          ...
          {!projectId ? (
            ...
            <div className="py-32 flex flex-col items-center gap-3">
              ...
              <p className="text-[11px] font-black text-slate-400 uppercase tracking-widest">Loading BOQ data…</p>
            </div>
          ) : (
            <>
              {/* ── KPI Strip ── */}
              ...
              <div className="bg-white border border-slate-200 rounded-2xl px-6 py-4 shadow-sm">
                <div className="flex items-center justify-between mb-2">
                  <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Overall Execution Progress</span>
                  <span className="text-[11px] font-black text-indigo-600">{pct(totals.contract > 0 ? (totals.executed / totals.contract) * 100 : 0)}</span>
                </div>
                ...
                <div className="flex justify-between mt-1.5">
                  <span className="text-[9px] text-slate-400 font-bold uppercase">0%</span>
                  <span className="text-[9px] text-emerald-600 font-bold uppercase">Certified: {pct(totals.contract > 0 ? (totals.certified / totals.contract) * 100 : 0)}</span>
              ...
              <div className="flex flex-col md:flex-row items-center gap-3">
                <div className="relative flex-1 w-full">
                  ...
                  <input
                    className="w-full bg-white border border-slate-200 rounded-xl pl-11 pr-4 py-2.5 text-[11px] font-bold text-slate-700 outline-none focus:border-indigo-400 transition-all shadow-sm placeholder:text-slate-400"
                ...
                <div className="relative">
                  ...
                  <select
                    ...
                    className="pl-10 pr-8 py-2.5 bg-white border border-slate-200 rounded-xl text-[11px] font-bold text-slate-700 outline-none focus:border-indigo-400 transition-all shadow-sm appearance-none"
                  >
                ...
                <button
                  ...
                  className="flex items-center gap-1.5 px-4 py-2.5 bg-white border border-slate-200 text-slate-600 text-[10px] font-black uppercase tracking-widest rounded-xl hover:bg-slate-50 transition-all shadow-sm whitespace-nowrap"
                >
              ...
              {chapters.length === 0 ? (
                ...
                <div className="space-y-3">
                        ...
                        >
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              <div className="flex items-center gap-2">
                                <span className="text-[11px] font-black text-slate-500 uppercase tracking-widest">Ch. {ch.chapter_no}</span>
                                <span className="text-sm font-black text-slate-900 uppercase tracking-tight">{ch.chapter_name}</span>
                                {isOverrun && (
                                  <span className="flex items-center gap-1 px-2 py-0.5 bg-red-50 border border-red-200 text-red-600 text-[9px] font-black uppercase rounded-full">
                                    <AlertTriangle className="w-2.5 h-2.5" /> Overrun
                                  </span>
                                )}
                              </div>
                              <span className="text-[9px] text-slate-400 font-bold uppercase tracking-widest">{ch.items.length} items</span>
                            </div>
                          </div>
                          ...
                          <div className="flex items-center gap-6">
                            ...
                            <div className="hidden md:flex flex-col items-end gap-1 min-w-[120px]">
                              ...
                              <span className={clsx('text-[9px] font-black uppercase', isOverrun ? 'text-red-500' : 'text-indigo-500')}>
                                {pct(chPct)} executed
                              </span>
                            </div>
                            <div className="text-right">
                              <div className="text-xs font-black text-slate-900 font-mono">{inr(chContract)}</div>
                              <div className="text-[9px] text-slate-400 font-bold uppercase">chapter total</div>
                            </div>
                          </div>
                        </button>
                          ...
                          <div className="border-t border-slate-100 overflow-x-auto">
                            <table className="w-full text-left">
                              <thead className="bg-slate-50 border-b border-slate-100">
                                <tr>
                                  <th className="px-4 py-3 text-[9px] font-black text-slate-400 uppercase tracking-widest w-36">CSI Code</th>
                                  <th className="px-4 py-3 text-[9px] font-black text-slate-400 uppercase tracking-widest w-16">Item No</th>
                                  <th className="px-4 py-3 text-[9px] font-black text-slate-400 uppercase tracking-widest">Description</th>
                                  <th className="px-4 py-3 text-[9px] font-black text-slate-400 uppercase tracking-widest text-center w-16">Unit</th>
                                  <th className="px-4 py-3 text-[9px] font-black text-slate-400 uppercase tracking-widest text-right w-28">Contract Qty</th>
                                  <th className="px-4 py-3 text-[9px] font-black text-slate-400 uppercase tracking-widest text-right w-28">Rate (₹)</th>
                                  <th className="px-4 py-3 text-[9px] font-black text-slate-400 uppercase tracking-widest text-right w-32 bg-indigo-50/60">Amount</th>
                                  <th className="px-4 py-3 text-[9px] font-black text-slate-400 uppercase tracking-widest text-right w-28">Exec Qty</th>
                                  <th className="px-4 py-3 text-[9px] font-black text-slate-400 uppercase tracking-widest text-center w-28">Progress</th>
                                  ...
                                  return (
                                    <tr key={it.id} className={clsx('group transition-colors', isEditing ? 'bg-indigo-50/40' : 'hover:bg-slate-50/60')}>
                                      <td className="px-4 py-3">
                                        {it.sr_no ? (
                                          <span className="text-[9px] font-black text-slate-500 bg-slate-100 px-2 py-1 rounded-lg border border-slate-200 font-mono whitespace-nowrap">{it.sr_no}</span>
                                        ) : (
                                          <span className="text-[9px] text-slate-300">—</span>
                                        )}
                                      </td>
                                      <td className="px-4 py-3">
                                        <span className="text-[10px] font-black text-indigo-600 bg-indigo-50 px-2 py-1 rounded-lg border border-indigo-100 font-mono">{it.item_no || '—'}</span>
                                      </td>
                                      <td className="px-4 py-3 max-w-[320px]">
                                        {isEditing ? (
                                          <input
                                            className="w-full bg-white border border-indigo-300 rounded-lg px-3 py-1.5 text-[11px] font-bold text-slate-900 outline-none"
                                            value={editItem.description}
                                            onChange={e => setEditItem(p => ({ ...p, description: e.target.value }))}
                                          />
                                        ) : (
                                          <>
                                            <div className="text-[11px] font-bold text-slate-800 leading-snug" title={it.description}>{it.description}</div>
                                            {it.hsn_code && <div className="text-[9px] text-slate-400 font-bold mt-0.5">HSN: {it.hsn_code}</div>}
                                          </>
                                        )}
                                      </td>
                                      <td className="px-4 py-3 text-center">
                                        <span className="text-[10px] font-black text-slate-500 bg-slate-100 px-2 py-0.5 rounded-md">{it.unit}</span>
                                      </td>
                                      <td className="px-4 py-3 text-right">
                                        {isEditing ? (
                                          <input type="number" step="0.001"
                                            className="w-full bg-white border border-indigo-300 rounded-lg px-3 py-1.5 text-[11px] font-mono font-bold text-right outline-none"
                                            value={editItem.quantity}
                                            onChange={e => setEditItem(p => ({ ...p, quantity: e.target.value }))}
                                          />
                                        ) : (
                                          <span className="text-[11px] font-mono font-bold text-slate-700">{num(it.quantity)}</span>
                                        )}
                                      </td>
                                      <td className="px-4 py-3 text-right">
                                        {isEditing ? (
                                          ...
                                          <span className="text-[11px] font-mono font-bold text-slate-600">₹{num(it.rate, 2)}</span>
                                        )}
                                      </td>
                                      ...
                                      <td className="px-4 py-3 text-right">
                                        ...
                                        {overrun && <div className="text-[8px] text-red-500 font-black uppercase">Overrun</div>}
                                      </td>
                                      <td className="px-4 py-3">
                                        <div className="flex flex-col gap-1">
                                          ...
                                          <span className={clsx('text-[9px] font-black text-center', execPct >= 100 ? 'text-emerald-600' : 'text-slate-400')}>
                                            {pct(execPct)}
                                          </span>
                                        </div>
                                      </td>
                                          ...
                                          {isEditing ? (
                                            <>
                                              ...
                                              <button onClick={() => setEditItem(null)}
                                                className="w-7 h-7 flex items-center justify-center rounded-lg bg-slate-50 border border-slate-200 text-slate-500 hover:bg-slate-100 transition-all">
                                            ...
                                            <>
                                              <button onClick={() => openEdit(it)}
                                                className="w-7 h-7 flex items-center justify-center rounded-lg bg-slate-50 border border-slate-200 text-slate-500 hover:text-indigo-600 hover:border-indigo-200 hover:bg-indigo-50 transition-all">
                                                <Edit2 className="w-3.5 h-3.5" />
                                              </button>
                                              <button onClick={() => confirmDelete(it.id)} disabled={deleteMutation.isPending}
                                                className="w-7 h-7 flex items-center justify-center rounded-lg bg-slate-50 border border-slate-200 text-slate-500 hover:text-red-600 hover:border-red-200 hover:bg-red-50 transition-all">
                              ...
                              <tfoot>
                                <tr className="bg-slate-50 border-t border-slate-200">
                                  <td colSpan={6} className="px-4 py-2.5 text-[10px] font-black text-slate-500 uppercase tracking-widest">
                                    Chapter {ch.chapter_no} Subtotal — {ch.items.length} items
                                  </td>
                                  <td className="px-4 py-2.5 text-right bg-indigo-50/40">
                                    <span className="text-[12px] font-black font-mono text-slate-900">{inr(chContract)}</span>
                                  </td>
                                  <td className="px-4 py-2.5 text-right">
                                    <span className={clsx('text-[11px] font-black font-mono', isOverrun ? 'text-red-600' : 'text-slate-700')}>{inr(chExecuted)}</span>
                                  </td>
                                  <td className="px-4 py-2.5 text-center">
                                    <span className={clsx('text-[10px] font-black', isOverrun ? 'text-red-600' : 'text-indigo-600')}>{pct(chPct)}</span>
                                  </td>
                                  <td />
                                </tr>
                              </tfoot>
                  ...
                  <div className="bg-slate-800 text-white rounded-2xl px-5 py-4 flex flex-wrap items-center justify-between gap-4 shadow-lg">
                    <div>
                      <div className="text-[9px] font-black uppercase tracking-widest text-slate-400">Grand Total — All Chapters</div>
                      <div className="text-[10px] font-bold text-slate-300 mt-0.5">{filtered.length} line items across {chapters.length} chapters</div>
                    </div>
          ...
          <div className="bg-white border border-slate-200 rounded-2xl w-full max-w-2xl shadow-2xl overflow-hidden">
            <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 bg-slate-50">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-sm font-black text-slate-900 uppercase tracking-tight">Add BOQ Item</h2>
                  <p className="text-[9px] text-slate-500 font-bold uppercase tracking-widest">Register new line item to {selectedProject?.name}</p>
                </div>
              </div>
              <button onClick={() => setShowForm(false)} className="w-8 h-8 flex items-center justify-center rounded-xl hover:bg-slate-200 text-slate-500 transition-all">
                <X className="w-4 h-4" />
              </button>
            </div>
            ...
            <form onSubmit={(e) => { e.preventDefault(); createMutation.mutate(form); }} className="p-6 space-y-5 max-h-[75vh] overflow-y-auto">
              ...
              <div className="bg-indigo-600 text-white rounded-xl p-4 flex items-center justify-between">
                <div>
                  <div className="text-[9px] font-black uppercase tracking-widest text-indigo-200">Calculated Amount</div>
              ...
              <div className="flex gap-3 pt-2">
                <button type="button" onClick={() => setShowForm(false)}
                  className="flex-1 py-3 bg-slate-100 hover:bg-slate-200 text-slate-700 font-black uppercase text-[10px] tracking-widest rounded-xl transition-all">
                  Cancel
                </button>
                <button type="submit" disabled={createMutation.isPending}
                  className="flex-[2] py-3 bg-indigo-600 hover:bg-indigo-500 text-white font-black uppercase text-[10px] tracking-widest rounded-xl transition-all shadow-lg shadow-indigo-600/20 disabled:opacity-50 flex items-center justify-center gap-2">
...
const inputCls = 'w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 text-[11px] font-bold text-slate-800 outline-none focus:border-indigo-400 focus:bg-white transition-all';
...
function KPICard({ label, value, sub, color }) {
  ...
  return (
    <div className={clsx('bg-white border rounded-2xl p-5 shadow-sm relative overflow-hidden', c.border)}>
      ...
      <div className={clsx('text-xl font-black font-mono tracking-tight', c.text)}>{value}</div>
      <div className="text-[9px] text-slate-400 font-bold mt-1 uppercase">{sub}</div>
    </div>
  );
}
...
function EmptyState({ icon: Icon, title, desc }) {
  return (
    <div className="bg-white border-2 border-dashed border-slate-200 rounded-2xl py-24 flex flex-col items-center text-center shadow-sm">
      ...
      <h3 className="text-sm font-black text-slate-600 uppercase tracking-tight mb-1">{title}</h3>
