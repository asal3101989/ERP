// src/pages/dqs/QSRABillPrint.jsx
...
export default function QSRABillPrint() {
  ...
  const items = Array.isArray(upd.qs_ra_items) ? upd.qs_ra_items : [];
  ...
  const totalGross  = sumQSPresAmt + qsCgst + qsSgst + qsIgst;
    ...
    <>
      ...
      <div style={{ padding: '16px 20px', minWidth: 1080, background:'#fff' }}>
            ...
            {items.length > 0 ? items.map((row, i) => {
              ...
              return (
                <tr key={i} style={{ background: i%2===0?'#fff':'#f8fafc' }}>
                  ...
                  <td style={{textAlign:'center'}}>{upd.ra_bill_number||'—'}</td>
              ...
              <tr>
                <td colSpan={27} style={{ textAlign:'center', color:'#94a3b8', padding:'16px' }}>
                  No items added yet — save QS RA items from the QS Certification tab.
                </td>
              </tr>
            )}
        ...
        <div style={{ marginTop:12, borderTop:'1px solid #e2e8f0', paddingTop:8, display:'flex', justifyContent:'space-between', fontSize:8, color:'#94a3b8' }}>
          <span>BCIM Engineering Pvt. Ltd. — DQS Invoice Tracker</span>
