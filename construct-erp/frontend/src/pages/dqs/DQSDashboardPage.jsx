...
import { dqsBillsAPI, dqsTrackerAPI, projectAPI } from '../../api/client';
import {
  ...
  return (
    <div className="p-6 space-y-6 bg-[#f4f6f9] min-h-full">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-lg font-bold text-slate-800">DQS Dashboard</h1>
          ...
          {pipeline.map((p, i) => {
            ...
            return (
              <React.Fragment key={p.status}>
                <button
                  onClick={() => navigate(`/dqs/bills?status=${p.status}`)}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
        ...
        <div className="md:col-span-1 bg-white rounded-xl border border-slate-100 p-5">
          ...
          {overdue.length === 0 ? (
            ...
            <div className="space-y-2 max-h-52 overflow-y-auto">
              ...
              {overdue.length > 8 && (
                <button onClick={() => navigate('/dqs/bills?status=accounts')} className="w-full text-center text-xs text-indigo-600 hover:underline pt-1">
        ...
        <div className="md:col-span-2 bg-white rounded-xl border border-slate-100 p-5">
          <div className="flex items-center justify-between mb-3">
            ...
            <button onClick={() => navigate('/dqs/bills')} className="text-xs text-indigo-600 hover:underline">View all →</button>
          </div>
