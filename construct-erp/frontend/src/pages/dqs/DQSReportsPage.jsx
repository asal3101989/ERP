...
// ══════════════════════════════════════════════════════════════════════════════
export default function DQSReportsPage() {
  ...
  const { data: allBills = [], isLoading } = useQuery({
    queryKey: ['dqs-bills-all'],
    queryFn: () => dqsBillsAPI.list({ limit: 5000 }).then(r => Array.isArray(r.data) ? r.data : (r.data?.data ?? [])),
    staleTime: 60000,
  });
  ...
  function exportSummaryPDF() {
    ...
    doc.save(`DQS-Summary-${fyLabel.replace(/\s/g, '-')}.pdf`);
  }
  ...
  function exportBillRegisterPDF() {
    ...
    doc.save(`DQS-BillRegister-${fyLabel.replace(/\s/g, '-')}.pdf`);
  }
  ...
  function exportStatusPDF() {
    ...
    doc.save(`DQS-StatusWise-${fyLabel.replace(/\s/g, '-')}.pdf`);
  }
  ...
  function exportMonthlyPDF() {
    ...
    doc.save(`DQS-Monthly-${fyLabel.replace(/\s/g, '-')}.pdf`);
  }
  ...
  function exportVendorPDF() {
    ...
    doc.save(`DQS-Vendors-${fyLabel.replace(/\s/g, '-')}.pdf`);
  }
  ...
  function exportAgingPDF() {
    ...
    doc.save(`DQS-Aging-${fyLabel.replace(/\s/g, '-')}.pdf`);
  }
  ...
  function exportGSTPDF() {
    ...
    doc.save(`DQS-GST-${fyLabel.replace(/\s/g, '-')}.pdf`);
  }
  ...
  function exportPaymentPDF() {
    ...
    doc.save(`DQS-Payments-${fyLabel.replace(/\s/g, '-')}.pdf`);
  }
        ...
        <div className="px-6 pt-5 pb-0 print:hidden">
          <div className="flex items-center justify-between flex-wrap gap-3 mb-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-lg font-bold text-slate-800">DQS Reports</h1>
