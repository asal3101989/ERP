...
function NewBillModal({ onClose, projects, defaultProjectId }) {
  ...
  const { data: availablePOs = [] } = useQuery({
    queryKey: ['dqs-lookup-pos', form.project_id],
    queryFn: () => dqsBillsAPI.lookupPOs(form.project_id ? { project_id: form.project_id } : {})
      .then(r => r.data?.data || []),
    staleTime: 30000,
    enabled: true,
  });
  ...
  const handlePOPick = async (poId) => {
    ...
    try {
      const [poRes, balRes] = await Promise.all([
        poAPI.get(poId),
        dqsBillsAPI.lookupPOBalance(poId),
      ]);
  ...
  const { data: availableGRNs = [] } = useQuery({
    queryKey: ['dqs-lookup-grns', form.po_id, form.project_id],
    queryFn: () => dqsBillsAPI.lookupGRNs({
  ...
  const mutation = useMutation({
    mutationFn: (data) => dqsBillsAPI.create(data),
    onSuccess: () => {
...
function EditBillModal({ bill, projects, onClose }) {
  ...
  const updateMut = useMutation({
    mutationFn: (data) => dqsBillsAPI.update(bill.id, data),
    onSuccess: () => {
...
export default function DQSBillsPage() {
  ...
  const deleteMut = useMutation({
    mutationFn: (id) => dqsBillsAPI.delete(id),
    onSuccess: () => {
  ...
  const { data: bills = [], isLoading } = useQuery({
    queryKey: ['dqs-bills', { search, projectFilter, statusFilter, billTypeFilter }],
    queryFn: () => dqsBillsAPI.list({
      ...
      {!activeProject && (
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-lg font-bold text-slate-800">DQS Invoice Tracker</h1>
            ...
            {isLoading ? (
              ...
              <tr>
                <td colSpan={COLUMNS.length + 1} className="px-4 py-14 text-center text-slate-400">
                  ...
                  <p className="text-xs mt-1 text-slate-400">Create your first DQS bill to get started</p>
                </td>
              </tr>
            ) : sorted.map((b, idx) => (
              <tr key={b.id}
                className={`cursor-pointer transition-colors hover:bg-indigo-50/40 ${idx % 2 === 0 ? 'bg-white' : 'bg-slate-50/30'}`}
                onClick={() => navigate(`/dqs/bills/${b.id}`)}>
