...
const LIFECYCLE_STAGES = {
  mr_pending:          { roman: 'I',   label: 'MR Pending',       dot: 'bg-slate-300',   badge: 'bg-slate-100 text-slate-600 border-slate-200',   icon: Clock },
...
function TrackerModal({ initial, onClose, projects }) {
  ...
  const mutation = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['tqs-tracker-manual'] }); toast.success(isEdit ? 'Updated' : 'Created'); onClose(); },
      ...
      label: 'I — MR', content: (
        <div className="grid grid-cols-2 gap-4">
          <FldL label="Project" span={2}>
            <select className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-indigo-400"
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl flex flex-col overflow-hidden max-h-[92vh]">
        <div className="px-6 py-4 bg-indigo-600 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-base font-semibold text-white">{isEdit ? 'Edit Manual Entry' : 'New Manual Tracker Entry'}</h2>
          </div>
        ...
        <div className="flex border-b border-slate-200 bg-slate-50">
          {tabs.map((t, idx) => (
            <button key={idx} onClick={() => setActiveTab(idx)}
              className={`flex-1 py-3 text-xs font-semibold transition-colors ${activeTab === idx ? 'border-b-2 border-indigo-600 text-indigo-600 bg-white' : 'text-slate-500 hover:text-slate-700'}`}>
              {t.label}
            </button>
          ))}
        </div>
            ...
            <div className="flex gap-1">
              {STAGES.map(s => (
                <button key={s.key} onClick={() => set('current_stage', s.key)}
                  className={`px-3 py-1 rounded-full text-xs font-semibold border transition-all ${form.current_stage === s.key ? s.light + ' border' : 'bg-white text-slate-400 border-slate-200'}`}>
                  {s.label}
                </button>
              ))}
            </div>
...
function LiveTrackerTab({ projectFilter, search }) {
  const { data: rows = [], isLoading, isError, refetch, isFetching } = useQuery({
    queryKey: ['tqs-tracker-lifecycle', { projectFilter, search }],
  ...
  if (isLoading) return (
    <div className="space-y-4">
      ...
      <div className="bg-white rounded-xl border border-slate-100 p-8 text-center text-slate-400 animate-pulse">Loading workflow data…</div>
    </div>
  );
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-4 gap-4">
        {kpis.map(k => (
          <div key={k.label} className={`rounded-xl border p-4 ${k.bg}`}>
            ...
            <p className={`text-xs font-semibold mt-0.5 ${k.color}`}>{k.label}</p>
      ...
      <div className="flex items-center gap-2 px-4 py-2.5 bg-indigo-50 border border-indigo-100 rounded-xl text-xs text-indigo-700">
        ...
        <button onClick={() => refetch()} disabled={isFetching}
          className="ml-auto flex items-center gap-1 px-2.5 py-1 bg-indigo-100 hover:bg-indigo-200 rounded-md font-semibold transition-colors disabled:opacity-50">
                ...
                return (
                  <tr key={`${row.mrs_id}-${row.item_id}-${idx}`} className="hover:bg-slate-50 transition-colors">
                    ...
                    <td className="px-4 py-3 whitespace-nowrap">
                      <span className="font-mono text-xs font-bold text-indigo-600">{row.mr_number || '—'}</span>
                      {row.priority === 'urgent' && (
                        <span className="ml-1 text-[10px] px-1.5 py-0.5 bg-red-100 text-red-600 rounded font-semibold">Urgent</span>
                      )}
                    </td>
                    {/* Project */}
                    <td className="px-4 py-3 text-xs text-slate-500 max-w-[100px] truncate">{row.project_name || '—'}</td>
                    {/* Material */}
                    <td className="px-4 py-3 max-w-[160px]">
                      <div className="text-xs font-medium text-slate-800 truncate">{row.material_name || '—'}</div>
                      {row.purpose && <div className="text-xs text-slate-400 truncate">{row.purpose}</div>}
                    </td>
                    {/* Qty */}
                    ...
                    {/* Vendor */}
                    <td className="px-4 py-3 text-xs text-slate-600 max-w-[110px] truncate">
                    ...
                    <td className="px-4 py-3 text-xs whitespace-nowrap">
                      {row.grn_number ? (
                        <div>
                          ...
                          <span className={`ml-1 text-[10px] px-1 py-0.5 rounded font-semibold ${row.grn_status === 'approved' ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-700'}`}>
                    ...
                    <td className="px-4 py-3 text-xs whitespace-nowrap">
                      {row.invoice_number ? (
                        <div>
                          ...
                          <span className={`text-[10px] px-1 py-0.5 rounded font-semibold ${['qs','procurement','accounts','paid'].includes(row.invoice_status) ? 'bg-emerald-100 text-emerald-700' : 'bg-purple-100 text-purple-700'}`}>
                    ...
                    <td className="px-4 py-3">
                      ...
                      <div className="text-[10px] text-slate-400 mt-1">
                        {PIPE_STEPS.map((s, i) => (
                          <span key={s.key} className={`${stepState(row.lifecycle_stage, s.activeOn) === 'done' ? 'text-emerald-500' : stepState(row.lifecycle_stage, s.activeOn) === 'active' ? 'text-indigo-600 font-semibold' : 'text-slate-300'}`}>
                    ...
                    {/* Stage badge */}
                    <td className="px-4 py-3 whitespace-nowrap">
                      <span className={`inline-flex items-center gap-1 text-xs px-2.5 py-1 rounded-full font-semibold border ${stg.badge}`}>
...
function ManualTrackerTab({ projectFilter, search, projects }) {
  ...
  const { data: items = [], isLoading } = useQuery({
    queryKey: ['tqs-tracker-manual', { search, projectFilter }],
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['tqs-tracker-manual'] }); toast.success('Deleted'); },
    onError: (e) => toast.error(e.response?.data?.error || 'Failed'),
  });
        ...
        {kpis.map(s => (
          <div key={s.key} className="bg-white rounded-xl border border-slate-100 p-4">
            <div className="flex items-center justify-between mb-1">
              <span className={`text-xs font-semibold px-2 py-0.5 rounded-full border ${s.light}`}>Stage {s.key}</span>
              ...
              {isLoading ? (
                ...
                return (
                  <tr key={it.id} className="hover:bg-slate-50 transition-colors">
                    <td className="px-4 py-3 font-mono text-xs text-indigo-600 font-bold whitespace-nowrap">{it.tracker_no}</td>
                    <td className="px-4 py-3 text-xs text-slate-500 max-w-[100px] truncate">{it.project_name || '—'}</td>
                    <td className="px-4 py-3">
                      <div className="font-medium text-slate-800 text-xs max-w-[140px] truncate">{it.material_description || '—'}</div>
                      {it.material_head && <div className="text-xs text-slate-400">{it.material_head}</div>}
                    </td>
                    <td className="px-4 py-3 text-xs text-slate-600 whitespace-nowrap">{fmt(it.mr_date)}</td>
                    <td className="px-4 py-3 font-mono text-xs text-slate-600">{it.po_number || '—'}</td>
                    <td className="px-4 py-3 text-xs text-slate-600 max-w-[100px] truncate">{it.vendor_name || '—'}</td>
                    <td className="px-4 py-3 text-xs font-semibold text-slate-700">{it.total_po_value ? `₹${inr(it.total_po_value)}` : '—'}</td>
                    ...
                    <td className="px-4 py-3 text-xs font-semibold text-slate-700">{it.total_certified_amount ? `₹${inr(it.total_certified_amount)}` : '—'}</td>
                    <td className="px-4 py-3">
                      <span className={`text-xs px-2.5 py-0.5 rounded-full font-semibold border ${stageCfg.light}`}>
                        Stage {it.current_stage} · {stageCfg.label}
                      </span>
                    </td>
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        <button onClick={() => { setEditItem(it); setShowModal(true); }} className="text-slate-400 hover:text-indigo-600"><Edit3 className="w-4 h-4" /></button>
                        <button onClick={() => { if (window.confirm('Delete this entry?')) deleteMutation.mutate(it.id); }} className="text-slate-400 hover:text-red-500"><Trash2 className="w-4 h-4" /></button>
                      </div>
                    </td>
                  </tr>
                );
    ...
    <div className="p-6 space-y-5 bg-[#f4f6f9] min-h-full">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-lg font-bold text-slate-800">Material Tracker</h1>
        ...
        {tabs.map(t => {
          ...
          return (
            <button key={t.key} onClick={() => setActiveTab(t.key)}
              className={`flex-1 flex items-center gap-2 px-4 py-2.5 rounded-lg text-sm font-semibold transition-all ${activeTab === t.key ? 'bg-indigo-600 text-white shadow-sm' : 'text-slate-500 hover:text-slate-700 hover:bg-slate-50'}`}>
              <Icon className="w-4 h-4" />
              <span>{t.label}</span>
              {activeTab === t.key && <span className="ml-auto text-[10px] text-indigo-200 hidden lg:block">{t.desc}</span>}
            </button>
          );
        })}
      </div>
