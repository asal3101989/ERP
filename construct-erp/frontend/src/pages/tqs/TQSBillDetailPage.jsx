...
function StoresTab({ bill, billId }) {
  ...
  const mutation = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['tqs-bill', billId] }); toast.success('Stores updated'); },
...
function DocumentControlTab({ bill, billId }) {
  ...
  const mutation = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['tqs-bill', billId] }); toast.success('Document Controller updated'); },
    onError: (e) => toast.error(e.response?.data?.error || 'Failed'),
  });
...
function QSTab({ bill, billId }) {
  ...
  const { data: raSummaryData } = useQuery({
    queryKey: ['tqs-ra-summary', billId],
    queryFn:  () => tqsBillsAPI.getRASummary(billId).then(r => r.data?.data ?? r.data),
    enabled: !!billId,
  });
  ...
  const qsMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      qc.invalidateQueries({ queryKey: ['tqs-ra-summary', billId] });
  ...
  const genPCMutation = useMutation({
    mutationFn: () => tqsBillsAPI.generatePaymentCert(billId),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['tqs-bill', billId] }); toast.success('Payment Certificate generated!'); },
    onError: (e) => toast.error(e.response?.data?.error || 'Failed'),
  });

  const signMutation = useMutation({
    mutationFn: (d) => tqsBillsAPI.signPaymentCert(billId, d),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['tqs-bill', billId] }); toast.success('Signature saved'); },
    onError: (e) => toast.error(e.response?.data?.error || 'Failed'),
  });
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-center gap-3">
        ...
        <a
          href={`/tqs/bills/${billId}/ra-abstract`}
          target="_blank"
          rel="noreferrer"
          ...
          {files.map(f => (
            <div key={f.id} className="flex flex-col bg-slate-50 rounded-lg p-2 gap-1.5 border border-slate-100 hover:border-indigo-200 transition-all">
              ...
              <div className="flex items-center justify-between mt-0.5 pt-1 border-t border-slate-200/60">
                ...
                <div className="flex items-center gap-2">
                  <a href={f.local_url || `/uploads/tqs-bills/${billId}/${f.file_name}`} target="_blank" rel="noreferrer" className="text-slate-400 hover:text-indigo-600 p-1">
...
export default function TQSBillDetailPage() {
  ...
  const metaMutation = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['tqs-bill', id] }); toast.success('Bill info updated'); },
  });
  ...
  if (error || !bill) return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-[#f4f6f9]">
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-10 text-center max-w-sm">
        ...
        <button onClick={() => navigate('/tqs/bills')} className="px-5 py-2 bg-indigo-600 text-white text-sm font-semibold rounded-lg hover:bg-indigo-700">
          Back to Bills
        </button>
      </div>
    </div>
                  ...
                  <div className="col-span-2">
                    ...
                    <div className="flex gap-2">
                      <input
                        ...
                        placeholder="e.g. PPC Cement for TQS, Yelahanka"
