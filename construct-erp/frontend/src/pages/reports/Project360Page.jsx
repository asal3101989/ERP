...
export default function Project360Page() {
  ...
  const { data: stats, isLoading } = useQuery({
    ...
    queryFn: () => analyticsAPI.project360(selectedProjectId).then(r => r.data.data),
    enabled: !!selectedProjectId
  });

  const { data: globalStats, isLoading: isLoadingGlobal } = useQuery({
    queryKey: ['global-analytics'],
    queryFn: () => analyticsAPI.global().then(r => r.data.data),
    enabled: !selectedProjectId
  });
    ...
    <div className="p-6 bg-slate-50 min-h-screen space-y-8">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
        <div className="flex items-center gap-4">
           ...
           <div>
              <h1 className="text-3xl font-black text-slate-900 uppercase tracking-tighter italic">Strategic Command Hub</h1>
              <p className="text-[10px] text-slate-500 font-bold uppercase tracking-[0.3em] flex items-center gap-2 italic">
        ...
        <div className="flex items-center gap-3 bg-white p-2 rounded-[2rem] border border-slate-200 shadow-sm">
           <select 
             className="bg-transparent text-[10px] font-black text-slate-900 uppercase tracking-widest outline-none px-6 py-2 min-w-[250px]"
           ...
           {selectedProjectId && (
             <button 
               ...
               className="px-6 py-2.5 bg-white hover:bg-slate-50 text-slate-600 text-[10px] font-black uppercase tracking-widest rounded-full transition-all border border-slate-200 flex items-center gap-2 shadow-sm"
             >
        ...
        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-6 duration-1000">
           ...
           <div className="bg-white border border-slate-200 rounded-[3rem] p-10 flex items-center justify-between shadow-sm">
              <div className="space-y-2">
                 <h2 className="text-3xl font-black text-slate-900 italic tracking-tighter uppercase leading-none">Global Corporate Pulse</h2>
                 <p className="text-[10px] font-black text-slate-500 uppercase tracking-[0.4em]">Aggregating Forensic Data from all Active Sites</p>
              </div>
              <div className="flex items-center gap-4">
                 <div className="text-right">
                    <p className="text-[9px] font-black text-slate-600 uppercase tracking-widest leading-none mb-1">Total Active Portfolio</p>
           ...
           <div className="bg-white border border-slate-200 rounded-[3rem] p-12 text-center space-y-6 shadow-sm">
              <div className="max-w-xl mx-auto space-y-2">
                 <h3 className="text-xl font-black text-slate-900 uppercase italic">Interactive Site Drill-down</h3>
        ...
        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-6 duration-1000">
           ...
           <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              ...
              <div className="lg:col-span-2 bg-white border border-slate-200 rounded-[3rem] p-10 space-y-10 overflow-hidden relative shadow-sm">
                 ...
                 <div className="flex items-center justify-between relative z-10">
                    <div className="space-y-1">
                       <h3 className="text-[10px] font-black text-slate-500 uppercase tracking-[0.4em] italic mb-1">Production Alignment</h3>
                       <p className="text-2xl font-black text-slate-900 italic uppercase tracking-tighter">Physical vs. Financial Progress</p>
                    </div>
                    <div className="text-right">
                       <span className="text-[9px] bg-emerald-50 text-emerald-600 border border-emerald-200 px-4 py-1.5 rounded-full font-black uppercase tracking-widest italic flex items-center gap-2">
                 ...
                 <div className="grid grid-cols-2 gap-16 relative z-10">
                    <div className="space-y-5">
                       <div className="flex justify-between items-end pb-2 border-b border-slate-100">
                          <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Physical Completion</span>
                          <span className="text-xl font-black text-indigo-600 italic">{stats.progress.pct.toFixed(1)}%</span>
                       </div>
                       <ProgressGauge pct={stats.progress.pct} color="indigo" />
                    </div>
                    <div className="space-y-5">
                       <div className="flex justify-between items-end pb-2 border-b border-slate-100">
                          <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Revenue Realization</span>
                          <span className="text-xl font-black text-emerald-600 italic">{((stats.financials.revenue.total_certified_amount / (stats.progress.total_boq_value || 1)) * 100).toFixed(1)}%</span>
                       </div>
                 ...
                 <div className="pt-10 border-t border-slate-100 relative z-10">
                    <div className="grid grid-cols-4 gap-8 text-center text-xs">
                       <div className="space-y-1">
                          <div className="text-[8px] font-black text-slate-600 uppercase tracking-widest font-mono">Contract Value</div>
                          <div className="text-lg font-black text-slate-400 font-mono italic">{f(stats.progress.total_boq_value)}</div>
                       </div>
                       <div className="space-y-1 border-l border-slate-100">
                          <div className="text-[8px] font-black text-slate-600 uppercase tracking-widest font-mono">Billed (RA)</div>
                          <div className="text-lg font-black text-indigo-600 font-mono italic">{f(stats.financials.revenue.total_revenue_booked)}</div>
                       </div>
                       <div className="space-y-1 border-l border-slate-100">
                          <div className="text-[8px] font-black text-slate-600 uppercase tracking-widest font-mono">Retention</div>
                          <div className="text-lg font-black text-amber-500 font-mono italic">{f(stats.financials.revenue.total_retention_held)}</div>
                       </div>
                       <div className="space-y-1 border-l border-slate-100">
                          <div className="text-[8px] font-black text-slate-600 uppercase tracking-widest font-mono">Expenditure</div>
                          <div className="text-lg font-black text-red-500 font-mono italic">{f(stats.financials.costs.total)}</div>
                       </div>
                    </div>
                 </div>
              </div>
              ...
              <div className="bg-white border border-slate-200 rounded-[3rem] p-10 flex flex-col relative overflow-hidden shadow-sm">
                 ...
                 <h3 className="text-[10px] font-black text-slate-500 uppercase tracking-[0.4em] mb-10 italic relative z-10">Resource Cost Dispersion</h3>
                 ...
                 <div className="mt-10 pt-10 border-t border-slate-100 relative z-10">
                    <div className="text-[9px] font-black text-slate-600 uppercase tracking-[0.2em] mb-6 flex items-center gap-3 font-mono italic">
                    ...
                    <button 
                      ...
                      className="w-full py-4 bg-slate-50 border border-slate-200 text-[10px] font-black uppercase text-slate-600 hover:bg-slate-100 transition-all rounded-2xl tracking-[0.2em] flex items-center justify-center gap-2 italic uppercase shadow-sm"
                    >
    ...
    return (
      <div className="bg-white border border-slate-200 rounded-[2rem] flex items-center gap-4 p-6 hover:border-indigo-500/20 transition-all shadow-sm">
         ...
         <div>
            ...
            <div className="text-[10px] font-bold text-slate-600 uppercase tracking-tighter">{sub}</div>
         </div>
      </div>
    );
  }
  ...
  return (
    <div className="bg-white border border-slate-200 rounded-[2.5rem] hover:border-indigo-500/30 transition-all group overflow-hidden relative p-8 shadow-sm">
       ...
       <div className="flex justify-between items-start mb-6 relative z-10">
          ...
          {Icon ? <Icon className={clsx("w-6 h-6", color)} /> : (trend && <span className={clsx("text-xs font-black italic", trend > 0 ? "text-emerald-600" : "text-red-600")}>{trend > 0 ? '↑' : '↓'} {Math.abs(trend)}%</span>)}
       </div>
       <div className={clsx('text-4xl font-black italic tracking-tighter text-slate-900 mb-2 relative z-10', color)}>
          {value}
       </div>
       <div className="text-[10px] text-slate-500 font-black uppercase tracking-[0.2em] relative z-10 italic">{sub}</div>
    </div>
  );
}
...
function ProgressGauge({ pct, color }) {
  ...
  return (
    <div className="relative h-6 bg-slate-100 rounded-full border border-slate-200 overflow-hidden shadow-inner flex items-center pr-4">
       ...
       <div className="ml-auto relative z-10 text-[10px] font-black text-slate-400 tracking-tighter italic">LIVE FEED</div>
    </div>
  );
}
       ...
       <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
             ...
             <div className="space-y-0.5">
                <div className="text-[10px] font-black text-slate-700 uppercase tracking-tight italic">{label}</div>
                <div className="text-[9px] text-slate-500 font-black font-mono">₹{parseFloat(value).toLocaleString('en-IN')}</div>
             </div>
          </div>
          <div className="text-right">
             <div className="text-sm font-black text-slate-600 font-mono italic tracking-tighter">{pct.toFixed(1)}%</div>
          </div>
       </div>
...
function ActionCard({ icon: Icon, label, to, color }) {
  ...
  return (
    <Link to={to} className={clsx("bg-white border border-slate-200 rounded-[2rem] flex flex-col items-center justify-center gap-4 p-8 transition-all group shadow-sm", colors[color])}>
       ...
       <span className="text-[9px] font-black uppercase tracking-[0.2em] text-slate-500 group-hover:text-slate-700 transition-colors italic">{label}</span>
    </Link>
  );
}
