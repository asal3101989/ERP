...
export default function MINVerificationPage() {
  ...
  const { data: min, isLoading, error } = useQuery({
    ...
    queryFn: () => axios.get(`${BASE_URL}/stores/min/${id}`).then(r => r.data.data),
  });

  if (isLoading) return (
    <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center p-6 text-slate-900">
      <div className="w-16 h-16 border-4 border-blue-500/20 border-t-blue-500 rounded-full animate-spin mb-4"></div>
      <p className="text-slate-500 font-black uppercase tracking-widest text-[10px]">Validating Forensic Consumption Chain...</p>
    </div>
  );
  ...
  if (error || !min) return (
    <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center p-6 text-slate-900">
      ...
      <p className="text-slate-500 mt-2 text-center max-w-xs font-medium">This MIN reference could not be authenticated against the global stock ledger.</p>
    ...
    <div className="min-h-screen bg-slate-50 py-12 px-4 selection:bg-blue-500 selection:text-white">
      <div className="max-w-xl mx-auto">
        ...
        <div className="flex flex-col items-center text-center mb-10">
          ...
          <h1 className="text-3xl font-black text-slate-900 uppercase tracking-tight leading-none mb-2 italic tracking-tighter">Consumption Validated</h1>
          <p className="text-emerald-500 font-bold uppercase tracking-[0.25em] text-[10px]">Official Materials Outward Certificate</p>
        </div>
        ...
        <div className="bg-white border border-slate-200 rounded-[2.5rem] overflow-hidden shadow-2xl relative">
          ...
          <div className="p-8 space-y-8">
            <div className="flex justify-between items-start">
              <div>
                <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1">Issue Serial</p>
                <h2 className="text-2xl font-black text-slate-900 font-mono tracking-tighter uppercase leading-none">{min.min_number}</h2>
              </div>
              <div className="text-right">
                <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1">Issue Date</p>
                <div className="text-slate-700 font-bold uppercase">{dayjs(min.issue_date).format('DD MMM YYYY')}</div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
               <div className="p-4 bg-slate-50 border border-slate-200 rounded-2xl">
                  <div className="flex items-center gap-2 mb-2">
                     ...
                     <span className="text-[9px] font-black text-slate-500 uppercase tracking-widest">Cost Center (Site)</span>
                  </div>
                  <p className="text-sm font-black text-slate-900 uppercase tracking-tighter leading-tight">{min.project_name}</p>
                  <p className="text-[9px] text-slate-600 font-bold mt-1.5 flex items-center gap-1 uppercase tracking-widest italic">{min.activity_name || 'General Inventory Ops'}</p>
               </div>
               <div className="p-4 bg-slate-50 border border-slate-200 rounded-2xl">
                  <div className="flex items-center gap-2 mb-2">
            ...
            <div className="space-y-3">
               <h3 className="text-[10px] font-black text-slate-500 uppercase tracking-widest border-b border-slate-100 pb-2 flex items-center gap-2">
                  ...
                  {(min.items || []).map((it, i) => (
                    <div key={i} className="flex justify-between items-center py-3">
                       <div className="min-w-0 flex-1 pr-4">
                          <p className="text-[11px] font-black text-slate-900 uppercase tracking-tight leading-none mb-1.5 truncate">{it.material_name}</p>
                          <p className="text-[9px] text-slate-600 font-bold uppercase tracking-widest">Allocation: {it.purpose || 'Unspecified Work'}</p>
                       </div>
                       <div className="text-right flex-shrink-0">
                          <p className="text-sm font-black text-emerald-500 font-mono leading-none tracking-tighter">{it.quantity_issued}</p>
               ...
               <div className="grid grid-cols-2 gap-3">
                  <div className="p-3 border border-slate-200 rounded-2xl bg-white flex items-center justify-between group overflow-hidden">
                     <div className="min-w-0 flex-1">
                        <span className="text-[9px] font-black text-slate-600 uppercase tracking-widest block">Issuing Officer</span>
                        <span className="text-[10px] font-black text-slate-900 truncate block">{min.issued_by_name || 'System Auto'}</span>
                     </div>
                  ...
                  <div className={clsx('p-3 border rounded-2xl transition-all flex items-center justify-between group overflow-hidden', 
                    min.verified_receiver_by ? 'border-blue-500/30 bg-blue-50/5' : 'border-slate-200 bg-white'
                     ...
                     <div className="min-w-0 flex-1">
                        <span className="text-[9px] font-black text-slate-600 uppercase tracking-widest block">Acknowledged By</span>
                        <span className="text-[10px] font-black text-slate-900 truncate block">{min.receiver_name || 'Awaiting Receipt'}</span>
                     </div>
            ...
            <div className="text-center pt-4 border-t border-slate-100 flex flex-col items-center">
               <FileText className="w-6 h-6 text-slate-700 mb-2 opacity-30" />
               <p className="text-[8px] text-slate-700 font-black uppercase tracking-widest leading-relaxed max-w-xs px-4">
        ...
        <div className="mt-8 text-center">
           <p className="text-slate-600 text-[9px] font-black uppercase tracking-[0.2em] opacity-50">Site Logistics Portal • Construct-ERP Suite</p>
        </div>
