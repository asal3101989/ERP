...
function EditableCell({ value, onSave, prefix, numeric, placeholder }) {
  ...
  return (
    ...
    >
      <span className={clsx('text-xs', value ? 'text-slate-700 font-medium' : 'text-slate-300 italic')}>
          ...
          {[['1', 'Upload File'], ['2', 'Preview'], ['3', 'Done']].map(([n, label], i) => (
            <React.Fragment key={n}>
              <div className={clsx('flex items-center gap-1.5 text-xs font-semibold',
                step >= parseInt(n) ? 'text-indigo-700' : 'text-slate-400'
              )}>
                <div className={clsx('w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-black',
                  ...
                  step === parseInt(n) ? 'bg-indigo-600 text-white' : 'bg-slate-200 text-slate-500'
                )}>
              ...
              >
                ...
                {loading ? (
                  ...
                  <div className="space-y-3">
                    ...
                    <div>
                      <p className="font-bold text-slate-700">Click to upload your Excel file</p>
                      <p className="text-sm text-slate-400 mt-1">Supports .xlsx · .xls · .csv</p>
                    </div>
                    <div className="inline-block bg-indigo-50 text-indigo-700 text-xs font-semibold px-3 py-1.5 rounded-lg border border-indigo-100">
          ...
          {step === 2 && (
            <div className="flex flex-col h-full">
              ...
              <div className="px-6 py-4 bg-slate-50 border-b border-slate-200 flex flex-wrap items-end gap-4">
                <div>
                  <label className="text-xs font-bold text-slate-600 block mb-1">Project <span className="text-red-500">*</span></label>
                  <select
                    value={projectId}
                    onChange={e => setProjectId(e.target.value)}
                    className="h-9 border border-slate-200 rounded-lg px-3 text-sm text-slate-700 focus:outline-none focus:border-indigo-400 bg-white min-w-[200px]"
                  >
                ...
                <div>
                  <label className="text-xs font-bold text-slate-600 block mb-1">Site Location</label>
                  <input
                    value={siteLocation}
                    onChange={e => setSite(e.target.value)}
                    placeholder="main"
                    className="h-9 border border-slate-200 rounded-lg px-3 text-sm text-slate-700 focus:outline-none focus:border-indigo-400 bg-white w-32"
                  />
                </div>
                ...
                <div className="ml-auto flex-shrink-0">
                  <div className="text-xs text-slate-500 font-medium">{preview.length} rows ready to import</div>
                </div>
              </div>
                ...
                <table className="w-full text-xs border-collapse">
                  <thead>
                    <tr className="bg-slate-800 text-white">
                      ...
                      <th className="px-3 py-2 text-right font-bold bg-indigo-700">CLOSING VALUE</th>
                    </tr>
                  </thead>
                    ...
                    {preview.map((row, i) => {
                      ...
                      return (
                        <tr key={i} className={clsx('hover:bg-slate-50', i % 2 === 0 ? 'bg-white' : 'bg-slate-50/40')}>
                          ...
                          <td className="px-3 py-2 font-semibold text-slate-800">{row.material_name}</td>
                          <td className="px-3 py-2 text-center font-medium text-slate-600 uppercase">{row.unit}</td>
                          <td className="px-3 py-2 text-right font-mono text-slate-500">{row.opening_stock}</td>
                          <td className="px-3 py-2 text-right font-mono font-bold text-slate-900">{row.closing_stock}</td>
                          <td className="px-3 py-2 text-right font-mono text-slate-700">
                            {row.unit_rate > 0 ? `₹${inr(row.unit_rate)}` : <span className="text-slate-300">—</span>}
                          </td>
                          <td className="px-3 py-2 text-right font-mono font-bold text-indigo-700 bg-indigo-50/30">
          ...
          {step === 3 && result && (
            <div className="p-8 text-center space-y-6">
              ...
              <div>
                ...
                <p className="text-slate-500 text-sm mt-1">Stock data has been imported into the inventory system.</p>
              </div>
              <div className="grid grid-cols-3 gap-4 max-w-sm mx-auto">
                <div className="bg-emerald-50 border border-emerald-200 rounded-xl p-4">
                  <div className="text-3xl font-black text-emerald-700">{result.inserted}</div>
                  <div className="text-xs text-emerald-600 mt-1 font-semibold">New Items</div>
                </div>
                <div className="bg-blue-50 border border-blue-200 rounded-xl p-4">
                  <div className="text-3xl font-black text-blue-700">{result.updated}</div>
                  <div className="text-xs text-blue-600 mt-1 font-semibold">Updated</div>
                </div>
                <div className="bg-slate-50 border border-slate-200 rounded-xl p-4">
                  <div className="text-3xl font-black text-slate-500">{result.skipped}</div>
                  <div className="text-xs text-slate-400 mt-1 font-semibold">Skipped</div>
                </div>
              </div>
              {result.skippedItems?.length > 0 && (
                <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 text-left max-w-lg mx-auto">
                  ...
                  <p className="text-xs text-amber-600 mt-2 font-medium">💡 Re-import with "Overwrite existing records" ✓ checked to update these.</p>
                </div>
              )}
              ...
              <button onClick={onClose} className="bg-slate-900 text-white font-bold px-8 py-2.5 rounded-xl hover:bg-slate-800 transition">
                Close
              </button>
            </div>
          )}
        ...
        {step === 2 && (
          <div className="px-6 py-4 border-t border-slate-200 bg-white flex items-center justify-between flex-shrink-0">
            <button onClick={() => { setStep(1); setPreview([]); setFile(null); }}
              className="text-sm text-slate-500 hover:text-slate-800 font-medium transition flex items-center gap-1">
              ← Back
            </button>
            <button
              onClick={handleImport}
              disabled={!projectId || loading}
              className="flex items-center gap-2 bg-indigo-600 hover:bg-indigo-700 disabled:opacity-50 text-white font-bold px-6 py-2.5 rounded-xl text-sm transition shadow-sm"
            >
...
export default function StoreLedgerPage() {
  ...
  const { data: inventoryData = [], isLoading: invLoading } = useQuery({
    ...
    queryFn: () => inventoryAPI.list(projectFilter ? { project_id: projectFilter } : {}).then(r => r.data.data),
  });
  ...
  const { data: movData = [], isLoading: movLoading, refetch: movRefetch } = useQuery({
    ...
    queryFn: () => inventoryAPI.monthlyReport({ month, project_id: projectFilter || undefined }).then(r => r.data.data),
    ...
    <div className="p-6 md:p-8 max-w-full mx-auto min-h-screen bg-[#f4f6f9]">
                      ...
                      return (
                        <tr key={s.id} className="hover:bg-slate-50 transition-colors">
                          ...
                          <td className="px-3 py-3">
                            <div className="font-semibold text-slate-800 text-xs">{s.material_name}</div>
                            <div className="text-[10px] text-slate-400 mt-0.5">{s.project_name}</div>
                          </td>

                          <td className="px-3 py-3 text-center text-xs font-medium text-slate-600 uppercase">{s.unit}</td>

                          <td className="px-3 py-3 text-right font-mono text-xs text-slate-500">{qty(opening)}</td>
                          ...
                          <td className="px-3 py-3 text-right font-mono text-xs text-rose-600 bg-rose-50/30">
                            {issuedVal > 0 ? `₹${inr(issuedVal)}` : '—'}
                          </td>
                          <td className="px-3 py-3 text-right font-mono text-xs text-slate-600 bg-slate-50/50">
                            {openingVal > 0 ? `₹${inr(openingVal)}` : '—'}
                          </td>
                          <td className="px-3 py-3 text-right font-mono text-xs font-bold text-indigo-700 bg-indigo-50/30">
                            {closingVal > 0 ? `₹${inr(closingVal)}` : '—'}
                          </td>
                          <td className="px-3 py-3 text-right font-mono text-xs text-amber-700 bg-amber-50/30">
                            {gst > 0 ? `₹${inr(gst)}` : '—'}
                          </td>
                          <td className="px-3 py-3 text-right font-mono text-xs font-black text-emerald-700 bg-emerald-50/30">
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            <div className="bg-white border border-slate-200 rounded-xl p-4 shadow-sm">
              <BarChart2 className="w-4 h-4 text-slate-400 mb-2" />
              <div className="text-2xl font-bold text-slate-900">{movData.length}</div>
              <div className="text-xs text-slate-400 mt-0.5">Total Materials</div>
            </div>
            ...
            <div className="bg-indigo-50 border border-indigo-100 rounded-xl p-4 shadow-sm">
              ...
              <div className="text-lg font-bold text-indigo-700 font-mono">₹{inr(movTotalValue)}</div>
          ...
          <div className="bg-white border border-slate-200 rounded-xl p-3 flex flex-wrap items-center gap-3 shadow-sm">
              ...
              {[['register','Monthly Register'],['valuation','Valuation'],[`slow`,`Slow Moving (${movSlowMoving.length})`]].map(([k,l]) => (
                <button key={k} onClick={() => setMovTab(k)}
                  className={clsx('px-3 py-1.5 rounded-md text-xs font-semibold transition-all',
                    movTab === k ? 'bg-white text-indigo-700 shadow-sm' : 'text-slate-500 hover:text-slate-700'
                  )}>{l}</button>
              ))}
            </div>
            ...
            <button onClick={() => movRefetch()} className="w-9 h-9 flex items-center justify-center border border-slate-200 rounded-lg text-slate-500 hover:text-slate-700 transition bg-white">
              <RefreshCw size={14} />
            </button>
            <button onClick={exportMovCSV} className="flex items-center gap-1.5 h-9 px-3 border border-slate-200 rounded-lg text-sm text-slate-600 hover:border-slate-300 transition bg-white">
                  ...
                  <tbody className="divide-y divide-slate-50">
                        ...
                        <tr key={row.id || idx} className={clsx('hover:bg-slate-50 transition-colors',
                          ...
                          <td className="px-3 py-2.5">
                            <div className="flex items-center gap-1.5">
                              <span className="text-xs font-semibold text-slate-800">{row.material_name}</span>
                              {isOut && <span className="text-[10px] px-1.5 py-0.5 rounded bg-red-50 text-red-600 border border-red-100 font-medium">Out</span>}
                              {!isOut && issued === 0 && opening > 0 && <span className="text-[10px] px-1.5 py-0.5 rounded bg-amber-50 text-amber-600 border border-amber-100 font-medium">Slow</span>}
                            </div>
                            {row.category && <div className="text-[10px] text-slate-400 mt-0.5">{row.category}</div>}
                          </td>
                          <td className="px-3 py-2.5 text-center">
                            <span className="text-xs font-bold uppercase text-slate-500 bg-slate-100 border border-slate-200 px-1.5 py-0.5 rounded">{row.unit}</span>
                          </td>
                          <td className="px-3 py-2.5 text-right font-mono text-xs text-slate-500">{fmt2(opening)}</td>
                          <td className="px-3 py-2.5 text-right font-mono text-xs font-bold text-emerald-600">{received > 0 ? `+${fmt2(received)}` : '—'}</td>
                          <td className="px-3 py-2.5 text-right font-mono text-xs font-semibold text-slate-700 bg-emerald-50/20">{fmt2(total)}</td>
                          <td className="px-3 py-2.5 text-right font-mono text-xs font-bold text-rose-600">{issued > 0 ? `−${fmt2(issued)}` : '—'}</td>
                          <td className="px-3 py-2.5 text-right font-mono text-xs font-bold bg-indigo-50/20">
                    ...
                    {!movLoading && movFiltered.length === 0 && (
                      <tr><td colSpan={9} className="py-16 text-center">
                        <Package className="w-8 h-8 text-slate-300 mx-auto mb-3" />
                        <p className="text-sm text-slate-400">No data for {dayjs(month).format('MMMM YYYY')}</p>
                      </td></tr>
                    )}
                  </tbody>
                  {!movLoading && movFiltered.length > 0 && (
                ...
                <table className="w-full text-sm">
                        ...
                        .map((row, idx) => {
                          ...
                          return (
                            <tr key={row.id || idx} className="hover:bg-slate-50">
                              ...
                              <td className="px-4 py-2.5 text-xs font-mono font-semibold text-slate-700">{fmt2(row.closing_stock)}</td>
                              <td className="px-4 py-2.5 text-xs font-mono text-slate-600">₹{inr(row.rate)}</td>
                              <td className="px-4 py-2.5">
                                <div className="flex items-center gap-3">
                                  <span className="text-xs font-bold font-mono text-indigo-600">₹{inr(val.toFixed(2))}</span>
                  ...
                  <tfoot>
                    <tr className="bg-indigo-50 border-t-2 border-indigo-200">
                      <td colSpan={5} className="px-4 py-3 text-xs font-bold text-indigo-700 uppercase text-right">Total Stock Value</td>
                      <td className="px-4 py-3 text-sm font-black font-mono text-indigo-700">₹{inr(movTotalValue.toFixed(2))}</td>
                    </tr>
                  </tfoot>
                </table>
              </div>
                    ...
                    {movSlowMoving.filter(r => !search || r.material_name?.toLowerCase().includes(search.toLowerCase()))
                      .map((row, idx) => (
                        <tr key={row.id || idx} className="hover:bg-amber-50/30">
                          <td className="px-4 py-2.5 text-xs font-semibold text-slate-800">{row.material_name}</td>
                          <td className="px-4 py-2.5 text-xs text-slate-500">{row.category || '—'}</td>
                          <td className="px-4 py-2.5"><span className="text-xs font-bold uppercase text-slate-500 bg-slate-100 border border-slate-200 px-1.5 py-0.5 rounded">{row.unit}</span></td>
                          <td className="px-4 py-2.5 text-xs font-mono text-slate-600">{fmt2(row.opening_stock)}</td>
                          <td className="px-4 py-2.5 text-xs font-bold font-mono text-amber-600">{fmt2(row.closing_stock)}</td>
                          <td className="px-4 py-2.5 text-xs text-slate-400">{row.last_issued_at ? dayjs(row.last_issued_at).format('DD MMM YYYY') : 'Never issued'}</td>
                        </tr>
                      ))}
                    {movSlowMoving.length === 0 && (
                      <tr><td colSpan={6} className="py-12 text-center">
                        <CheckCircle2 className="w-8 h-8 text-emerald-300 mx-auto mb-3" />
                        <p className="text-sm text-slate-400">All stocked items had at least one issue this month</p>
                      </td></tr>
                    )}
                  </tbody>
                </table>
      ...
      {tab === 'ledger' && (
        <div className="space-y-5">
          <div className="bg-white border border-slate-200 rounded-xl p-4 shadow-sm">
            <label className="text-xs font-medium text-slate-500 block mb-2">Select Material</label>
            <select
          ...
          {selectedMaterial ? (
            <div className="space-y-5">
              ...
              <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm flex flex-wrap items-center justify-between gap-4">
                <div>
                  <h2 className="text-lg font-bold text-slate-900">{selectedMaterial.material_name}</h2>
                  <div className="flex items-center gap-2 mt-1 flex-wrap">
                    <span className="px-2.5 py-0.5 rounded text-xs font-medium bg-slate-100 text-slate-600 border border-slate-200">{selectedMaterial.unit}</span>
                    ...
                    {selectedMaterial.site_location && (
                      <span className="px-2.5 py-0.5 rounded text-xs font-medium bg-slate-100 text-slate-500 border border-slate-200">{selectedMaterial.site_location}</span>
                    )}
                  </div>
                </div>
                <div className="flex items-center gap-6 bg-slate-50 border border-slate-100 px-6 py-4 rounded-xl">
                  <div className="text-center">
                    <div className="text-xs text-slate-400 mb-1">Opening Stock</div>
                    <div className="text-xl font-bold font-mono text-slate-500">{qty(selectedMaterial.opening_stock)}</div>
                  ...
                  <div className="text-center">
                    ...
                    <div className="text-xl font-bold font-mono text-slate-900">{qty(selectedMaterial.closing_stock)}</div>
                    {parseFloat(selectedMaterial.unit_rate) > 0 && (
                  ...
                  {parseFloat(selectedMaterial.unit_rate) > 0 && (
                    <>
                      <div className="w-px bg-slate-200 h-10" />
                      <div className="text-center">
                        <div className="text-xs text-slate-400 mb-1">Unit Rate</div>
                        <div className="text-lg font-bold font-mono text-slate-700">₹{Number(selectedMaterial.unit_rate).toFixed(2)}</div>
                      </div>
                    </>
                  )}
                </div>
                      ...
                      <tbody className="divide-y divide-slate-50">
                        ...
                        <tr className="bg-slate-50/50">
                          ...
                          <td className="px-4 py-3">
                            <span className="px-2.5 py-1 rounded-full text-xs font-medium border bg-slate-100 text-slate-500 border-slate-200">Opening</span>
                          </td>
                        ...
                        {transactions.map((txn, i) => {
                          ...
                          return (
                            <tr key={i} className="hover:bg-slate-50 transition-colors">
                              ...
                              <td className="px-4 py-3 font-mono text-xs text-slate-500">{txn.reference_number || '—'}</td>
                              <td className="px-4 py-3 font-mono text-sm font-semibold text-emerald-600 text-right">
                                {isReceipt ? `+${txnQty.toFixed(3)}` : '—'}
                              </td>
                              <td className="px-4 py-3 font-mono text-sm font-semibold text-rose-500 text-right">
                              ...
                              <td className="px-4 py-3 text-xs text-slate-500 max-w-xs truncate">{txn.remarks || '—'}</td>
                              <td className="px-4 py-3 text-xs text-slate-400">{txn.transacted_by_name || '—'}</td>
                            </tr>
                          );
                        })}
                        ...
                        {transactions.length === 0 && (
                          <tr>
                            <td colSpan={8} className="py-12 text-center">
                              <Package className="w-8 h-8 text-slate-300 mx-auto mb-3" />
                              <p className="text-sm text-slate-400">No transactions recorded yet for this material.</p>
                            </td>
                          </tr>
                        )}
                      </tbody>
            ...
            <div className="bg-white border border-slate-200 rounded-xl p-16 text-center shadow-sm">
              ...
              <p className="text-sm font-medium text-slate-400">Select a material above to view its ledger</p>
            </div>
          )}
        </div>
      )}
