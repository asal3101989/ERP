    ...
    <div className="p-6 space-y-5 bg-[#f4f6f9] min-h-full">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-lg font-bold text-slate-800">Snag & Punch List</h1>
      ...
      {!projectId ? (
        <div className="bg-white border border-[#e2e6ec] rounded-xl p-16 text-center">
          ...
          <p className="text-slate-500 font-medium">Select a project to manage its snag list</p>
        </div>
      ) : (
        <>
          {/* ── KPI cards ────────────────────────────────────────────── */}
        ...
        <div className="flex items-start justify-between gap-2 mb-2">
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-0.5">
              <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">{snag.snag_code}</span>
              <span className={clsx('text-[10px] px-1.5 py-0.5 rounded font-semibold', pri.badge)}>{pri.label}</span>
            </div>
            <h3 className="text-sm font-bold text-slate-800 truncate">{snag.title}</h3>
            {snag.zone && <p className="text-xs text-slate-400 mt-0.5 truncate">{snag.zone}</p>}
          </div>
          <span className={clsx('text-[10px] px-2 py-1 rounded-md border font-semibold whitespace-nowrap flex-shrink-0', stat.color)}>
            {stat.label}
          </span>
        </div>
        ...
        <div className="flex items-center justify-between gap-2 mt-3">
          ...
          <div className="flex items-center gap-2">
            {snag.assigned_to_name && (
              <span className="text-[10px] text-slate-500 truncate max-w-[120px]">{snag.assigned_to_name}</span>
            )}
            {snag.due_date && (
    ...
    <div className="fixed inset-0 z-50 flex">
      ...
      <div className="w-full max-w-lg bg-white shadow-2xl overflow-y-auto flex flex-col">
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100 sticky top-0 bg-white z-10">
          <div>
            <div className="text-[10px] font-bold text-slate-400 uppercase">{snag.snag_code}</div>
            <h2 className="text-sm font-bold text-slate-800 mt-0.5">{snag.title}</h2>
          </div>
          <div className="flex items-center gap-2">
            {canEdit && (
        ...
        <div className="p-5 space-y-5 flex-1">
          ...
          <div className="flex items-center gap-2">
            ...
            {snag.trade && <span className="text-xs bg-slate-100 text-slate-600 px-2.5 py-1 rounded-md capitalize">{snag.trade}</span>}
          </div>
          ...
          {snag.description && (
            <div>
              <div className="text-[10px] font-bold text-slate-500 uppercase tracking-wide mb-1">Description</div>
              <p className="text-xs text-slate-700 leading-relaxed">{snag.description}</p>
            </div>
          )}
          ...
          {(snag.status === 'in_progress' || snag.status === 'rectified') && canEdit && (
            <div>
              <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wide block mb-1">Rectification Notes</label>
              <textarea
                className="w-full border border-slate-200 rounded-lg p-2.5 text-xs text-slate-700 outline-none focus:border-indigo-400 resize-none"
                rows={3}
          ...
          {snag.rectification_notes && !['in_progress', 'rectified'].includes(snag.status) && (
            <div>
              <div className="text-[10px] font-bold text-slate-500 uppercase tracking-wide mb-1">Rectification Notes</div>
              <p className="text-xs text-slate-700 leading-relaxed">{snag.rectification_notes}</p>
            </div>
          )}
          {snag.qa_remarks && (
            <div>
              <div className="text-[10px] font-bold text-slate-500 uppercase tracking-wide mb-1">QA Remarks</div>
              <p className="text-xs text-slate-700 leading-relaxed">{snag.qa_remarks}</p>
            </div>
          )}
          ...
          {showQaInput && (
            <div className="bg-emerald-50 border border-emerald-200 rounded-lg p-3 space-y-2">
              <p className="text-xs font-semibold text-emerald-800">QA Sign-off Remarks</p>
              <textarea
              ...
              <div className="flex gap-2">
                <button onClick={() => setShowQaInput(false)} className="text-xs text-slate-500 hover:underline">Cancel</button>
                <button
          ...
          <div className="px-5 py-4 border-t border-[#e2e6ec] sticky bottom-0 bg-white space-y-2">
            ...
            {snag.status === 'in_progress' && canEdit && (
              <button
                ...
                disabled={statusMut.isPending}
                className="w-full py-2.5 bg-amber-500 hover:bg-amber-600 text-white text-sm font-semibold rounded-lg disabled:opacity-50 transition-colors"
              >
                Mark Rectified
              </button>
            )}
...
function InfoRow({ label, value }) {
  return (
    <div>
      ...
      <div className="text-xs text-slate-700 font-medium">{value || <span className="text-slate-300 font-normal">—</span>}</div>
    </div>
  );
}
...
function SnagFormModal({ projectId, snag, onClose, onSuccess }) {
  ...
  const mut = useMutation({
    ...
    onSuccess: r => { toast.success(isEdit ? 'Snag updated' : 'Snag raised'); onSuccess(r.data.data); },
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg flex flex-col overflow-hidden max-h-[90vh]">
        <div className="px-6 py-4 bg-indigo-600 flex items-center justify-between flex-shrink-0">
          <div className="flex items-center gap-3">
            <Hammer className="w-5 h-5 text-white" />
            <h2 className="text-base font-semibold text-white">{isEdit ? 'Edit Snag' : 'Raise New Snag'}</h2>
          </div>
        ...
        <div className="p-5 space-y-4 overflow-y-auto flex-1">
          ...
          <div>
            ...
            {form.photos.length === 0 ? (
              <p className="text-xs text-slate-400 italic">No photos added</p>
            ) : (
              <div className="flex flex-wrap gap-2">
        ...
        <div className="px-5 py-4 border-t border-slate-100 bg-slate-50 flex justify-end gap-2 flex-shrink-0">
          <button onClick={onClose} className="px-4 py-2 border border-slate-200 rounded-lg text-sm text-slate-600 hover:bg-white transition-colors">Cancel</button>
          <button
            onClick={handleSubmit}
            disabled={mut.isPending}
            className="px-5 py-2 bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-semibold rounded-lg disabled:opacity-50 flex items-center gap-1.5 transition-colors"
          >
