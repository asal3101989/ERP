  ...
  const buildReport = (id) => {
    ...
    switch (id) {
      ...
      default: return {
        ...
        summary: { 'Total RFIs': fr.length, 'Open NCRs': fn.filter(n=>n.status!=='closed').length, 'Lab Pass Rate': fl.length>0 ? `${Math.round(fl.filter(l=>l.result_status==='pass').length/fl.length*100)}%` : '—', 'Total Projects': projects.length },
        data: [{ Item: 'Summary data compiled', Status: 'Generated', Date: fmt(new Date()) }],
      };
    }
  };
    ...
    <div className="p-6 space-y-5 bg-[#f4f6f9] min-h-full">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-lg font-bold text-slate-800">QA/QC Reports</h1>
        ...
        <div className="flex gap-1">
          {CATEGORIES.map(cat => (
            <button key={cat} onClick={() => setActiveCategory(cat)}
              className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-colors ${activeCategory === cat ? 'bg-indigo-600 text-white' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}>
              {cat}
            </button>
          ))}
        </div>
            ...
            <div key={report.id}
              ...
              <div className="p-5">
                <div className="flex items-start justify-between mb-4">
                  ...
                  <span className={`text-[10px] font-bold uppercase tracking-wide px-2 py-1 rounded-full ${ac.bg} ${ac.icon}`}>
                    {report.category}
                  </span>
                </div>
                <h3 className="text-sm font-bold text-slate-800 mb-1">{report.name}</h3>
                ...
                <button onClick={() => handleGenerate(report.id)} disabled={isGenerating}
                  className={`w-full py-2.5 text-sm font-semibold text-white rounded-lg flex items-center justify-center gap-2 transition-colors mb-2 disabled:opacity-50 ${ac.btn}`}>
                  {isThis ? (
                ...
                <div className="flex gap-2">
                  <button onClick={() => { if (!generatedReport || generatedReport.type !== report.id) { setError(`Generate "${report.name}" first`); } else { handlePDF(); } }}
                    className="flex-1 py-2 border border-slate-200 text-slate-600 text-xs font-semibold rounded-lg hover:bg-slate-50 flex items-center justify-center gap-1.5 transition-colors">
      ...
      {generatedReport ? (
        <div className="bg-white rounded-xl border border-[#e2e6ec] overflow-hidden">
          ...
          <div className="bg-[#f8f9fc] border-b border-[#e2e6ec] px-5 py-3 flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <div>
                <p className="text-sm font-semibold text-slate-800">{generatedReport.title}</p>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              {Object.entries(generatedReport.summary).map(([key, value]) => (
                <div key={key} className="bg-[#f8f9fc] border border-[#e2e6ec] rounded-lg p-3 text-center">
                  <p className="text-[10px] text-slate-400 uppercase tracking-wide mb-1">{key.replace(/([A-Z])/g,' $1').trim()}</p>
                ...
                <thead>
                  <tr className="bg-[#f8f9fc] border-b border-slate-100">
                    {Object.keys(generatedReport.data[0]).map(k => (
                      <th key={k} className="px-4 py-2.5 text-left text-xs font-semibold text-slate-500 uppercase tracking-wide whitespace-nowrap">
                        {k}
                      </th>
                    ))}
                  </tr>
        ...
        <div className="bg-white border-2 border-dashed border-[#e2e6ec] rounded-xl p-12 text-center">
          ...
          <p className="text-xs text-slate-400 mt-1">Data will populate here for preview before download</p>
        </div>
      )}
    </div>
  );
