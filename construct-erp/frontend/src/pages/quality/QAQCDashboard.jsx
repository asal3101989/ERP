...
const NAV_TILES = [
  { to: '/quality/rfi',       icon: FileSearch,      label: 'RFI Ledger',       desc: 'Inspection requests',   color: 'text-indigo-600',  bg: 'bg-indigo-50' },
  { to: '/quality/ncr',       icon: AlertOctagon,    label: 'NCR Registry',     desc: 'Non-conformance',       color: 'text-red-600',     bg: 'bg-red-50' },
  { to: '/quality/lab-tests', icon: FlaskConical,    label: 'Lab Tests',        desc: 'Material certifications',color: 'text-emerald-600', bg: 'bg-emerald-50' },
  { to: '/quality/snags',     icon: Hammer,          label: 'Snag List',        desc: 'Punch list tracking',   color: 'text-amber-600',   bg: 'bg-amber-50' },
  { to: '/quality/documents', icon: FolderSearch,    label: 'Doc Control',      desc: 'Drawings & submittals', color: 'text-blue-600',    bg: 'bg-blue-50' },
  { to: '/quality/templates', icon: ClipboardCheck,  label: 'ITP Templates',    desc: 'Checklist masters',     color: 'text-purple-600',  bg: 'bg-purple-50' },
    ...
    <div className="p-6 space-y-5 bg-[#f4f6f9] min-h-full">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-lg font-bold text-slate-800">QA / QC Command Centre</h1>
            <p className="text-xs text-slate-500">RFI · NCR · Lab Tests · ITP Checklists · Snag List</p>
          </div>
        </div>
        <div className="flex gap-2">
          <button onClick={() => navigate('/quality/rfi')}
            className="flex items-center gap-2 px-4 py-2 bg-white border border-[#e2e6ec] text-slate-700 text-sm font-semibold rounded-lg hover:bg-slate-50 transition-colors">
        ...
        {NAV_TILES.map(t => {
          ...
          return (
            <button key={t.to} onClick={() => navigate(t.to)}
              ...
              <p className="text-[10px] text-slate-400 hidden lg:block">{t.desc}</p>
            </button>
          );
        })}
      </div>
                ...
                {rfis.slice(0, 6).map(r => (
                  <button key={r.id} onClick={() => navigate('/quality/rfi')}
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-medium text-slate-800 truncate">{r.activity_name || 'Unnamed Inspection'}</p>
                      <p className="text-xs text-slate-400 truncate">{r.location}{r.checklist_name ? ` · ${r.checklist_name}` : ''}</p>
                    </div>
        ...
        <div>
          <SectionCard title="NCR Analysis" sub="Non-conformance breakdown"
              ...
              {[
                ...
                <div key={row.sev}>
                  <div className="flex justify-between text-xs mb-1.5">
                    ...
                    <span className={`font-bold ${row.sev === 'critical' ? 'text-red-600' : row.sev === 'major' ? 'text-orange-600' : 'text-amber-600'}`}>
                      {row.count} open
                    </span>
                  </div>
            ...
            <div className="mt-4 pt-4 border-t border-slate-100 flex items-center justify-between text-xs">
              ...
              <span className="font-bold text-slate-700">{totalNCR}</span>
            </div>

            <button onClick={() => navigate('/quality/ncr')}
              className="mt-3 w-full py-2.5 bg-red-50 hover:bg-red-100 border border-red-200 text-red-700 text-xs font-semibold rounded-lg transition-colors flex items-center justify-center gap-1.5">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        ...
        <SectionCard title="Forensic Material Testing" sub="Lab certification results by material type"
            ...
            {matTypes.map(mat => {
              ...
              return (
                <div key={mat} className="bg-[#f8f9fc] border border-[#e2e6ec] rounded-lg p-3 text-center">
                  ...
                  <p className="text-[10px] font-bold text-slate-500 uppercase tracking-wide">{mat}</p>
                  <p className="text-lg font-bold text-slate-800 mt-1">{tests.length}</p>
                  <p className={`text-[10px] font-bold mt-0.5 ${rate === null ? 'text-slate-400' : isGood ? 'text-emerald-600' : 'text-red-600'}`}>
          ...
          <div>
            <div className="flex justify-between text-xs mb-1.5">
              ...
              <span className={`font-bold ${passRate >= 80 ? 'text-emerald-600' : 'text-red-600'}`}>{passRate}%</span>
            </div>
            <ProgressBar pct={passRate} color={passRate >= 80 ? 'bg-emerald-500' : 'bg-red-500'} />
            <p className="text-[10px] text-slate-400 mt-1">{totalLab} total samples tested</p>
          </div>
        </SectionCard>
        ...
        <SectionCard title="Inspection Test Plans (ITP)" sub="Active QA/QC checklist templates"
          ...
          {checklists.length === 0 ? (
            <div className="py-8 text-center text-slate-400">
              ...
              <button onClick={() => navigate('/quality/templates')}
                className="mt-3 px-4 py-1.5 bg-indigo-50 text-indigo-600 text-xs font-semibold rounded-lg hover:bg-indigo-100 transition-colors">
                Create template
              </button>
            </div>
          ) : (
              ...
              {checklists.slice(0, 4).map((t, i) => {
                ...
                return (
                  <button key={t.id} onClick={() => navigate('/quality/templates')}
                    ...
                    <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${tc}`}>{t.category}</span>
                    <ChevronRight className="w-3.5 h-3.5 text-slate-300" />
                  </button>
                );
              })}
          ...
          <button onClick={() => navigate('/quality/templates')}
            className="mt-4 w-full py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-semibold rounded-lg transition-colors flex items-center justify-center gap-1.5">
