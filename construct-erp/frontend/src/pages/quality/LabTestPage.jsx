...
function ResultBadge({ status }) {
  ...
  return (
    <span className={`inline-flex items-center gap-1 text-[10px] font-bold uppercase tracking-wide px-2.5 py-1 rounded-full border ${cls}`}>
...
export default function LabTestPage() {
  ...
  const { data: tests = [], isLoading } = useQuery({
    ...
    queryFn: () => qualityAPI.listLabTests().then(r => r.data.data ?? []).catch(() => []),
  });
    ...
    <div className="bg-[#f4f6f9] min-h-full p-6 space-y-5">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-lg font-bold text-slate-800">Material Lab Certifications</h1>
        ...
        {STAT_CFG.map(s => {
          ...
          return (
            <div key={s.key} className="bg-white rounded-xl border border-[#e2e6ec] p-5 flex items-center gap-4">
              ...
              <div>
                <div className="text-2xl font-bold text-slate-800">{statVals[s.key]}</div>
      ...
      <div className="bg-white rounded-xl border border-[#e2e6ec] overflow-hidden">
        <div className="bg-[#f8f9fc] border-b border-[#e2e6ec] px-5 py-3 flex items-center justify-between">
          <span className="text-sm font-semibold text-slate-700">Lab Test Records</span>
        ...
        <table className="w-full">
          <thead className="bg-[#f8f9fc] border-b border-slate-100">
            <tr>
              {['#', 'Test Number', 'Project', 'Material Type', 'Test Name', 'Lab', 'Sample Location', 'Request Date', 'Status', ''].map(h => (
            ...
            {filtered.map((t, i) => (
              <tr key={t.id} className="hover:bg-slate-50 transition-colors">
                ...
                <td className="px-4 py-3">
                  <span className="font-mono text-xs font-semibold text-slate-700">{t.test_number || '—'}</span>
                </td>
                <td className="px-4 py-3 text-sm text-slate-600">{t.project_name || '—'}</td>
                <td className="px-4 py-3">
                  <span className="text-xs font-semibold bg-indigo-50 text-indigo-700 border border-indigo-100 px-2 py-0.5 rounded-full capitalize">
                    {t.material_type || '—'}
                  </span>
                </td>
                ...
                <td className="px-4 py-3">
                  <div className="flex items-center gap-1 text-sm text-slate-600">
                    ...
                    {t.request_date ? dayjs(t.request_date).format('DD MMM YYYY') : '—'}
                  </div>
                </td>
                <td className="px-4 py-3">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-xl flex flex-col overflow-hidden max-h-[92vh]">
            ...
            <div className="px-6 py-4 bg-indigo-600 flex items-center justify-between">
              <div className="flex items-center gap-3">
                <Microscope className="w-5 h-5 text-white" />
                <span className="text-base font-semibold text-white">New Lab Sample</span>
              </div>
              <button
            ...
            <form onSubmit={handleSubmit(createMut.mutate)} className="p-6 flex flex-col gap-4 overflow-y-auto">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-medium text-slate-500 mb-1">Lab Name</label>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-medium text-slate-500 mb-1">Request Date</label>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-medium text-slate-500 mb-1">Result Status</label>
                ...
                <div>
                  <label className="block text-xs font-medium text-slate-500 mb-1">Result Value</label>
