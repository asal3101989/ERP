...
export default function ChecklistTemplatePage() {
  ...
  const createMut = useMutation({
    ...
    onSuccess: () => {
      ...
      reset();
      setShowForm(false);
      qc.invalidateQueries(['quality-checklists']);
    },
  });
    ...
    <div className="bg-[#f4f6f9] min-h-full p-6 space-y-5">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-lg font-bold text-slate-800">ITP Template Master</h1>
        ...
        {templates?.map(tpl => (
          <div key={tpl.id} className="bg-white rounded-xl border border-[#e2e6ec] overflow-hidden flex flex-col">
            <div className="bg-[#f8f9fc] border-b border-[#e2e6ec] px-5 py-3 flex items-center justify-between">
              <span className="text-xs font-semibold text-indigo-600 bg-indigo-50 border border-indigo-100 px-2.5 py-0.5 rounded-full uppercase tracking-wide">
                {tpl.category}
              </span>
            ...
            <div className="p-5 flex flex-col flex-1 gap-3">
              ...
              <div className="space-y-2 flex-1">
                ...
                {tpl.items?.length > 3 && (
                  <div className="text-xs text-slate-400 pl-5">+ {tpl.items.length - 3} more checks</div>
                )}
              </div>
            </div>
          </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-xl flex flex-col overflow-hidden max-h-[92vh]">
            ...
            <div className="px-6 py-4 bg-indigo-600 flex items-center justify-between">
              <div className="flex items-center gap-3">
                <ClipboardCheck className="w-5 h-5 text-white" />
                <span className="text-base font-semibold text-white">New ITP Template</span>
              </div>
            ...
            <form onSubmit={handleSubmit(createMut.mutate)} className="p-6 flex flex-col gap-5 overflow-y-auto">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-medium text-slate-500 mb-1">Protocol Name</label>
                  <input
              ...
              <div>
                <div className="flex items-center justify-between mb-3">
                  <span className="text-xs font-semibold text-slate-700">Checklist Items</span>
                  <button
                    type="button"
              ...
              <div className="flex gap-3 pt-1">
                ...
                <button
                  ...
                  className="flex-[2] bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-semibold rounded-lg px-4 py-2 flex items-center justify-center gap-2 transition-colors disabled:opacity-60"
                >
