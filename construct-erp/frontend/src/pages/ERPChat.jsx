...
function Avatar({ name = '', size = 'md', photo }) {
  ...
  return (
    <div className={clsx(cls, 'rounded-full flex items-center justify-center font-bold flex-shrink-0', avatarColor(name))}>
      {getInitials(name)}
    </div>
  );
}
      ...
      <aside className="w-60 flex-shrink-0 bg-white border-r border-slate-100 flex flex-col overflow-hidden">
        <div className="px-4 py-3.5 border-b border-slate-100 flex items-center gap-2.5">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-sm font-bold text-slate-800 truncate">Construct ERP</p>
        ...
        <div className="px-3 py-2.5 border-b border-slate-100">
          <div className="flex items-center gap-2 bg-slate-50 border border-slate-200 rounded-lg px-2.5 py-1.5">
            ...
            <input placeholder="Jump to channel…" className="flex-1 bg-transparent text-xs text-slate-600 placeholder-slate-400 outline-none" />
          </div>
        </div>
        ...
        <nav className="flex-1 overflow-y-auto py-2">
          ...
          <div className="px-3 mb-1">
            <button onClick={() => setCollapsed(p => ({ ...p, channels: !p.channels }))}
              className="flex items-center gap-1 text-[10px] font-semibold text-slate-400 uppercase tracking-wider w-full hover:text-slate-600">
          ...
          {!collapsed.channels && CHANNELS.map(ch => {
            ...
            return (
              <button key={ch.id} onClick={() => setActiveChannel(ch.id)}
                ...
                {badge > 0 && <span className="text-[10px] font-bold bg-indigo-600 text-white px-1.5 py-0.5 rounded-full">{badge}</span>}
              </button>
            );
          })}
            ...
            return (
              <button key={emp.id} onClick={() => setActiveChannel(dmId)}
                ...
                <div className="min-w-0 flex-1">
                  <p className={clsx('text-xs truncate', isActive ? 'font-semibold text-indigo-700' : 'font-medium text-slate-700')}>{name}</p>
                  <p className="text-[10px] text-slate-400 truncate">{emp.designation_name || emp.designation || ''}</p>
                </div>
        ...
        <div className="px-3 py-3 border-t border-slate-100 flex items-center gap-2.5">
          ...
          <div className="min-w-0 flex-1">
            <p className="text-xs font-semibold text-slate-700 truncate">{user?.name || user?.username || 'You'}</p>
        ...
        <div className="bg-white border-b border-slate-100 px-5 py-3 flex items-center gap-3 flex-shrink-0">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-bold text-slate-800">{activeCh.label}</span>
              {pinned.length > 0 && (
            ...
            <p className="text-[11px] text-slate-400 truncate">{activeCh.desc}</p>
          </div>
          <div className="flex items-center gap-1.5">
            <button onClick={() => { setSearchOpen(v => !v); if (pinsOpen) setPinsOpen(false); }}
              className={clsx('p-1.5 rounded-lg transition-all', searchOpen ? 'bg-indigo-50 text-indigo-600' : 'text-slate-400 hover:bg-slate-50')}>
              <Search className="w-4 h-4" />
            </button>
            <button onClick={() => { setPinsOpen(v => !v); if (searchOpen) { setSearchOpen(false); setSearchQuery(''); } }}
              className={clsx('p-1.5 rounded-lg transition-all', pinsOpen ? 'bg-amber-50 text-amber-600' : 'text-slate-400 hover:bg-slate-50')}>
            ...
            <div className="space-y-2">
              {pinned.map(m => (
                <div key={m.id} className="flex items-start gap-2 bg-white border border-amber-200 rounded-xl px-3 py-2">
                  ...
                  <div className="flex-1 min-w-0">
                    <span className="text-xs font-semibold text-slate-700 mr-1.5">{m.sender_name}</span>
        ...
        <div className="px-5 pb-5 flex-shrink-0">
          <div className="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden focus-within:border-indigo-300 focus-within:ring-2 focus-within:ring-indigo-100 transition-all">
            ...
            <textarea ref={textRef} value={inputVal} onChange={handleInput}
              ...
              rows={1}
              className="w-full px-4 py-3 text-sm text-slate-700 placeholder-slate-400 outline-none resize-none bg-transparent leading-relaxed"
              style={{ minHeight: 44, maxHeight: 120 }}
            />
            <div className="flex items-center gap-1 px-3 pb-2.5">
              <button onClick={() => fileRef.current?.click()}
                className="p-1.5 rounded-lg text-slate-400 hover:bg-slate-100 hover:text-slate-600 transition-colors" title="Attach file">
                <Paperclip className="w-4 h-4" />
              </button>
              <button className="p-1.5 rounded-lg text-slate-400 hover:bg-slate-100 hover:text-slate-600 transition-colors" title="Emoji">
                <Smile className="w-4 h-4" />
              </button>
              <button className="p-1.5 rounded-lg text-slate-400 hover:bg-slate-100 hover:text-slate-600 transition-colors" title="Mention">
              ...
              <button onClick={sendMsg} disabled={!inputVal.trim() && pendingFiles.length === 0}
                className={clsx('ml-auto flex items-center gap-1.5 px-4 py-1.5 rounded-xl text-xs font-semibold transition-all',
