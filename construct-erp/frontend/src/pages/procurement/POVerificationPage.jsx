...
export default function POVerificationPage() {
  ...
  const { data: po, isLoading, error } = useQuery({
    ...
    queryFn: () => axios.get(`${BASE_URL}/purchase-orders/public/verify/${id}`).then(r => r.data.data),
  });
  ...
  if (isLoading) return (
    <div className="min-h-screen bg-slate-950 flex flex-col items-center justify-center p-6 bg-[radial-gradient(circle_at_center,_var(--tw-gradient-stops))] from-blue-900/10 via-slate-950 to-slate-950">
      ...
      <p className="text-slate-400 font-black uppercase tracking-widest text-[10px] animate-pulse">Authenticating Document Reference...</p>
    </div>
  );
  ...
  if (error || !po) return (
    <div className="min-h-screen bg-slate-950 flex flex-col items-center justify-center p-6">
      ...
      <h1 className="text-2xl font-black text-slate-100 uppercase tracking-tight">Invalid Document</h1>
      <p className="text-slate-500 mt-2 text-center max-w-xs font-medium">This Purchase Order record could not be verified. It may have been revoked or the reference is incorrect.</p>
      <Link to="/" className="mt-8 px-8 py-3 bg-slate-100 text-slate-900 font-black rounded-xl text-xs uppercase tracking-widest">Back to Secure Portal</Link>
    </div>
  );
    ...
    <div className="min-h-screen bg-slate-950 py-12 px-4 selection:bg-blue-500 selection:text-white">
      <div className="max-w-xl mx-auto">
        ...
        <div className="flex flex-col items-center text-center mb-10">
          ...
          <h1 className="text-3xl font-black text-slate-100 uppercase tracking-tight leading-none mb-2">Certificate of Authenticity</h1>
          <p className="text-emerald-500/80 font-bold uppercase tracking-[0.25em] text-[10px]">Official Digital Purchase Order Record</p>
        </div>
        ...
        <div className="bg-slate-900/50 border border-slate-800 rounded-[2.5rem] overflow-hidden backdrop-blur-md shadow-2xl relative">
          ...
          <div className="p-8 space-y-8">
            ...
            <div className="flex justify-between items-start">
              <div>
                <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1">PO Reference</p>
                <h2 className="text-2xl font-black text-slate-100 font-mono tracking-tighter uppercase">{po.serial_no_formatted || po.po_number}</h2>
              </div>
            ...
            <div className="grid grid-cols-2 gap-4">
               <div className="p-4 bg-slate-950/50 border border-slate-800 rounded-2xl">
                  ...
                  <p className="text-sm font-black text-slate-100 uppercase leading-tight tracking-tighter">{po.project_name}</p>
               </div>
               <div className="p-4 bg-slate-950/50 border border-slate-800 rounded-2xl">
                  <div className="flex items-center gap-2 mb-2">
                     <Landmark className="w-3 h-3 text-amber-500" />
                     <span className="text-[9px] font-black text-slate-500 uppercase tracking-widest">Vendor Account</span>
                  </div>
                  <p className="text-sm font-black text-slate-100 uppercase leading-tight tracking-tighter">{po.vendor_name}</p>
               </div>
            </div>
               ...
               <div className="space-y-2">
                  {(po.items || []).map((it, i) => (
                    <div key={i} className="flex justify-between items-center py-2 border-b border-slate-800/50 last:border-0 grow">
                       <div>
                          <p className="text-xs font-black text-slate-100 uppercase tracking-tight leading-none">{it.material_name}</p>
                          <p className="text-[9px] text-slate-500 font-bold mt-1 uppercase tracking-tighter">Unit: {it.unit} | Net Auth: {parseFloat(it.quantity)}</p>
                       </div>
                    </div>
                  ))}
               </div>
            ...
            <div className="space-y-4">
               ...
               <div className="grid grid-cols-2 gap-3">
                  <div className="p-3 border border-slate-800 rounded-xl bg-slate-950/30 flex items-center justify-between">
                     ...
                     <span className={po.verified_audit_name ? "text-[10px] font-black text-emerald-500 uppercase" : "text-[10px] font-bold text-slate-600 uppercase"}>
                       {po.verified_audit_name ? 'SIGNED' : 'PENDING'}
                     </span>
                  </div>
                  <div className="p-3 border border-slate-800 rounded-xl bg-slate-950/30 flex items-center justify-between">
                     <span className="text-[10px] font-bold text-slate-500 uppercase">Finance</span>
                     <span className={po.checked_finance_name ? "text-[10px] font-black text-emerald-500 uppercase" : "text-[10px] font-bold text-slate-600 uppercase"}>
                       {po.checked_finance_name ? 'SIGNED' : 'PENDING'}
                     </span>
                  </div>
                  <div className="p-3 border border-slate-800 rounded-xl bg-slate-950/30 flex items-center justify-between">
                     <span className="text-[10px] font-bold text-slate-500 uppercase">Mgmt</span>
                     <span className={po.released_mgmt_name ? "text-[10px] font-black text-emerald-500 uppercase" : "text-[10px] font-bold text-slate-600 uppercase"}>
                       {po.released_mgmt_name ? 'SIGNED' : 'PENDING'}
                     </span>
                  </div>
                  <div className="p-3 border border-slate-800 rounded-xl bg-slate-950/30 flex items-center justify-between">
                     <span className="text-[10px] font-bold text-slate-500 uppercase">Director</span>
                     <span className={po.authorized_md_name ? "text-[10px] font-black text-emerald-500 uppercase" : "text-[10px] font-bold text-slate-600 uppercase"}>
        ...
        <div className="mt-10 text-center">
           <p className="text-slate-600 text-[10px] font-bold uppercase tracking-widest">© 2026 BCIM Construction Group • Secure Document Portal</p>
        </div>
