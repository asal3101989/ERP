...
export default function BidOpportunityDetailPage() {
  ...
  if (!data) return <div className="text-slate-400 py-12 text-center">Opportunity not found</div>;
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-center gap-3">
        <button onClick={() => navigate(-1)} className="p-2 text-slate-400 hover:text-white"><ArrowLeft className="w-5 h-5" /></button>
        <div className="flex-1">
          <h1 className="text-xl font-bold text-white">{opp.project_name}</h1>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
          <div className="bg-slate-800 border border-slate-700 rounded-xl p-5 space-y-3">
            <div className="flex items-center justify-between">
              <h3 className="text-sm font-semibold text-slate-300 uppercase tracking-wide">Opportunity Details</h3>
              <button onClick={() => setEditForm({ ...opp })} className="text-xs text-slate-400 hover:text-white">Edit</button>
            </div>
            {[
          ...
          <div className="bg-slate-800 border border-slate-700 rounded-xl p-5 space-y-4">
            <h3 className="text-sm font-semibold text-slate-300 uppercase tracking-wide">Win Probability</h3>
            <div className="flex items-end gap-3">
              <span className={`text-5xl font-black ${probColor(opp.win_probability)}`}>{opp.win_probability}%</span>
            </div>
            ...
            <p className="text-sm text-slate-400 mt-2 whitespace-pre-wrap">{opp.scope_summary || 'No scope summary'}</p>
            {opp.lost_reason && (
            ...
            {opp.dropped_reason && (
              <div className="p-3 bg-slate-700/50 border border-slate-600 rounded-lg">
                <p className="text-xs text-slate-400 font-medium">Dropped Reason</p>
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            ...
            <div className="flex gap-2">
              {costEdit ? (
                <>
                  <button onClick={() => setCostEdit(null)} className="px-3 py-1.5 border border-slate-600 text-slate-400 rounded-lg text-sm">Discard</button>
              ...
              <tbody>
                    ...
                    {costEdit ? (
                      <>
                        ...
                        <td className="px-3 py-2"><input type="number" value={item.rate} onChange={e => { const a = [...costEdit]; a[idx].rate = e.target.value; a[idx].amount = ((parseFloat(a[idx].quantity)||0) * (parseFloat(e.target.value)||0)).toFixed(2); setCostEdit(a); }}
                          className="w-24 px-2 py-1 bg-slate-700 border border-slate-600 rounded text-white text-xs focus:outline-none" /></td>
                        <td className="px-3 py-2 text-slate-300 text-xs">{item.amount ? `₹${parseFloat(item.amount).toLocaleString('en-IN')}` : '—'}</td>
                        ...
                        <td className="px-3 py-2">
                          <button onClick={() => { const a = [...costEdit]; a.splice(idx, 1); setCostEdit(a); }} className="text-slate-600 hover:text-red-400"><Trash2 className="w-3.5 h-3.5" /></button>
                        </td>
                      </>
                    ) : (
                      <>
                        <td className="px-3 py-2.5 text-slate-400 text-xs">{item.category}</td>
                        <td className="px-3 py-2.5 text-slate-300">{item.description}</td>
                        <td className="px-3 py-2.5 text-slate-400 text-xs">{item.quantity}</td>
                        <td className="px-3 py-2.5 text-slate-400 text-xs">{item.unit}</td>
                        ...
                        <td className="px-3 py-2.5 text-slate-400 text-xs">{item.margin_pct}%</td>
                        <td />
                      </>
                    )}
                  </tr>
                ...
                {costItems.length > 0 && (
                  <tr className="border-t border-slate-600 bg-slate-750">
                    <td colSpan={5} className="px-3 py-2.5 text-slate-400 font-semibold text-sm">Total</td>
        ...
        <div className="space-y-4">
          <div>
            ...
            <input type="file" multiple
              ...
              className="block text-sm text-slate-400 file:mr-3 file:py-2 file:px-4 file:rounded-lg file:border-0 file:bg-emerald-600 file:text-white file:text-sm file:cursor-pointer hover:file:bg-emerald-500"
            />
          </div>
          <div className="bg-slate-800 border border-slate-700 rounded-xl overflow-hidden">
                  ...
                  <tbody>
                    {opp.documents.map(d => (
                      <tr key={d.id} className="border-b border-slate-700/50">
                        ...
                        <td className="px-4 py-3 text-slate-500 text-xs">{d.doc_type}</td>
                        <td className="px-4 py-3 text-slate-400">{d.uploaded_by_name || '—'}</td>
                        <td className="px-4 py-3 text-slate-500 text-xs">{dayjs(d.uploaded_at).format('DD MMM YY HH:mm')}</td>
                        <td className="px-4 py-3">
                          <button onClick={async () => { await bidOpportunityAPI.removeDoc(id, d.id); qInvalidate(); toast.success('Deleted'); }}
                            className="text-slate-600 hover:text-red-400"><Trash2 className="w-4 h-4" /></button>
                        </td>
                      </tr>
                    ))}
                  </tbody>
                    ...
                    <div key={log.id} className="flex gap-4 relative">
                      ...
                      <div className="flex-1 pb-1">
                        <div className="flex items-center gap-2">
                          ...
                          {!log.to_status && <span className="text-xs text-slate-400 capitalize">{log.action}</span>}
                          {log.from_status && (
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60">
          <div className="bg-slate-800 border border-slate-700 rounded-2xl p-6 w-full max-w-md">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-lg font-bold text-white capitalize">Move to: {transStatus}</h2>
              <button onClick={() => setShowTransition(false)} className="text-slate-400 hover:text-white"><X className="w-5 h-5" /></button>
            </div>
            <div className="space-y-3">
              ...
              {(transStatus === 'lost' || transStatus === 'dropped') && (
                <div>
                  <label className="block text-xs text-slate-400 mb-1">{transStatus === 'lost' ? 'Lost Reason' : 'Drop Reason'}</label>
            ...
            <div className="flex gap-3 mt-4">
              <button onClick={() => setShowTransition(false)} className="flex-1 py-2 border border-slate-600 text-slate-400 rounded-lg text-sm">Cancel</button>
              <button
