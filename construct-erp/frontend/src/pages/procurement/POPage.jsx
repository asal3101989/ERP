...
function NewPOModal({ onClose, vendors, projects, onCreate, isPending, prefill }) {
  ...
  return (
    <div className="fixed inset-0 z-[60] bg-[#f4f6f9]">
      <div className="bg-[#f0f2f5] w-full h-full flex flex-col shadow-sm overflow-hidden border border-slate-200">
        ...
        <div className="flex items-center justify-between px-6 py-3.5 bg-white border-b border-slate-200 flex-shrink-0">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 bg-indigo-50 border border-indigo-100 rounded-xl flex items-center justify-center">
              <span className="text-indigo-600 font-black text-sm">B</span>
            </div>
            <div>
              <p className="text-sm font-black text-slate-900">
                {poType === 'Amendment' ? 'Amended Purchase Order' : 'Purchase Order'}
              </p>
              <p className="text-[11px] text-slate-500">BCIM-PUR-F-03 · 4-stage authorization workflow</p>
            </div>
          </div>
          <div className="flex items-center gap-3">
            {grandTotal > 0 && (
              <span className="font-mono text-sm text-indigo-700 bg-indigo-50 border border-indigo-100 px-3 py-1 rounded-xl">
            ...
            <span className={clsx('text-xs font-semibold px-2.5 py-1 rounded-full',
              poType === 'Amendment' ? 'bg-orange-500 text-white' : 'bg-indigo-50 text-indigo-700 border border-indigo-100')}>
              {poType}
            </span>
            <button onClick={onClose} className="w-9 h-9 rounded-xl border border-slate-200 text-slate-400 hover:text-slate-700 hover:bg-slate-50 flex items-center justify-center transition-all">
              <X className="w-4 h-4" />
            </button>
          </div>
        </div>
...
function POQtySection({ poId }) {
  ...
  const isBillFallback = source === 'bill_fallback';
  const hasExceeded = !isFallback && !isBillFallback && items.some(it =>
  ...
  return (
    <div className="bg-white border border-slate-200 rounded-xl overflow-hidden">
      <div className={`px-4 py-3 flex items-center justify-between ${hasExceeded ? 'bg-gradient-to-r from-red-600 to-red-700' : 'bg-gradient-to-r from-teal-600 to-teal-700'}`}>
        <span className="text-xs font-bold text-white uppercase tracking-wider">Quantity Tracking</span>
        {hasExceeded
          ? <span className="text-[11px] text-red-200 font-semibold">⚠ Quantity Exceeded</span>
          : <span className="text-[11px] text-teal-200">All quantities within limit</span>}
      </div>
      ...
      {isLoading ? (
        ...
        <div className="p-5 text-center">
          <p className="text-xs text-slate-400 font-medium">No item records found for this PO</p>
          <p className="text-[10px] text-slate-300 mt-1">This PO was created before item-level tracking.<br/>Check the Bills section below for invoice totals.</p>
        </div>
      ) : isBillFallback ? (
        /* ── Bill-only fallback: show invoice amounts as rows ── */
        <div className="divide-y divide-slate-50">
          <div className="px-4 py-2 bg-amber-50 border-b border-amber-100">
            <p className="text-[10px] text-amber-700">⚠ Legacy PO — showing billed invoice amounts. GRN quantities not linked to this PO.</p>
          </div>
          <div className="grid grid-cols-3 px-4 py-2 bg-slate-50 text-[9px] font-bold text-slate-400 uppercase tracking-wide">
            ...
            <div key={i} className="px-4 py-3">
              <div className="grid grid-cols-3 items-center gap-1">
                ...
                <div className="text-right">
                  <p className="text-xs font-bold text-indigo-600">{inr(it.invoiced_qty)}</p>
                </div>
              </div>
            </div>
          ))}
        ...
        <div className="divide-y divide-slate-50">
          ...
          {items.map((it, i) => {
            ...
            return (
              <div key={i} className={`px-4 py-3 ${anyExceed ? 'bg-red-50' : ''}`}>
              ...
              {editingNum ? (
                ...
                <div className="flex items-center gap-1.5">
                  ...
                  <button
                    ...
                    className="w-5 h-5 rounded flex items-center justify-center text-slate-400 hover:text-indigo-500 hover:bg-indigo-50 transition"
                  >
