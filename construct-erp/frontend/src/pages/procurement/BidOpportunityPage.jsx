...
const STATUS_COLORS = {
  ...
  dropped:      'bg-slate-700 text-slate-500 border border-slate-600',
};
const SECTOR_LABELS = {
...
const fmt = (n) => n != null ? `₹${(parseFloat(n)/1e7).toFixed(2)} Cr` : '—';
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-white">Bid Opportunities</h1>
      ...
      <div className="grid grid-cols-3 md:grid-cols-6 gap-3">
        {kpis.map(k => (
          <button key={k.key}
            ...
            <div className={`text-2xl font-bold ${k.color}`}>{stats[k.key] ?? 0}</div>
      ...
      <div className="grid grid-cols-3 gap-3">
        <div className="bg-slate-800 border border-slate-700 rounded-xl p-4 col-span-1 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-2xl font-bold text-emerald-400">{stats.win_rate_pct ?? 0}%</p>
          </div>
        </div>
        ...
        <div className="bg-slate-800 border border-slate-700 rounded-xl p-4 flex items-center justify-between">
          <span className="text-slate-400 text-sm">Won Value</span>
            ...
            <tbody>
              {opps.map(o => (
                <tr key={o.id}
                  ...
                  <td className="px-4 py-3 text-slate-400 text-xs">{SECTOR_LABELS[o.sector] || o.sector}</td>
                  <td className="px-4 py-3 text-slate-300">{fmtL(o.bid_value)}</td>
                  <td className="px-4 py-3">
                    <span className={`text-sm font-bold ${probColor(o.win_probability)}`}>{o.win_probability}%</span>
                  </td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60">
          <div className="bg-slate-800 border border-slate-700 rounded-2xl p-6 w-full max-w-md shadow-xl">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-lg font-bold text-white">New Bid Opportunity</h2>
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs text-slate-400 mb-1">Bid Value (₹)</label>
              ...
              <div>
                <label className="block text-xs text-slate-400 mb-1">Bid Submission Date</label>
            ...
            <div className="flex gap-3 mt-5">
              <button onClick={() => setShowNew(false)} className="flex-1 py-2 border border-slate-600 text-slate-400 rounded-lg text-sm hover:text-white">Cancel</button>
              <button
