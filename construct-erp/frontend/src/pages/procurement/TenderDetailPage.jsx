...
export default function TenderDetailPage() {
  ...
  if (!data) return <div className="text-slate-400 py-12 text-center">Tender not found</div>;
  ...
  return (
    <div className="space-y-5">
      ...
      {tab === 'overview' && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
          <div className="bg-slate-800 border border-slate-700 rounded-xl p-5 space-y-3">
            <h3 className="text-sm font-semibold text-slate-300 uppercase tracking-wide">Tender Details</h3>
            {[
          ...
          <div className="bg-slate-800 border border-slate-700 rounded-xl p-5 space-y-4">
            <h3 className="text-sm font-semibold text-slate-300 uppercase tracking-wide">Scope of Work</h3>
            ...
            {tender.awarded_vendor_name && (
              <div className="mt-4 p-3 bg-emerald-900/20 border border-emerald-700 rounded-lg">
                ...
                <p className="text-white font-semibold mt-1">{tender.awarded_vendor_name}</p>
            ...
            <tbody>
              {(tender.scope_items || []).map((item, idx) => (
                <tr key={item.id} className="border-b border-slate-700/50">
                  ...
                  <td className="px-4 py-3 text-slate-400 font-mono text-xs">{item.item_code || '—'}</td>
      ...
      {tab === 'vendors' && (
        <div className="bg-slate-800 border border-slate-700 rounded-xl overflow-hidden">
          <div className="p-4 border-b border-slate-700 flex items-center justify-between">
            <h3 className="text-sm font-semibold text-white">Invited Vendors ({(tender.invited_vendors || []).length})</h3>
          </div>
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-slate-700">
          ...
          {compData?.l1_analysis?.length > 0 && (
            <div className="bg-slate-800 border border-slate-700 rounded-xl overflow-hidden">
              <div className="p-3 border-b border-slate-700 bg-slate-750">
                <h4 className="text-sm font-semibold text-slate-300">L1 Analysis</h4>
              </div>
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-slate-700">
                    {['Rank', 'Vendor', 'Final Amount', 'Premium over L1', 'Tech Score', 'Status'].map(h => (
                      <th key={h} className="text-left px-4 py-2.5 text-slate-400 text-xs font-medium">{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {compData.l1_analysis.map(b => (
                    <tr key={b.bid_id} className={`border-b border-slate-700/50 ${b.is_l1 ? 'bg-emerald-900/10' : ''}`}>
                      <td className="px-4 py-2.5">
                        <span className={`text-xs font-bold ${b.is_l1 ? 'text-emerald-400' : 'text-slate-500'}`}>L{b.rank}</span>
                      </td>
          ...
          {compData?.scope_items?.length > 0 && compData?.bids?.length > 0 && (
            <div className="bg-slate-800 border border-slate-700 rounded-xl overflow-x-auto">
              <div className="p-3 border-b border-slate-700">
                <h4 className="text-sm font-semibold text-slate-300">Rate Comparison Matrix</h4>
              </div>
              <table className="text-xs whitespace-nowrap">
                <thead>
                  <tr className="border-b border-slate-700">
                    <th className="text-left px-4 py-2.5 text-slate-400">Item</th>
                    <th className="px-4 py-2.5 text-slate-400 text-right">Unit</th>
                    <th className="px-4 py-2.5 text-slate-400 text-right">Qty</th>
                    {compData.bids.map(b => (
                ...
                <tbody>
                  {compData.scope_items.map(si => {
                    ...
                    return (
                      <tr key={si.id} className="border-b border-slate-700/50">
                        ...
                        <td className="px-4 py-2 text-slate-500 text-right">{si.unit}</td>
                        <td className="px-4 py-2 text-slate-500 text-right">{si.quantity}</td>
                        {compData.bids.map(b => {
                          const cell = compData.matrix?.[si.id]?.[b.id];
                          const rate = parseFloat(cell?.unit_rate || 0);
                          const isL1 = rate > 0 && rate === minRate;
                          return (
                            <td key={b.id} className={`px-4 py-2 text-right ${isL1 ? 'text-emerald-400 font-semibold bg-emerald-900/10' : 'text-slate-300'}`}>
                  ...
                  <tr className="border-t border-slate-600 bg-slate-750">
                    <td colSpan={3} className="px-4 py-2 text-slate-400 font-semibold">Total</td>
                    {compData.bids.map(b => (
                      <td key={b.id} className={`px-4 py-2 text-right font-bold ${b.is_winner ? 'text-emerald-400' : 'text-white'}`}>
        ...
        <div className="space-y-4">
          <div>
            ...
            <input type="file" multiple
              ...
              className="block text-sm text-slate-400 file:mr-3 file:py-2 file:px-4 file:rounded-lg file:border-0 file:bg-violet-600 file:text-white file:text-sm file:cursor-pointer hover:file:bg-violet-500"
            />
          </div>
          <div className="bg-slate-800 border border-slate-700 rounded-xl overflow-hidden">
      ...
      {showCancelModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60">
          <div className="bg-slate-800 border border-slate-700 rounded-2xl p-6 w-full max-w-md">
            <h2 className="text-lg font-bold text-white mb-4">Cancel Tender</h2>
            <textarea
            ...
            <div className="flex gap-3 mt-4">
              <button onClick={() => setShowCancelModal(false)} className="flex-1 py-2 border border-slate-600 text-slate-400 rounded-lg text-sm">Back</button>
              <button
      ...
      {showAwardModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60">
          <div className="bg-slate-800 border border-slate-700 rounded-2xl p-6 w-full max-w-md">
            <h2 className="text-lg font-bold text-white mb-4">Award Tender</h2>
            <p className="text-sm text-slate-400 mb-4">Select the winning bid. A Purchase Order will be auto-created.</p>
            <select
            ...
            <div className="flex gap-3">
              <button onClick={() => setShowAwardModal(false)} className="flex-1 py-2 border border-slate-600 text-slate-400 rounded-lg text-sm">Cancel</button>
              <button
