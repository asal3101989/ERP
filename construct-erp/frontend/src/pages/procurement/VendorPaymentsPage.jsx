    ...
    <div className="p-6 md:p-7 max-w-7xl mx-auto min-h-screen bg-[#f4f6f9]">
      <div className="flex flex-col xl:flex-row xl:items-end justify-between gap-4 mb-5">
        <div>
          ...
          <h1 className="text-2xl md:text-[28px] font-bold text-slate-900 leading-tight">Vendor Payments</h1>
        ...
        <div className="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden">
          <div className="px-5 py-4 border-b border-slate-100 flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-slate-400 mt-0.5">Invoices with live paid and balance calculations</p>
            </div>
          ...
          {loading ? (
            ...
            <div className="p-14 text-center">
              ...
              <p className="text-xs text-slate-400 mt-1">Create invoices in the vendor billing module and record payments here.</p>
            </div>
          ) : (
              ...
              <table className="min-w-full">
                <thead className="bg-slate-50 border-b border-slate-100">
                  <tr className="text-[11px] uppercase tracking-[0.18em] text-slate-400">
                    ...
                    <th className="text-left font-semibold px-4 py-3">Invoice Amt</th>
                    <th className="text-left font-semibold px-4 py-3">Paid / Balance</th>
                  ...
                  {filtered.map(inv => (
                    <tr key={inv.id} className="hover:bg-slate-50/70 transition-colors">
                      <td className="px-4 py-3 align-top">
                        <div className="text-sm font-semibold text-slate-900 leading-tight">{inv.invoice_number || inv.invNo || '—'}</div>
                        <div className="text-[11px] text-slate-400 mt-0.5 truncate max-w-[200px]">{inv.po_number || inv.poRef || 'No PO reference'}</div>
                      </td>
                      <td className="px-4 py-3 align-top">
                        <span className={clsx(
                      ...
                      <td className="px-4 py-3 align-top">
                        ...
                        <div className="text-[11px] text-slate-400 truncate max-w-[170px]">{clean(inv.vendor_name) || 'Vendor'}</div>
                      </td>
                      <td className="px-4 py-3 align-top">
                        <div className="text-sm text-slate-700 truncate max-w-[190px]">{inv.project_name}</div>
                      </td>
                      <td className="px-4 py-3 align-top">
                        <div className="text-sm font-bold text-indigo-600">{money(inv.invoice_total)}</div>
                        <div className="text-[11px] text-slate-400">Invoice value</div>
                      </td>
                      <td className="px-4 py-3 align-top">
                        <div className="text-sm font-semibold text-emerald-700">{money(inv.paid_amount)}</div>
                ...
                paymentRows.map(row => (
                  <div key={row.id} className="rounded-2xl border border-slate-200 bg-slate-50 p-3">
                    <div className="flex items-center justify-between gap-3">
                      <div>
                        <div className="text-xs text-slate-400 uppercase tracking-[0.18em] font-semibold">{row.reference_number || row.payment_mode || 'Payment'}</div>
                        <div className="text-sm font-semibold text-slate-900 mt-0.5">{row.entity_name || 'Vendor'}</div>
                      </div>
                      <div className="text-sm font-bold text-indigo-600">{money(row.amount || row.net_amount)}</div>
                    </div>
        ...
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl">
            <div className="p-5 border-b border-slate-100 flex items-start justify-between gap-3">
              <div>
                <h2 className="text-base font-semibold text-slate-900">Record Vendor Payment</h2>
                <p className="text-xs text-slate-400 mt-0.5">{selectedInvoice.invoice_number || selectedInvoice.invNo || 'Invoice'} · {selectedInvoice.vendor_name}</p>
              </div>
              <button onClick={() => setSelectedInvoice(null)} className="w-8 h-8 flex items-center justify-center rounded-lg border border-slate-200 text-slate-400 hover:text-slate-600 transition-all">
                ×
              </button>
            </div>
            ...
            <div className="p-5 grid grid-cols-2 gap-4">
              ...
              <div>
                <label className="block text-xs text-slate-500 mb-1">Reference / Cheque No. *</label>
                <input
                  value={payForm.ref}
