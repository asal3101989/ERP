    ...
    return (
      <div className="p-8 max-w-7xl mx-auto min-h-screen bg-[#f4f6f9] flex items-center justify-center">
        <div className="bg-white border border-red-100 rounded-2xl p-10 text-center shadow-sm max-w-sm">
          ...
          <h2 className="text-lg font-bold text-slate-800 mb-1">Indent Not Found</h2>
    ...
    return (
      <div className="p-8 max-w-7xl mx-auto min-h-screen bg-[#f4f6f9] flex items-center justify-center">
        <div className="bg-white border border-emerald-100 rounded-2xl p-10 text-center shadow-sm max-w-sm">
          ...
          <h2 className="text-lg font-bold text-slate-800 mb-1">Quotation Submitted</h2>
          <p className="text-sm text-slate-400 mb-5">
    ...
    <div className="p-6 md:p-8 max-w-7xl mx-auto min-h-screen bg-[#f4f6f9]">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
        <div className="flex items-center gap-3">
          <button
            onClick={() => navigate(-1)}
            className="w-9 h-9 flex items-center justify-center bg-white border border-slate-200 rounded-lg text-slate-500 hover:text-indigo-600 hover:border-indigo-300 transition-all shadow-sm"
          >
          ...
          <div>
            <div className="flex items-center gap-1.5 text-xs text-slate-400 mb-0.5">
              ...
              <ChevronRight className="w-3 h-3" />
              <span className="text-slate-600 font-medium">Enter Vendor Quotes</span>
            </div>
            <h1 className="text-xl font-bold text-slate-900">Register Vendor Quotation</h1>
          </div>
        </div>
        ...
        <button
          ...
          className="flex items-center gap-2 bg-indigo-600 text-white px-5 py-2.5 rounded-xl text-sm font-semibold shadow-sm hover:bg-indigo-700 transition-all disabled:opacity-50"
        >
      ...
      <div className="bg-white border border-slate-200 rounded-xl p-4 mb-5 shadow-sm">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div>
            <p className="text-[11px] text-slate-400 font-medium uppercase tracking-wide mb-0.5">Indent No.</p>
            <p className="text-sm font-bold text-indigo-600">{indent.serial_no_formatted || indent.mrs_number || '—'}</p>
          </div>
          <div>
            <p className="text-[11px] text-slate-400 font-medium uppercase tracking-wide mb-0.5">Project</p>
            <p className="text-sm font-semibold text-slate-800">{indent.project_name || '—'}</p>
          </div>
          <div>
            <p className="text-[11px] text-slate-400 font-medium uppercase tracking-wide mb-0.5">Required By</p>
            <p className="text-sm text-slate-700">
              {indent.required_by ? dayjs(indent.required_by).format('D MMM YYYY') : '—'}
            </p>
          </div>
          <div>
            <p className="text-[11px] text-slate-400 font-medium uppercase tracking-wide mb-0.5">Priority</p>
        ...
        <div className="md:col-span-1 space-y-4">
          ...
          <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm space-y-4">
            ...
            <div className="space-y-1.5">
              <label className="text-xs font-semibold text-slate-500">Select Vendor *</label>
              <select
          ...
          <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm">
            <div className="flex items-center gap-2 pb-3 border-b border-slate-100 mb-4">
              ...
              <h3 className="text-sm font-bold text-slate-700">Quote Summary</h3>
            </div>
            ...
            <div className="space-y-2.5">
              ...
              <div className="pt-3 border-t border-slate-100 flex justify-between items-center">
                ...
                <span className="text-lg font-bold font-mono text-emerald-600">₹{inr(grandTotal)}</span>
              </div>
            </div>
          ...
          <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm">
            <div className="flex items-center justify-between pb-3 border-b border-slate-100 mb-4">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <h3 className="text-sm font-bold text-slate-700">Material Itemized Rates</h3>
                  <p className="text-[11px] text-slate-400 mt-0.5">Enter unit rate for each material item</p>
                </div>
              </div>
              <span className="px-2.5 py-1 bg-indigo-50 border border-indigo-100 rounded-full text-xs font-semibold text-indigo-600">
            ...
            {itemRates.length === 0 ? (
              ...
              <div className="space-y-3">
                {/* Column headers */}
                <div className="grid grid-cols-12 gap-2 px-1 text-[11px] font-bold text-slate-400 uppercase tracking-wide">
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="col-span-4 flex items-start gap-2">
                        <div className="w-5 h-5 rounded-full bg-indigo-100 text-indigo-600 text-[10px] font-bold flex items-center justify-center shrink-0 mt-0.5">
                          {idx + 1}
                        </div>
                        <div>
                          <p className="text-xs font-semibold text-slate-800 leading-tight">{it.material_name}</p>
                          {it.material_category && (
                            <p className="text-[10px] text-slate-400 mt-0.5">{it.material_category}</p>
                          )}
                      ...
                      <div className="col-span-1 flex items-center justify-center">
                        <span className="text-sm font-semibold text-slate-700">{it.quantity}</span>
                      </div>
                      ...
                      <div className="col-span-1">
                        <select
                          className="w-full bg-white border border-slate-200 rounded-lg px-1.5 py-2 text-xs text-slate-700 outline-none focus:border-indigo-400 transition-all"
                          value={it.gst_rate}
                      ...
                      <div className="col-span-2 flex items-center justify-end">
                        {hasRate ? (
                          <div className="text-right">
                            <p className="text-sm font-bold font-mono text-slate-800">₹{inr(total)}</p>
                            {gst > 0 && (
                      ...
                      <div className="col-span-2">
                        <input
                          className="w-full bg-white border border-slate-200 rounded-lg px-2 py-2 text-xs text-slate-700 outline-none focus:border-indigo-400 transition-all"
            ...
            <div className="mt-5 pt-5 border-t border-slate-100 space-y-1.5">
              ...
              <textarea
                className="w-full bg-slate-50 border border-slate-200 rounded-lg px-3 py-2.5 text-sm text-slate-700 outline-none focus:border-indigo-400 transition-all resize-none"
                rows={3}
            ...
            <div className="mt-5 pt-4 border-t border-slate-100 flex items-center justify-between">
              ...
              <button
                onClick={handleSubmit}
                disabled={createMutation.isPending}
                className="flex items-center gap-2 bg-indigo-600 text-white px-6 py-2.5 rounded-xl text-sm font-semibold shadow-sm hover:bg-indigo-700 transition-all disabled:opacity-50"
              >
                <Save className="w-4 h-4" />
                {createMutation.isPending ? 'Submitting…' : 'Submit Quotation'}
              </button>
