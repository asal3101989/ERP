...
const exportCSV = (rows) => {
  const headers = ['#', 'Material', 'Category', 'Unit', 'Rate (₹)', 'Stock Qty', 'Stock Value (₹)', 'Source', 'Status', 'Project', 'Vendor', 'Doc Ref'];
  const csvRows = [
    headers.join(','),
    ...rows.map((r, i) => {
...
const CatCard = ({ cat, onClick, active }) => (
  ...
  >
    <p className="text-xs font-semibold truncate max-w-[120px]">{cat.category}</p>
  ...
  return (
    <div className="mt-4 border-t border-gray-100 pt-4">
      ...
      <input
        ...
        className="w-full border border-gray-200 rounded px-2 py-1.5 text-xs text-gray-600 focus:outline-none focus:ring-1 focus:ring-blue-400"
      />
    </div>
  );
};
    ...
    <div className="p-4 overflow-y-auto h-full">
      <div className="flex items-start justify-between mb-3">
        <div className="min-w-0 flex-1">
          <h3 className="font-bold text-gray-800 text-sm leading-snug pr-2">{row.material_name}</h3>
      ...
      <div className="bg-blue-50 rounded-lg px-4 py-3 mb-3">
        ...
        <p className="text-2xl font-bold text-blue-800">₹{fmt(row.rate)}</p>
      ...
      <div className="grid grid-cols-2 gap-2 mb-3">
        <div className="bg-gray-50 rounded-lg px-3 py-2">
          ...
          <p className="text-sm font-bold text-gray-700">{fmt(row.opening_stock, 0)}</p>
          <p className="text-xs text-gray-400">{row.unit}</p>
        </div>
        <div className="bg-gray-50 rounded-lg px-3 py-2">
          <p className="text-xs text-gray-500">Closing Stock</p>
          <p className="text-sm font-bold text-gray-700">{fmt(row.stock_qty, 0)}</p>
          <p className="text-xs text-gray-400">{row.unit}</p>
        </div>
      </div>
      ...
      <div className="bg-emerald-50 rounded-lg px-3 py-2 mb-3">
        ...
        <p className="text-base font-bold text-emerald-800">₹{fmt(stockValue)}</p>
          ...
          <table className="w-full text-sm">
            <thead className="sticky top-0 z-10 bg-gray-100">
              <tr>
                ...
                <th className="px-3 py-2 text-right text-xs font-semibold text-gray-500 hidden lg:table-cell">Value (₹)</th>
                    ...
                    >
                      ...
                      <td className="px-3 py-2">
                        <div className="flex items-center gap-1.5">
                          <span className={`text-xs px-1.5 py-0.5 rounded font-semibold ${sourceBadge(row.source_type)}`}>
                            {sourceLabel(row.source_type)}
                          </span>
                      ...
                      <td className="px-3 py-2 text-xs text-gray-500 hidden md:table-cell truncate max-w-[100px]">
                        {row.category}
                      </td>
                      ...
                      <td className="px-3 py-2 text-right text-xs text-gray-600 hidden lg:table-cell">
            ...
            {filteredRates.length > 0 && (
              <tfoot className="sticky bottom-0 bg-gray-100 border-t-2 border-gray-300">
                <tr>
                  <td colSpan={6} className="px-3 py-2 text-xs font-bold text-gray-600">
                    Total ({filteredRates.length} items)
                  </td>
                  <td className="px-3 py-2 text-right text-xs font-bold text-gray-800 hidden lg:table-cell">
