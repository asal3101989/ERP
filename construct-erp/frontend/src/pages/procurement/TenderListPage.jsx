    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-white">Procurement Tenders</h1>
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="bg-slate-800 border border-slate-700 rounded-xl p-4 flex items-center justify-between">
          <span className="text-slate-400 text-sm">Total Estimated Value</span>
          <span className="text-white font-bold">{fmt(stats.total_estimated)}</span>
        </div>
        <div className="bg-slate-800 border border-slate-700 rounded-xl p-4 flex items-center justify-between">
          <span className="text-slate-400 text-sm">Total Awarded Value</span>
            ...
            <tbody>
              {tenders.map(t => (
                <tr key={t.id}
                  ...
                  <td className="px-4 py-3 text-slate-400 text-xs">{t.project_name || '—'}</td>
                  <td className="px-4 py-3 text-slate-400 text-xs">{TYPE_LABELS[t.tender_type] || t.tender_type}</td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60">
          <div className="bg-slate-800 border border-slate-700 rounded-2xl p-6 w-full max-w-md shadow-xl">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-lg font-bold text-white">New Tender</h2>
              <button onClick={() => setShowNew(false)} className="text-slate-400 hover:text-white"><X className="w-5 h-5" /></button>
            </div>
            <div className="space-y-4">
              <div>
                <label className="block text-xs text-slate-400 mb-1">Tender Title *</label>
                <input
              ...
              <div>
                <label className="block text-xs text-slate-400 mb-1">Tender Type</label>
                <select
              ...
              <div>
                <label className="block text-xs text-slate-400 mb-1">Bid Close Date</label>
                <input
