...
function EditCell({ value, onSave, prefix, numeric, placeholder }) {
  ...
  if (editing) return (
    <div className="flex items-center gap-1">
      ...
      <button onClick={() => setEditing(false)} className="text-slate-400"><X size={12} /></button>
    </div>
  );
  return (
...
function ImportModal({ projects, onClose, onDone }) {
  ...
  const handleFile = async (e) => {
    ...
    try {
      ...
      setPreview(res.data.data || []); setStep(2);
      ...
      <div className="relative bg-white shadow-sm w-full h-full flex flex-col overflow-hidden">
        ...
        <div className="px-6 py-3 bg-slate-50 border-b border-slate-200 flex items-center gap-2 flex-shrink-0">
          {[['1','Upload File'],['2','Preview'],['3','Done']].map(([n, label], i) => (
            <React.Fragment key={n}>
              <div className={clsx('flex items-center gap-1.5 text-xs font-semibold', step >= parseInt(n) ? 'text-indigo-700' : 'text-slate-400')}>
                <div className={clsx('w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-black',
                  step > parseInt(n) ? 'bg-emerald-500 text-white' : step === parseInt(n) ? 'bg-indigo-600 text-white' : 'bg-slate-200 text-slate-500'
          ...
          {step === 1 && (
            <div className="p-8 space-y-6">
              <div onClick={() => fileRef.current?.click()}
                ...
                {loading ? (
                  <div className="space-y-3"><Loader2 size={36} className="mx-auto text-indigo-500 animate-spin" /><p className="text-slate-500 font-medium">Parsing file…</p></div>
                ) : (
                  <div className="space-y-3">
                    <Upload size={36} className="mx-auto text-slate-300 group-hover:text-indigo-400 transition-colors" />
                    <div><p className="font-bold text-slate-700">Click to upload your Excel file</p><p className="text-sm text-slate-400 mt-1">Supports .xlsx · .xls · .csv</p></div>
                  </div>
                )}
              </div>
              <div className="bg-amber-50 border border-amber-200 rounded-xl p-4">
                <p className="text-xs font-bold text-amber-800 mb-2 uppercase tracking-wide">Expected columns</p>
          ...
          {step === 2 && (
            <div className="flex flex-col h-full">
              <div className="px-6 py-4 bg-slate-50 border-b border-slate-200 flex flex-wrap items-end gap-4">
                ...
                <div className="ml-auto text-xs text-slate-500">{preview.length} rows ready</div>
              </div>
          ...
          {step === 3 && result && (
            <div className="p-8 text-center space-y-6">
              ...
              <div>
                ...
                <p className="text-slate-500 text-sm mt-1">Stock data has been imported into inventory.</p>
              </div>
        ...
        {step === 2 && (
          <div className="px-6 py-4 border-t border-slate-200 bg-white flex items-center justify-between flex-shrink-0">
            <button onClick={() => { setStep(1); setPreview([]); setFile(null); }} className="text-sm text-slate-500 hover:text-slate-800 font-medium transition">← Back</button>
            <button onClick={handleImport} disabled={!projectId || loading}
              className="flex items-center gap-2 bg-indigo-600 hover:bg-indigo-700 disabled:opacity-50 text-white font-bold px-6 py-2.5 rounded-xl text-sm transition">
          ...
          <div className="bg-slate-50 border border-slate-200 rounded-xl p-4 flex items-center justify-between">
            <div>
              ...
              <div className="text-2xl font-black text-emerald-600 font-mono mt-0.5">
                {qty(item.closing_stock)} <span className="text-sm text-slate-400">{item.unit}</span>
              </div>
            </div>
            {parseFloat(item.unit_rate) > 0 && (
              <div className="text-right">
                <div className="text-xs text-slate-500 font-medium">Unit Rate</div>
                <div className="text-base font-bold text-slate-700 font-mono">₹{inr(item.unit_rate)}</div>
              </div>
            )}
          </div>
          <form onSubmit={handleSubmit(d => issueMutation.mutate({ ...d, inventory_id: item.id, project_id: item.project_id }))} className="space-y-4">
            <div>
              <label className="text-xs font-bold text-slate-600 block mb-1.5">Quantity to Issue *</label>
              <input type="number" step="0.001"
                {...register('quantity', { required: true, max: { value: parseFloat(item.closing_stock), message: `Max: ${item.closing_stock}` } })}
                className="w-full border border-slate-200 rounded-xl px-4 py-2.5 text-base font-mono font-bold focus:outline-none focus:border-emerald-400"
                placeholder={`Max ${qty(item.closing_stock)}`} />
            </div>
            <div>
              <label className="text-xs font-bold text-slate-600 block mb-1.5">Issued To *</label>
            ...
            <div>
              <label className="text-xs font-bold text-slate-600 block mb-1.5">Remarks</label>
            ...
            <div className="flex gap-3 pt-2">
              ...
              <button type="submit" disabled={issueMutation.isPending}
                className="flex-[2] py-2.5 bg-emerald-600 hover:bg-emerald-700 disabled:opacity-60 text-white font-bold rounded-xl text-sm transition flex items-center justify-center gap-2">
                <ArrowUpDown size={14} />
...
export default function InventoryPage() {
  ...
  const { data: batches = [] } = useQuery({
    ...
    queryFn: () => inventoryAPI.getBatches(expandedId).then(r => r.data.data),
    enabled: !!expandedId,
  });
    ...
    <div className="p-6 md:p-8 max-w-full mx-auto min-h-screen bg-[#f4f6f9]">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
        <div>
          ...
          <p className="text-sm text-slate-400 mt-0.5">Material-wise stock position with rate, value & issue tracking</p>
        </div>
        <div className="flex items-center gap-3">
          {(outCount > 0 || lowCount > 0) && (
            <div className="flex items-center gap-2 px-4 py-2.5 bg-amber-50 border border-amber-200 rounded-xl text-sm font-medium text-amber-700">
              ...
              {outCount > 0 && lowCount > 0 && ' · '}
              {lowCount > 0 && `${lowCount} need reorder`}
            </div>
          )}
          <button onClick={() => refetch()}
            className="flex items-center gap-1.5 text-xs font-medium text-slate-500 hover:text-slate-800 border border-slate-200 rounded-lg px-3 py-2 transition bg-white">
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-3 mb-5">
        <div className="bg-white border border-slate-200 rounded-xl p-4 shadow-sm">
          <div className="text-2xl font-bold text-slate-800">{allItems.length}</div>
        ...
        <div className="bg-amber-50 border border-amber-100 rounded-xl p-4 shadow-sm">
          ...
          <div className="text-xs text-amber-500 mt-1">Low / Reorder</div>
        </div>
        <div className="bg-indigo-50 border border-indigo-100 rounded-xl p-4 shadow-sm">
          <div className="text-lg font-bold text-indigo-700 font-mono">₹{inr(totalClosingVal)}</div>
      ...
      <div className="bg-white border border-slate-200 rounded-xl p-3 flex flex-wrap items-center gap-3 shadow-sm mb-4">
        <div className="relative flex-1 min-w-[180px]">
          ...
          <input className="w-full bg-slate-50 border border-slate-200 rounded-lg pl-9 pr-4 py-2 text-sm text-slate-900 placeholder:text-slate-400 outline-none focus:border-indigo-400"
        ...
        <div className="flex items-center gap-1.5">
          {[['all','All'],['out','Out of Stock'],['low','Low/Reorder'],['adequate','Adequate']].map(([v,l]) => (
            <button key={v} onClick={() => setFilterStatus(v)}
              className={clsx('px-3 py-1.5 rounded-lg text-xs font-semibold border transition',
                filterStatus === v ? 'bg-slate-900 text-white border-slate-900' : 'bg-white text-slate-500 border-slate-200 hover:border-slate-400'
              )}>{l}</button>
          ))}
        </div>
      </div>
        ...
        {isLoading ? (
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-slate-800 text-white">
                  <th className="px-3 py-3 text-center text-xs font-bold uppercase w-10">SL</th>
                  <th className="px-3 py-3 text-left   text-xs font-bold uppercase">CATEGORY</th>
                  <th className="px-3 py-3 text-left   text-xs font-bold uppercase">MATERIAL DESCRIPTION</th>
                  <th className="px-3 py-3 text-center text-xs font-bold uppercase w-16">UNIT</th>
                  <th className="px-3 py-3 text-right  text-xs font-bold uppercase">OPENING STOCK</th>
                  <th className="px-3 py-3 text-right  text-xs font-bold uppercase">CLOSING STOCK</th>
                  <th className="px-3 py-3 text-right  text-xs font-bold uppercase">RATE (₹)</th>
                  <th className="px-3 py-3 text-right  text-xs font-bold uppercase bg-rose-700">ISSUED VALUE</th>
                  <th className="px-3 py-3 text-right  text-xs font-bold uppercase bg-slate-700">OPENING VALUE</th>
                  <th className="px-3 py-3 text-right  text-xs font-bold uppercase bg-indigo-700">CLOSING VALUE</th>
                  <th className="px-3 py-3 text-right  text-xs font-bold uppercase bg-amber-700">GST @18%</th>
                  <th className="px-3 py-3 text-right  text-xs font-bold uppercase bg-emerald-700">GRAND TOTAL</th>
                    ...
                    <React.Fragment key={item.id}>
                      <tr className={clsx('border-b border-slate-100 hover:bg-slate-50 transition-colors',
                        ...
                        <td className="px-3 py-3">
                          <div className="font-semibold text-slate-800 text-xs">{item.material_name}</div>
                          <div className="text-[10px] text-slate-400 mt-0.5">{item.project_name}{item.site_location ? ` · ${item.site_location}` : ''}</div>
                        </td>

                        <td className="px-3 py-3 text-center text-xs font-medium text-slate-600 uppercase">{item.unit}</td>

                        <td className="px-3 py-3 text-right font-mono text-xs text-slate-500">{qty(opening)}</td>

                        {/* Closing stock + bar */}
                        <td className="px-3 py-3 text-right">
                          <div className="font-mono text-xs font-bold text-slate-900">{qty(closing)}</div>
                          ...
                          <span className={clsx('text-[10px] font-semibold px-1.5 py-0.5 rounded-full border mt-0.5 inline-block', st.cls)}>
                            {st.label}
                          </span>
                        </td>
                        ...
                        <td className="px-3 py-3 text-right font-mono text-xs text-slate-600 bg-slate-50/30">
                          {openingVal > 0 ? `₹${inr(openingVal)}` : '—'}
                        </td>
                        <td className="px-3 py-3 text-right font-mono text-xs font-bold text-indigo-700 bg-indigo-50/20">
                          {closingVal > 0 ? `₹${inr(closingVal)}` : '—'}
                        </td>
                        <td className="px-3 py-3 text-right font-mono text-xs text-amber-700 bg-amber-50/20">
                          {gst > 0 ? `₹${inr(gst)}` : '—'}
                        </td>
                        <td className="px-3 py-3 text-right font-mono text-xs font-black text-emerald-700 bg-emerald-50/20">
                          ...
                          <div className="flex items-center justify-center gap-1.5">
                            <button onClick={() => setExpandedId(isExpanded ? null : item.id)}
                              ...
                              className={clsx('p-1.5 rounded-lg border text-xs transition',
                                isExpanded ? 'bg-slate-900 text-white border-slate-900' : 'border-slate-200 text-slate-500 hover:border-slate-400 hover:text-slate-800'
                              )}>
                              <Layers size={13} />
                            </button>
                            {closing > 0 && (
                            ...
                            <button onClick={() => { if (window.confirm('Delete this inventory record?')) deleteMutation.mutate(item.id); }}
                              title="Delete"
                              className="p-1.5 rounded-lg border border-slate-200 text-slate-400 hover:border-red-200 hover:text-red-500 transition">
                              <X size={13} />
                            </button>
                          </div>
                        </td>
                      ...
                      {isExpanded && (
                        <tr className="bg-slate-50 border-b border-slate-200">
                          <td colSpan={13} className="px-6 py-4">
                            ...
                            {batches.length === 0 ? (
                              <p className="text-xs text-slate-400 italic">No batch records found.</p>
                            ) : (
                                  ...
                                  return (
                                    <div key={b.id} className={clsx('bg-white border rounded-xl p-4 shadow-sm',
                                      ...
                                      <div className="flex items-center justify-between mb-2">
                                        <span className="text-[10px] font-mono font-bold text-slate-500 uppercase">{b.batch_number}</span>
                                        {i === 0 && <span className="text-[9px] bg-emerald-100 text-emerald-700 font-bold px-2 py-0.5 rounded-full">FIFO #1</span>}
                                      </div>
                                      ...
                                      {b.expiry_date && (
                                        <div className={clsx('text-[10px] font-bold mt-1 flex items-center gap-1',
                                          isExpired ? 'text-red-500' : isNearExp ? 'text-amber-500' : 'text-slate-400'
                                        )}>
