...
import {
  ...
  SkipForward, AlertCircle, FileText, Loader2, Hammer,
} from 'lucide-react';
    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="bg-white border-b border-slate-200 px-6 py-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-lg font-bold text-slate-900">Bulk Import Work Orders</h1>
        ...
        {step === 'select' && (
          <div className="max-w-2xl mx-auto space-y-5">
            ...
            <div
              ...
              <p className="text-sm font-semibold text-slate-700">Click to select WO PDFs  or drag &amp; drop</p>
              <p className="text-xs text-slate-400 mt-1">Filenames must follow WOTQS###-VendorName.pdf</p>
          ...
          <div className="flex gap-4 h-[calc(100vh-180px)]">
              ...
              {queue.map((q, i) => (
                <button key={i} onClick={() => setCurrent(i)}
                  className={clsx('w-full text-left px-3 py-2.5 text-xs flex items-center gap-2 transition-colors',
                    i === current ? 'bg-amber-50 text-amber-700 font-semibold' : 'text-slate-600 hover:bg-slate-50')}>
            ...
            <div className="w-80 flex-shrink-0 bg-white rounded-xl border border-slate-200 flex flex-col overflow-hidden">
              <div className="px-4 py-3 bg-slate-900 flex items-center justify-between flex-shrink-0">
                <div>
                  <p className="text-sm font-bold text-white">{curItem.woNumber}</p>
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-3">
                ...
                <div>
                  <label className="block text-xs font-medium text-slate-500 mb-1">Contractor / Vendor <span className="text-red-500">*</span></label>
                ...
                <div>
                  <label className="block text-xs font-medium text-slate-500 mb-1">Contract Value (₹) <span className="text-red-500">*</span></label>
                ...
                <div>
                  <label className="block text-xs font-medium text-slate-500 mb-1">Subject / Work Description</label>
                  <input value={curItem.form.subject} onChange={e => setField('subject', e.target.value)} className={inputCls} />
                </div>

                <div>
                  <label className="block text-xs font-medium text-slate-500 mb-1">Scope of Work</label>
              ...
              <div className="flex-shrink-0 p-4 border-t border-slate-100 space-y-2">
                <button onClick={handleSaveNext}
                  className="w-full bg-amber-600 hover:bg-amber-700 text-white text-sm font-semibold rounded-lg py-2.5 flex items-center justify-center gap-2">
            ...
            <div className="bg-white rounded-xl border border-slate-200 p-6 space-y-4">
              ...
              <div className="max-h-64 overflow-y-auto border border-slate-100 rounded-lg divide-y divide-slate-50">
                {saved.map((r, i) => (
                  <div key={i} className="flex items-center justify-between px-3 py-2 text-xs">
                    <span className="font-semibold text-slate-700">{r.wo_number}</span>
                    <span className="text-slate-500">{vendors.find(v => v.id === r.vendor_id)?.name || '—'}</span>
                    <span className="font-semibold text-slate-900">₹{Number(r.total_value || 0).toLocaleString('en-IN')}</span>
                  </div>
                ))}
              </div>
              ...
              <div className="flex gap-3">
                ...
                <button onClick={handleImport} disabled={importing || !projectId || !savedCount}
                  className="flex-1 bg-amber-600 hover:bg-amber-700 disabled:opacity-50 text-white text-sm font-semibold rounded-lg py-2.5 flex items-center justify-center gap-2">
          ...
          <div className="max-w-xl mx-auto">
            <div className="bg-white rounded-xl border border-slate-200 p-6 space-y-4">
              ...
              <a href="/procurement/work-orders"
                className="block w-full text-center bg-amber-600 hover:bg-amber-700 text-white text-sm font-semibold rounded-lg py-2.5">
                Go to Work Orders →
              </a>
            </div>
          </div>
