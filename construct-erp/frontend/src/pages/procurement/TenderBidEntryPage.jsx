...
export default function TenderBidEntryPage() {
  ...
  if (loadT) return <div className="text-slate-400 py-12 text-center">Loading…</div>;
  ...
  return (
    <div className="space-y-6 max-w-4xl">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-xl font-bold text-white">Enter Bid</h1>
      ...
      <div className="bg-slate-800 border border-slate-700 rounded-xl p-5 grid grid-cols-1 md:grid-cols-2 gap-4">
        <h3 className="col-span-full text-sm font-semibold text-slate-300 uppercase tracking-wide">Bid Header</h3>
        <div>
        ...
        <div>
          <label className="block text-xs text-slate-400 mb-1">Bid Reference</label>
        ...
        <div>
          <label className="block text-xs text-slate-400 mb-1">Validity (days)</label>
          <input type="number" value={form.validity_days} onChange={e => setForm(f => ({ ...f, validity_days: e.target.value }))}
            className="w-full px-3 py-2 bg-slate-700 border border-slate-600 rounded-lg text-white text-sm focus:outline-none" />
        </div>
        <div>
          <label className="block text-xs text-slate-400 mb-1">Completion (days)</label>
        ...
        <div>
          <label className="block text-xs text-slate-400 mb-1">Mobilisation Advance %</label>
        ...
        <div>
          <label className="block text-xs text-slate-400 mb-1">Technical Score (0–100)</label>
          <input type="number" value={form.technical_score} onChange={e => setForm(f => ({ ...f, technical_score: e.target.value }))}
            className="w-full px-3 py-2 bg-slate-700 border border-slate-600 rounded-lg text-white text-sm focus:outline-none" />
        </div>
        <div>
          <label className="block text-xs text-slate-400 mb-1">Financial Score (0–100)</label>
      ...
      {bidItems.length > 0 && (
        <div className="bg-slate-800 border border-slate-700 rounded-xl overflow-hidden">
          <div className="p-4 border-b border-slate-700">
            <h3 className="text-sm font-semibold text-white">Rate Schedule</h3>
            <p className="text-xs text-slate-400 mt-0.5">Enter unit rates — bid amount is auto-calculated</p>
          </div>
            ...
            <tbody>
              {bidItems.map((item, idx) => (
                <tr key={item.scope_item_id || idx} className="border-b border-slate-700/50">
                  ...
                  <td className="px-4 py-2.5 text-slate-400 text-xs">{item.quantity}</td>
      ...
      <div className="bg-slate-800 border border-slate-700 rounded-xl p-5">
        <h3 className="text-sm font-semibold text-slate-300 uppercase tracking-wide mb-4">Financial Summary</h3>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div>
            <label className="block text-xs text-slate-400 mb-1">Final Amount</label>
            <div className="px-3 py-2 bg-slate-700/50 border border-slate-600 rounded-lg text-emerald-400 font-bold text-sm">
              {finalAmount > 0 ? `₹${finalAmount.toLocaleString('en-IN', { maximumFractionDigits: 0 })}` : '—'}
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="flex gap-3">
        <button onClick={() => navigate(-1)} className="px-6 py-2.5 border border-slate-600 text-slate-400 rounded-lg text-sm hover:text-white">Cancel</button>
        <button
