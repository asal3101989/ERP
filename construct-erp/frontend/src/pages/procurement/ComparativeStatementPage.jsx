...
export default function ComparativeStatementPage() {
  ...
  const { indent = {}, items = [], vendors = [], existingPO = null } = csData || {};
  ...
  const handleSignOff = () => {
    if (currentStage?.id === 'approve' && !selectedVendorId)
      return toast.error('Select the recommended (L1) vendor first');
    approveMut.mutate({
      stage: currentStage.id,
      data:  currentStage.id === 'approve' ? { selected_vendor_id: selectedVendorId } : {},
    });
  };
    ...
    <div className="bg-[#f4f6f9] min-h-screen">
      ...
      <div className="sticky top-0 z-30 bg-white border-b border-slate-200 shadow-sm px-6 py-3 flex items-center justify-between gap-4 no-print">
        <div className="flex items-center gap-3">
          <button
            ...
            className="w-8 h-8 flex items-center justify-center border border-slate-200 rounded-lg text-slate-500 hover:text-indigo-600 hover:border-indigo-300 transition-all"
          >
            <ArrowLeft className="w-4 h-4" />
          </button>
          <div>
            <h1 className="text-sm font-bold text-slate-800">Comparative Rate Approval</h1>
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-1">
            ...
            {isApproved && (
              <span className="ml-1 px-2.5 py-1 rounded-full text-[11px] font-semibold bg-emerald-100 border border-emerald-300 text-emerald-700">
                ✓ Approved
              </span>
            )}
          </div>
          ...
          {isApproved && (() => {
            ...
            if (existingPO) {
              return (
                <button
                  ...
                  className="flex items-center gap-1.5 px-4 py-2 bg-emerald-50 border border-emerald-300 text-emerald-700 text-xs font-semibold rounded-lg hover:bg-emerald-100 transition-all shadow-sm"
                >
            ...
            return (
              <button
                ...
                className="flex items-center gap-1.5 px-4 py-2 bg-emerald-600 text-white text-xs font-semibold rounded-lg hover:bg-emerald-700 transition-all shadow-sm"
              >
          ...
          {!isApproved && (
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-2 bg-emerald-600 text-white text-xs font-semibold rounded-lg hover:bg-emerald-700 transition-all"
            >
          ...
          <button
            onClick={handlePrint}
            className="flex items-center gap-1.5 px-3 py-2 bg-slate-700 text-white text-xs font-semibold rounded-lg hover:bg-slate-800 transition-all"
          >
          ...
          {currentStage && !isApproved && (
            <button
              ...
              className="flex items-center gap-1.5 px-4 py-2 bg-indigo-600 text-white text-xs font-semibold rounded-lg hover:bg-indigo-700 transition-all disabled:opacity-60"
            >
      ...
      <div className="p-6 max-w-[1600px] mx-auto">
        ...
        <div className="no-print flex items-center justify-between mb-2 gap-2">
          <button
            onClick={() => scroll(-1)}
            className="flex items-center gap-1 px-3 py-1.5 bg-white border border-slate-200 rounded-lg text-slate-600 text-xs font-semibold shadow-sm hover:bg-slate-50 hover:border-indigo-300 transition-all"
          >
            ◀ Scroll Left
          </button>
