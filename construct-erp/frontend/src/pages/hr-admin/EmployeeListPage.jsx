    ...
    >
      ...
      <div className="flex items-start justify-between mb-4">
        <div className="relative">
          <div className="w-12 h-12 rounded-xl flex items-center justify-center text-white font-bold text-lg select-none"
        ...
        <span className={`text-[11px] font-semibold px-2.5 py-1 rounded-full flex items-center gap-1.5 ${status.bg} ${status.text}`}>
      ...
      <div className="mb-3">
        ...
        <p className="text-sm text-gray-600 mt-1.5 font-medium">{emp.designation_name || emp.designation || '—'}</p>
        ...
        <div className="flex flex-wrap gap-3 items-center">
          ...
          <div className="relative flex-1 min-w-48">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm text-gray-800 placeholder-gray-400 focus:outline-none focus:border-indigo-400 focus:ring-2 focus:ring-indigo-50 transition-all"
            />
            {search && (
          ...
          >
            ...
            Filters
            {activeFilters > 0 && (
              <span className="w-4 h-4 rounded-full bg-indigo-600 text-white text-[9px] font-bold flex items-center justify-center">{activeFilters}</span>
            )}
          </button>

          {/* View toggle */}
          <div className="flex items-center gap-1 bg-gray-100 rounded-xl p-1">
            <button onClick={() => setView('card')} className={`p-1.5 rounded-lg transition-all ${view === 'card' ? 'bg-white shadow-sm text-indigo-600' : 'text-gray-400 hover:text-gray-600'}`}>
              <LayoutGrid className="w-4 h-4" />
            </button>
            <button onClick={() => setView('list')} className={`p-1.5 rounded-lg transition-all ${view === 'list' ? 'bg-white shadow-sm text-indigo-600' : 'text-gray-400 hover:text-gray-600'}`}>
              ...
              <div className="flex flex-wrap gap-3 mt-3 pt-3 border-t border-gray-100">
                <div>
                  ...
                  <select
                    ...
                    className="px-3 py-2 bg-gray-50 border border-gray-200 rounded-xl text-sm text-gray-700 focus:outline-none focus:border-indigo-400"
                  >
                ...
                <div>
                  <label className="text-xs text-gray-500 font-medium block mb-1">Employment Type</label>
            ...
            {employees.map(emp => {
              ...
              return (
                ...
                >
                  ...
                  <div className="col-span-2 text-sm text-gray-600 truncate pr-3">{emp.department_name || '—'}</div>
                  <div className="col-span-2 text-sm text-gray-600 truncate pr-3">{emp.designation_name || emp.designation || '—'}</div>
