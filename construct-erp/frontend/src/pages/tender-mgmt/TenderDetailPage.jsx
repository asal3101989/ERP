...
const STATUS_COLORS = {
  prospect:    'bg-slate-100 text-slate-600 border border-slate-200',
...
const NEXT_BTN = {
  ...
  dropped:      { label: 'No Bid',         cls: 'border border-slate-300 text-slate-500 hover:bg-slate-50 bg-white' },
};

const fmtL = (n) => n != null ? `₹${parseFloat(n).toLocaleString('en-IN', { maximumFractionDigits: 0 })}` : '—';
    ...
    <div className="p-6 md:p-8 max-w-7xl mx-auto min-h-screen bg-[#f4f6f9] space-y-5">
      ...
      <div className="flex items-center gap-3">
        <button
          ...
          className="p-2 rounded-lg text-slate-500 hover:text-slate-700 hover:bg-white border border-transparent hover:border-slate-200 transition-colors"
        >
          <ArrowLeft className="w-5 h-5" />
        </button>
        <div className="flex-1 min-w-0">
          <h1 className="text-xl font-bold text-slate-900 truncate">{t.project_name}</h1>
      ...
      {isFinal && (
        <div className={`rounded-xl border p-4 ${
          ...
          {(t.lost_reason || t.dropped_reason) && (
            <p className="text-sm text-slate-600 mt-1">{t.lost_reason || t.dropped_reason}</p>
          )}
        </div>
      )}
        ...
        {tabs.map(tb => (
          <button
            ...
            className={`flex items-center gap-1.5 px-3 py-2 rounded-lg text-sm font-medium transition-colors whitespace-nowrap
              ${tab === tb.key ? 'bg-indigo-600 text-white shadow-sm' : 'text-slate-500 hover:text-slate-700 hover:bg-slate-50'}`}
          >
      ...
      {tab === 'overview' && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
          ...
          <div className="bg-white border border-slate-200 rounded-xl shadow-sm p-5 space-y-2.5">
            <h3 className="text-xs font-semibold text-slate-400 uppercase tracking-widest mb-3">Tender Details</h3>
            {[
              ...
              <div key={k} className="flex justify-between text-sm border-b border-slate-100 pb-2 last:border-0 last:pb-0">
                ...
                <span className="text-slate-800 text-right ml-4 font-medium">{v}</span>
              </div>
            ))}
          </div>
          ...
          <div className="space-y-4">
            <div className="bg-white border border-slate-200 rounded-xl shadow-sm p-5">
              <h3 className="text-xs font-semibold text-slate-400 uppercase tracking-widest mb-3">Win Probability</h3>
              <div className={`text-5xl font-black ${probColor(t.win_probability)}`}>
                {t.win_probability}%
              </div>
            ...
            {t.notice_inviting_tender && (
              <div className="bg-white border border-slate-200 rounded-xl shadow-sm p-5">
                <h3 className="text-xs font-semibold text-slate-400 uppercase tracking-widest mb-3">NIT / Scope</h3>
                <p className="text-sm text-slate-700 whitespace-pre-wrap leading-relaxed">{t.notice_inviting_tender}</p>
              </div>
            )}

            {t.our_rank && (
              <div className="bg-white border border-slate-200 rounded-xl shadow-sm p-4">
                <p className="text-xs text-slate-500 font-medium">Our Rank in Evaluation</p>
                <p className="text-3xl font-black text-indigo-600 mt-1">L{t.our_rank}</p>
                {t.competitor_info && (
                  <p className="text-xs text-slate-500 mt-2">{t.competitor_info}</p>
                )}
              </div>
            )}
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
          ...
          <div className="bg-white border border-slate-200 rounded-xl shadow-sm p-5">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-xs font-semibold text-slate-400 uppercase tracking-widest">Earnest Money Deposit (EMD)</h3>
              {!emdEdit && (
                <button
              ...
              <div className="space-y-3">
                  ...
                  <>
                    ...
                    {emdForm.emd_submitted && (
                      <div className="grid grid-cols-2 gap-3">
                        ...
                        <div>
                          <label className="text-xs text-slate-500 block mb-1">Ref / DD No.</label>
                          <input
                        ...
                        <div>
                          <label className="text-xs text-slate-500 block mb-1">Valid Upto</label>
                          <input
                            type="date"
                ...
                <div className="flex gap-2 pt-1">
                  <button
                    ...
                    className="flex-1 py-2 border border-slate-200 text-slate-500 rounded-lg text-sm hover:bg-slate-50"
                  >
                    Discard
                  </button>
                  <button
          ...
          <div className="space-y-4">
            <div className="bg-white border border-slate-200 rounded-xl shadow-sm p-5">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-xs font-semibold text-slate-400 uppercase tracking-widest">Key Dates</h3>
                {!emdEdit && (
                  <button
                    onClick={() => { setEmdEdit(true); setEmdForm({ ...t }); }}
                    className="text-xs text-indigo-600 hover:text-indigo-700 font-medium"
            ...
            <div className="bg-white border border-slate-200 rounded-xl shadow-sm p-5">
              <h3 className="text-xs font-semibold text-slate-400 uppercase tracking-widest mb-4">Our Submission</h3>
              <div className="space-y-2.5">
                {[
                  ['Our Bid Amount',      fmtL(t.our_bid_amount)],
                  ...
                  <div key={k} className="flex justify-between text-sm border-b border-slate-100 pb-2 last:border-0 last:pb-0">
                    <span className="text-slate-500 shrink-0">{k}</span>
                    <span className="text-slate-800 font-medium text-right max-w-[60%]">{v}</span>
                  </div>
                ))}
              </div>
            </div>
      ...
      {tab === 'cost' && (
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <h3 className="text-base font-semibold text-slate-800">Cost Estimate</h3>
            <div className="flex gap-2">
              {costEdit ? (
                <>
                  <button
                    onClick={() => setCostEdit(null)}
                    className="px-3 py-1.5 border border-slate-200 text-slate-500 rounded-lg text-sm hover:bg-slate-50"
                  >
                    Discard
                  </button>
                  <button
          ...
          {t.bid_value && costTotal > 0 && (
            <div className="bg-white border border-slate-200 rounded-xl shadow-sm p-4 flex flex-wrap items-center gap-6 text-sm">
              <div>
                <p className="text-slate-500 text-xs">Est. Bid Value</p>
                <p className="text-slate-800 font-bold">{fmtL(t.bid_value)}</p>
              </div>
              <div>
                <p className="text-slate-500 text-xs">Cost Total</p>
                <p className="text-slate-800 font-bold">{fmtL(costTotal)}</p>
              </div>
              <div>
                <p className="text-slate-500 text-xs">Gross Margin</p>
                <p className={`font-bold ${(t.bid_value - costTotal) >= 0 ? 'text-emerald-600' : 'text-red-600'}`}>
          ...
          <div className="bg-white border border-slate-200 rounded-xl shadow-sm overflow-x-auto">
              ...
              <thead>
                <tr className="border-b border-slate-100 bg-slate-50">
                  {['Category', 'Description', 'Qty', 'Unit', 'Rate (₹)', 'Amount (₹)', 'Margin %', ''].map(h => (
                    <th key={h} className="text-left px-3 py-2.5 text-slate-500 font-semibold text-xs whitespace-nowrap uppercase tracking-wide">
                      {h}
                    </th>
                  ))}
                </tr>
              ...
              <tbody>
                  ...
                  <tr key={idx} className="border-b border-slate-100 hover:bg-slate-50/60">
                    {costEdit ? (
                      <>
                        ...
                        <td className="px-3 py-2 text-slate-600 text-xs">{item.amount ? `₹${parseFloat(item.amount).toLocaleString('en-IN')}` : '—'}</td>
                        <td className="px-3 py-2">
                      ...
                      <>
                        ...
                        <td className="px-3 py-2.5 text-slate-500 text-xs">{item.quantity}</td>
                        <td className="px-3 py-2.5 text-slate-500 text-xs">{item.unit}</td>
                        <td className="px-3 py-2.5 text-slate-600 text-xs">{fmtL(item.rate)}</td>
                        <td className="px-3 py-2.5 text-slate-800 font-medium">{fmtL(item.amount)}</td>
                        <td className="px-3 py-2.5 text-slate-500 text-xs">{item.margin_pct}%</td>
                        <td />
                      </>
                    )}
                  </tr>
                ...
                {costTotal > 0 && (
                  <tr className="border-t border-slate-200 bg-slate-50">
                    <td colSpan={5} className="px-3 py-2.5 text-slate-600 font-semibold text-sm">Total</td>
            ...
            {costItems.length === 0 && !costEdit && (
              <div className="text-center py-10 text-slate-400 text-sm">No cost estimate — click Edit to start</div>
            )}
          </div>
        </div>
      )}
      ...
      {tab === 'docs' && (
        <div className="space-y-4">
          <div className="bg-white border border-slate-200 rounded-xl shadow-sm p-4">
            <label className="block text-xs text-slate-500 font-medium mb-2">Upload Documents</label>
            <input
              type="file"
              multiple
              onChange={async (e) => {
                ...
                <tbody>
                  {t.documents.map(doc => (
                    <tr key={doc.id} className="border-b border-slate-100 hover:bg-slate-50/60">
                      ...
                      <td className="px-4 py-3 text-slate-400 text-xs">{dayjs(doc.uploaded_at).format('DD MMM YY HH:mm')}</td>
                      <td className="px-4 py-3">
                        <button
                          onClick={async () => {
                ...
                {t.activity_log.map(log => (
                  <div key={log.id} className="flex gap-4">
                    ...
                    <div className="flex-1 pb-1">
                      ...
                      {log.note && <p className="text-sm text-slate-700 mt-1">{log.note}</p>}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 p-4 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md border border-slate-100">
            ...
            <div className="p-5 space-y-4">
              <div>
                <label className="block text-xs text-slate-500 font-medium mb-1.5">Note (optional)</label>
                <textarea
                  value={transNote}
            ...
            <div className="flex gap-3 p-5 pt-0">
              <button
                onClick={() => setShowTransition(false)}
                className="flex-1 py-2.5 border border-slate-200 text-slate-500 rounded-xl text-sm hover:bg-slate-50"
              >
                Cancel
              </button>
              <button
