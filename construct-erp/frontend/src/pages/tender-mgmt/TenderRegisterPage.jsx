...
const STATUS_COLORS = {
  prospect:     'bg-slate-100 text-slate-600 border border-slate-300',
  ...
  dropped:      'bg-slate-100 text-slate-400 border border-slate-200',
};
...
export default function TenderRegisterPage() {
  ...
  const inputCls = 'w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-lg text-sm text-slate-900 placeholder:text-slate-400 focus:outline-none focus:border-indigo-400 transition-all';
    ...
    <div className="p-6 md:p-8 max-w-7xl mx-auto min-h-screen bg-[#f4f6f9]">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
        <div>
          ...
          <h1 className="text-2xl font-bold text-slate-900">Tender Register</h1>
          <p className="text-sm text-slate-400 mt-0.5">Track tenders received from clients — bid to win projects</p>
        </div>
        <button
      ...
      <div className="grid grid-cols-3 md:grid-cols-6 gap-4 mb-6">
        {KPI_STYLES.map(k => (
          <div key={k.key}
            ...
            <div className={`text-2xl font-bold font-mono ${k.numCls}`}>{stats[k.key] ?? 0}</div>
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        <div className="bg-white border border-slate-200 rounded-xl p-4 shadow-sm flex items-center justify-between">
          ...
          <span className="text-xl font-bold text-emerald-600">{stats.win_rate_pct ?? 0}%</span>
        </div>
        <div className="bg-white border border-slate-200 rounded-xl p-4 shadow-sm flex items-center justify-between">
          <span className="text-sm text-slate-500">Total Pipeline</span>
          <span className="text-sm font-bold text-slate-800">{fmtCr(stats.total_pipeline_value)}</span>
        </div>
        <div className="bg-white border border-slate-200 rounded-xl p-4 shadow-sm flex items-center justify-between">
          <span className="text-sm text-slate-500">Won Value</span>
          <span className="text-sm font-bold text-emerald-600">{fmtCr(stats.total_won_value)}</span>
        </div>
      </div>
      ...
      <div className="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden">
            ...
            <tbody>
              {tenders.map(t => (
                <tr key={t.id}
                  ...
                  <td className="px-4 py-3 font-mono text-indigo-600 text-xs font-semibold whitespace-nowrap">{t.opportunity_number}</td>
                  <td className="px-4 py-3 text-slate-500 text-xs">{t.client_tender_ref || '—'}</td>
                  <td className="px-4 py-3 text-slate-800 font-medium whitespace-nowrap">{t.client_name}</td>
                  <td className="px-4 py-3 text-slate-600 max-w-[180px] truncate">{t.project_name}</td>
                  <td className="px-4 py-3 text-slate-500 text-xs">{t.tender_category || '—'}</td>
                  <td className="px-4 py-3 text-slate-500 text-xs">{t.client_type || '—'}</td>
                  <td className="px-4 py-3 text-slate-700 font-medium whitespace-nowrap">{fmtL(t.bid_value)}</td>
        ...
        {!isLoading && tenders.length === 0 && (
          <div className="py-20 text-center border-t border-slate-100">
            ...
            <p className="text-slate-500 font-medium">No tenders registered yet</p>
            <p className="text-xs text-slate-400 mt-1">Click "Register Tender" to log a new tender notice</p>
          </div>
        )}
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between p-6 border-b border-slate-100">
              <div>
                <h2 className="text-lg font-bold text-slate-900">Register New Tender</h2>
                <p className="text-xs text-slate-400 mt-0.5">Log a tender notice received from a client</p>
              </div>
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="p-3 bg-slate-50 rounded-lg border border-slate-200">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <label htmlFor="emd_req" className="text-sm text-slate-700 font-medium">EMD Required</label>
                </div>
                {form.emd_required && (
            ...
            <div className="flex gap-3 px-6 pb-6">
              <button onClick={() => setShowNew(false)}
                className="flex-1 py-2.5 border border-slate-200 text-slate-500 rounded-lg text-sm hover:bg-slate-50 transition-colors">
                Cancel
              </button>
              <button
