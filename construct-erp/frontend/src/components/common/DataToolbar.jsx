    ...
    <div className="flex items-center gap-2">
      {templateData && (
        <button onClick={handleDownloadTemplate} className="btn-secondary !text-teal-400 !border-teal-400/20 hover:bg-teal-400/10 py-1.5 px-3">
          <Download size={14} className="mr-2" /> <span className="hidden sm:inline text-[10px] font-black uppercase tracking-widest">Template</span>
        </button>
      )}
      <div className="relative">
        <button className="btn-secondary flex items-center gap-2 border-white/10 hover:bg-white/5 py-1.5 px-3">
          <Upload size={14} /> <span className="hidden sm:inline text-[10px] font-black uppercase tracking-widest">Import</span>
        </button>
        <input 
          type="file" 
      ...
      <button onClick={handleExport} className="btn-secondary flex items-center gap-2 border-white/10 hover:bg-white/5 py-1.5 px-3">
        <Download size={14} /> <span className="hidden sm:inline text-[10px] font-black uppercase tracking-widest">Export</span>
      </button>
      {!hideAdd && (
        <button onClick={onAdd} className="btn-primary flex items-center gap-2 py-1.5 px-4 shadow-lg shadow-blue-500/20">
          <Plus size={14} /> <span className="hidden sm:inline text-[10px] font-black uppercase tracking-widest">{addLabel}</span>
        </button>
      )}
    </div>
  );
